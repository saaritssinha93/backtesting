...
DIR_15M = "etf_indicators_15min"      # <-- 15m intraday indicators
...
def build_signals_for_ticker(ticker: str) -> List[dict]:
    ...
    path_15m = os.path.join(DIR_15M, f"{ticker}_etf_indicators_15min.csv")
...
def apply_portfolio_rules(evaluated_df: pd.DataFrame) -> Tuple[pd.DataFrame, dict]:
    """
    Capital-only portfolio simulation (NO ticker lock).
    """
    ...
    for i, row in df.iterrows():
        ...
        # release realized trades exited by now
...
def main() -> None:
    tickers_i = list_tickers(DIR_15M, "_etf_indicators_15min.csv")
    ...
    for t in tickers_in_signals:
        path = os.path.join(DIR_15M, f"{t}_etf_indicators_15min.csv")
    ...
    out_df_ruled.to_csv(eval_path, index=False)
