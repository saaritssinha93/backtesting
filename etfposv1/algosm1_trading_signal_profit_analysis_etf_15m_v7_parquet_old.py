...
def apply_portfolio_rules(evaluated_df: pd.DataFrame) -> Tuple[pd.DataFrame, dict]:
    """
    Capital-only portfolio simulation (NO ticker lock) WITHOUT PROFIT RECYCLING (no compounding).

    Key behavior:
    - Each taken trade deploys a fixed CAPITAL_PER_SIGNAL_RS from the *current* cash pool.
    - When a trade exits:
        * The principal (invested) is released back to cash.
        * LOSSES reduce cash (so future capacity can shrink).
        * PROFITS are **NOT** added to cash. Profits are tracked separately as `banked_profit`
          and included in final equity, but they are not available for subsequent entries.

    This matches: "don't add profit to capital for subsequent trade calculations".
    """
    df = evaluated_df.sort_values(["signal_time_ist", "ticker"]).reset_index(drop=True).copy()

    # Deployable principal pool (used for future entries). Starts at MAX_CAPITAL_RS.
    cash: float = float(MAX_CAPITAL_RS)

    # Profit that is realised but NOT reinvested (kept separate from cash).
    banked_profit: float = 0.0

    # Open trades lock deployed principal until their release_time.
    ...
    # Running realised P&L and count (informational)
    ...
    # Running portfolio snapshots (updated at each signal processing step)
    ...
    # NEW (optional, does not break any existing columns)
    df["banked_profit_rs_to_date"] = np.nan
    df["total_equity_after_entry"] = np.nan  # cash + locked principal + banked profit
    ...
    for i, row in df.iterrows():
        ...
        #    - Principal returns to cash.
        #    - Losses reduce cash.
        #    - Profits are banked (NOT added to cash).
        # ------------------------------------------------------------
        ...
        for tr in open_trades:
            if (not tr["is_unrealized"]) and (tr["release_time"] <= signal_time):
                ...
                # Principal always returns.
                # Losses reduce cash; profits do NOT increase cash.
                cash_delta = inv + min(pnl, 0.0)
                cash += float(cash_delta)

                # Bank only positive realised profit (not used for next entries)
                if pnl > 0:
                    banked_profit += float(pnl)

                cash_after_rel = float(cash)

                cash_released_now += float(cash_delta)
                realised_pnl_cum += float(pnl)
                ...
                booking_ledger_rows.append({
                    ...
                    "return_value_rs": rv,
                    "pnl_rs": pnl,
                    "pnl_pct": (pnl / inv * 100.0) if inv else np.nan,
                    "cash_before_release_rs": cash_before_rel,
                    "cash_after_release_rs": cash_after_rel,
                    "banked_profit_rs_to_date": float(banked_profit),
                })
            else:
        ...
        # ------------------------------------------------------------
        # 2) TAKE / SKIP the signal based on cash AFTER releases
        # ------------------------------------------------------------
        if cash < float(CAPITAL_PER_SIGNAL_RS):
            ...
            df.at[i, "banked_profit_rs_to_date"] = float(banked_profit)
            df.at[i, "total_equity_after_entry"] = float(cash) + locked_after + float(banked_profit)
            continue

        # Take the trade (principal deployment)
        df.at[i, "taken"] = True
        df.at[i, "skip_reason"] = ""
        ...
        # Determine when the principal is released and what value returns
        ...
        if is_unreal:
            ...
            if pd.isna(exit_time):
                # Defensive: treat as unrealised if exit_time missing
                release_time = FAR_FUTURE_LOCK
                is_unreal = True
                return_value = float(row.get("mtm_value", invested))
            else:
    ...
    # ------------------------------------------------------------
    # FINAL EQUITY
    # - Release any realised trades still in open_trades (no later signal occurred)
    # - Unrealised trades remain valued at mtm_value
    # - Add banked_profit to equity (but it's not deployable capital)
    # ------------------------------------------------------------
    ...
    for tr in open_trades:
        ...
        if tr["is_unrealized"]:
            # Count full MTM value (principal + MTM P&L) in equity
            open_positions_value += rv
        else:
            # Principal returns; losses reduce cash; profits are banked
            cash += inv + min(pnl, 0.0)
            if pnl > 0:
                banked_profit += pnl

    final_equity = float(cash) + float(open_positions_value) + float(banked_profit)
    ...
    summary = {
        ...
        # Deployable principal cash pool (profits not included)
        "cash_end": float(cash),

        # Banked profit not deployable, but part of equity
        "banked_profit": float(banked_profit),
...
def main() -> None:
    ...
    print(f"  Banked profit (not reinvested) : {fmt_rs(summary.get('banked_profit', 0.0))}")
    ...
    if isinstance(ledger_df, pd.DataFrame) and (not ledger_df.empty):
        ...
        # Save ledger files (PARQUET only)
