# -*- coding: utf-8 -*-
"""
eqidv4_eod_scheduler_for_15mins_data.py  (FIXED)
================================================
...
Run:
    python eqidv4_eod_scheduler_for_15mins_data.py

Optional:
    python eqidv4_eod_scheduler_for_15mins_data.py --buffer-sec 75 --max-workers 24
"""
...
# ---------------------------------------------------------------------
# Locate eqidv4 folder (robust even if your repo is nested oddly)
# ---------------------------------------------------------------------
...
EQIDV4_DIR = _find_core_dir(SCRIPT_DIR)
if str(EQIDV4_DIR) not in sys.path:
    sys.path.insert(0, str(EQIDV4_DIR))

import trading_data_continous_run_historical_alltf_v3_parquet_stocksonly as core  # noqa: E402


# ---------------------------------------------------------------------
# Monkey-patch kite session to read api_key.txt / access_token.txt from EQIDV4_DIR
...
def setup_kite_session_from_eqidv4_dir():
    from kiteconnect import KiteConnect  # imported here to avoid import costs on module import
    api_key = _read_first_line(EQIDV4_DIR / "api_key.txt")
    access_token = _read_first_line(EQIDV4_DIR / "access_token.txt")
    kc = KiteConnect(api_key=api_key)
    kc.set_access_token(access_token)
    return kc

core.setup_kite_session = setup_kite_session_from_eqidv4_dir
...
def main() -> None:
    ...
    print("[LIVE] EQIDV4 15m scheduler started.")
    print(f"       Using EQIDV4_DIR: {EQIDV4_DIR}")
    ...
    while True:
        ...
        print(f"[RUN ] Updating EQIDV4 15m for slot {slot_end.strftime('%H:%M')} at {dt.strftime('%Y-%m-%d %H:%M:%S%z')}")
        try:
            run_update_15m_once(
