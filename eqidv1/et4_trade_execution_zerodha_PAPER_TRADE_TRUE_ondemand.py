...
EXPECTED_COLUMNS = [
    ...
    "Price", "Quantity"  # Added "Quantity" to match the latest execute_paper_trades function
]
...
# Initialize a set to track tickers already simulated/traded for today
...
def execute_paper_trades(last_trading_day, kite, api_semaphore, paper_trade=True):
    ...
    for index, row in new_trades_df.iterrows():
        try:
            ...
            order_type='MARKET'  # MARKET Order for Cover
            ...
            if paper_trade:
                ...
                # Simulate Cover Order Placement
                simulated_order_id = f"PAPER-{signal_id}-{datetime.now(india_tz).strftime('%H%M%S')}"
                logging.info(
                    f"[SIMULATION] Placed Cover {transaction_type} order for {ticker} at Market Price with SL at {stop_loss_price}. "
                    f"Signal_ID: {signal_id}, Simulated Order ID: {simulated_order_id}"
                )
                print(
                    f"[SIMULATION] Placed Cover {transaction_type} order for {ticker} at Market Price with SL at {stop_loss_price}. "
                    f"Signal_ID: {signal_id}, Simulated Order ID: {simulated_order_id}"
                ...
                log_simulation_result(
                    ...
                    price=price,  # Assuming 'price' as simulated entry price
                    quantity=quantity,
                    target_price=None,         # Not applicable for simulation
                ...
                with api_semaphore:
                    try:
                        # 1. Place Cover Order (Market Entry + SL)
                        cover_order_id = kite.place_order(
                            variety=variety,
                            exchange=exchange,
                            ...
                            quantity=quantity,
                            product=product,
                            order_type=order_type,           # MARKET Order
                            # price parameter omitted for MARKET orders
                            trigger_price=stop_loss_price,   # 2% SL
                            validity=validity,
                            tag=tag
                        )

                        logging.info(
                            f"Placed Cover {transaction_type} MARKET order for {ticker} with SL at {stop_loss_price}. "
                            f"Signal_ID: {signal_id}, Order ID: {cover_order_id}"
                        )
                        print(
                            f"Placed Cover {transaction_type} MARKET order for {ticker} with SL at {stop_loss_price}. "
                            f"Signal_ID: {signal_id}, Order ID: {cover_order_id}"
                        ...
                        # 2. Monitor Cover Order Fill
                        filled = False
                        filled_price = price  # Initialize with expected price
                        while not filled:
                            orders = kite.orders()
                            for order in orders:
                                if order['order_id'] == cover_order_id and order['status'] == 'COMPLETE':
                                    filled = True
                                    filled_price = float(order['average_price']) if order['average_price'] else price
                                    break
                            if not filled:
                                time.sleep(2)  # Wait before next check

                        logging.info(f"Cover order {cover_order_id} for {ticker} has been filled at {filled_price}.")
                        print(f"Cover order {cover_order_id} for {ticker} has been filled at {filled_price}.")

                        # 3. Place Target Order (Limit Sell or Buy based on transaction_type)
                        if transaction_type == 'BUY':
                            target_transaction_type = 'SELL'
                            target_price = round(filled_price * 1.02, 2)  # 2% Target
                        else:
                            target_transaction_type = 'BUY'
                            target_price = round(filled_price * 0.98, 2)  # 2% Target
                        ...
                        logging.info(
                            f"Placed Target {target_transaction_type} LIMIT order for {ticker} at {target_price}. "
                            f"Signal_ID: {signal_id}, Target Order ID: {target_order_id}"
                        )
                        print(
                            f"Placed Target {target_transaction_type} LIMIT order for {ticker} at {target_price}. "
                            f"Signal_ID: {signal_id}, Target Order ID: {target_order_id}"
                        ...
                        # 4. Implement OCO Logic: Monitor Target and SL Orders
                        #    Once either is filled, cancel the other
                        #    This is a simple polling mechanism; for production, consider WebSockets

                        oco_completed = False
                        while not oco_completed:
                            orders = kite.orders()
                            target_filled = False
                            sl_filled = False
                            for order in orders:
                                if order['order_id'] == target_order_id and order['status'] == 'COMPLETE':
                                    ...
                                    break
                                if order['order_id'] == cover_order_id and order['status'] == 'COMPLETE':
                                    # SL has been triggered and Cover Order is complete
                                    sl_filled = True
                                    break
                            if target_filled:
                                # Cancel SL Order if Target is filled
                                try:
                                    kite.cancel_order(order_id=cover_order_id, variety=kite.CO)
                                    logging.info(f"Target order {target_order_id} filled. Cancelled SL order {cover_order_id}.")
                                    print(f"Target order {target_order_id} filled. Cancelled SL order {cover_order_id}.")
                                except Exception as e:
                                    logging.error(f"Failed to cancel SL order {cover_order_id}: {e}")
                                    print(f"Failed to cancel SL order {cover_order_id}: {e}")
                                oco_completed = True
                            elif sl_filled:
                                # Cancel Target Order if SL is filled
                                try:
                                    kite.cancel_order(order_id=target_order_id, variety='co')
                                    logging.info(f"SL order {cover_order_id} triggered. Cancelled Target order {target_order_id}.")
                                    print(f"SL order {cover_order_id} triggered. Cancelled Target order {target_order_id}.")
                                except Exception as e:
                        ...
                        simulated_tickers.add(ticker)
                        # Add this signal_id to executed_signals after successful order placement and OCO handling
                        executed_signals.add(signal_id)

                    except Exception as e:
                        logging.error(f"Failed to place Cover order for {ticker}: {e}")
                        print(f"Failed to place Cover order for {ticker}: {e}")
...
class CSVChangeHandler(FileSystemEventHandler):
    def __init__(self, target_file, callback, debounce_delay=5):
        super().__init__()
...
def main():
    ...
    # Determine last trading day (assuming today if it's a trading day)
    ...
    def trades_callback():
        logging.info("CSV file change detected. Executing trades.")
        print("CSV file change detected. Executing trades.")
        execute_paper_trades(last_trading_day, kite, api_semaphore, paper_trade=PAPER_TRADE)

    # Setup Watchdog Observer for the specific papertrade file
    target_filename = CSV_FILENAME_PATTERN.format(last_trading_day)
    event_handler = CSVChangeHandler(target_file=target_filename, callback=trades_callback, debounce_delay=5)
    observer = Observer()
    observer.schedule(event_handler, path=CSV_DIRECTORY, recursive=False)
    observer.start()
    logging.info(f"Started Watchdog Observer for {target_filename}.")
    print(f"Started Watchdog Observer for {target_filename}.")

    # Optionally, execute trades immediately if within trading window
    start_time_naive = datetime.combine(now.date(), datetime_time(9, 31, 0))  # 9:31:00 AM
    end_time_naive = datetime.combine(now.date(), datetime_time(18, 30, 0))   # 6:30:00 PM
    ...
    if start_time <= now <= end_time:
        ...
        logging.info("Current time is before the trading window. Waiting to start monitoring.")
        print("Current time is before the trading window. Waiting to start monitoring.")
    else:
        logging.warning("Current time is past the trading window. Trades will execute upon next file change.")
        print("Current time is past the trading window. Trades will execute upon next file change.")
    ...
    try:
        ...
        observer.stop()
        logging.info("Shutting down Watchdog Observer and exiting script.")
        print("Shutting down Watchdog Observer and exiting script.")
    observer.join()
