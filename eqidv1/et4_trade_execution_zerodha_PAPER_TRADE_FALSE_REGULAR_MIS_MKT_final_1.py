# -*- coding: utf-8 -*-
"""
Created on Mon Mar  3 14:48:26 2025
...
Code 2 (Concurrent Real Trades) with Forced Close at 15:15:
 - Reads trades from 'papertrade_{YYYY-MM-DD}.csv'.
 - Places real or paper trades for each CSV row, concurrently.
 - Does NOT skip repeated tickers. Multiple trades for the same ticker are allowed.
 - Still avoids re-trading the *same Signal_ID* (so each CSV row is only traded once).
 - Places MIS order with 2% target and 2% SL, both snapped to 0.5 increments.
 - Spawns a background thread per real trade, so multiple real trades can run in parallel.
 - Any real trade still open at 15:15 is forcibly closed automatically.
"""
...
# 1) Configuration
############################################
PAPER_TRADE = False  # Toggle True/False: simulation vs. real trades
...
def load_executed_signals():
    ...
    to avoid re-trading the same CSV row multiple times.
    """
...
def current_market_price(kite_, ticker):
    ...
    Returns 0.0 if error or if kite_ is not set.
    """
    if kite_ is None:
        return 0.0
    try:
...
def place_mis_trade(kite_, ticker, transaction_type, quantity):
    ...
    For real trades:
      ...
      6. Wait for fill of either one, or forcibly close at 15:15 if not yet filled.
    """
    def _log(msg):
        logging.info(msg)
        print(msg)
    ...
    india_tz = pytz.timezone("Asia/Kolkata")
    # Define the forced close time as 15:15 for the current day
    ...
    # 6) Wait for either target or SL to fill; force close at 15:15 otherwise
    trade_active = True
    while trade_active:
        now = datetime.now(india_tz)

        # Check if it's past 15:15 => forcibly exit if still active
        if now >= forced_close_dt:
            _log(f"[REAL] Time >= 15:15. Forcing close for {ticker}.")
            try:
                # Cancel target and SL if they are still open
                kite_.cancel_order(kite_.VARIETY_REGULAR, target_order_id)
            except Exception as ce_tgt:
                _log(f"[REAL] Error canceling Target {target_order_id}: {ce_tgt}")
            try:
                kite_.cancel_order(kite_.VARIETY_REGULAR, stop_loss_order_id)
            except Exception as ce_sl:
                _log(f"[REAL] Error canceling SL {stop_loss_order_id}: {ce_sl}")

            # Send a market order in the opposite direction to close
            if transaction_type.upper() == "BUY":
                close_txn_type = "SELL"
            else:
                close_txn_type = "BUY"
            ...
            try:
                ...
                _log(f"[REAL] Forced close order placed => {close_txn_type} {ticker} qty={quantity} (ID={close_order_id})")
        ...
        # Otherwise check if target or SL has been filled
        orders = kite_.orders()
...
############################################
# 9) Concurrency: Real Trade in a Thread
############################################
def real_trade_thread(
    kite_,
    ticker,
    txn_type,
    quantity,
    ...
    """
    Runs place_mis_trade in a background thread so multiple trades
    can be placed concurrently. We use a semaphore if given,
    so we don't exceed MAX_CONCURRENT_TRADES at once.
    """
    if semaphore:
        semaphore.acquire()
    ...
    try:
        ...
        # On success callback (to log results, etc.)
        on_success(trade_info)
    except Exception as e:
        # On failure callback (to revert or log the error)
        on_failure(e)
    finally:
        if semaphore:
            semaphore.release()
...
def execute_trades(last_trading_day, kite_, api_semaphore, paper_trade=True):
    ...
    2) For each row, skip only if 'Signal_ID' was already executed (so we don't do the exact same line again).
    3) Force quantity=1 always.
    4) If paper_trade => log a simulation.
       If real => *spawn a thread* that calls place_mis_trade, so multiple trades are concurrent.
    5) Store 'Signal_ID' in executed_signals to avoid repeated trades on the *exact same row*.
    """
    ...
    india_tz = pytz.timezone('Asia/Kolkata')
    threads = []  # hold references to background threads
    ...
    for idx, row in fresh_df.iterrows():
        try:
            ...
            # Mark this signal as executed so we don't re-do it
            ...
            if paper_trade:
                ...
                # REAL TRADE: Spawn a background thread
                if kite_ is None:
                ...
                def on_success(trade_info):
                    """
                    Called once place_mis_trade finishes successfully in the thread.
                    We'll log the final result to 'trade_log'.
                    """
                    log_trade_result(
                        ticker=ticker,
                        transaction_type=txn_type,
                    ...
                    # If you wanted to do anything else upon success, do it here.

                def on_failure(err):
                    """
                    Called if place_mis_trade raises an exception in the thread.
                    """
                    msg = f"[REAL] Trade failed for {ticker}, reason: {err}"
                    logging.error(msg)
                    print(msg)
                    # If you want to remove the signal_id from executed_signals to retry, do that here.
                    # executed_signals.remove(signal_id)

                # Kick off thread so we do not block
                t = threading.Thread(
                    ...
                    args=(
                        ...
                        on_success,
                        on_failure,
                        api_semaphore,  # pass the concurrency semaphore
                    ),
                    daemon=True
                )
                t.start()
    ...
    # Save updated signals so we don't do them again
    save_executed_signals(executed_signals)

    logging.info("Finished submitting trades. They are running in background threads.")
    print("Finished submitting trades. Trades are in background threads now.")

    # If you want to wait for all to finish before returning, uncomment:
    # for th in threads:
    #     th.join()
    # print("All background trade threads completed.")
    # logging.info("All background trade threads completed.")
...
def main():
    ...
    # Example: Trading window 9:31 to 15:30
    ...
    if now < start_time:
        ...
        print("Currently after trading window. Watchdog still runs, but trades might be out of hours.")
        logging.warning("Currently after trading window. We might place trades if code triggers.")
    else:
    ...
    def get_last_trading_day():
        d = now.date()
        # Step backward if weekend or holiday
    ...
    # We use a semaphore to limit concurrency to MAX_CONCURRENT_TRADES
