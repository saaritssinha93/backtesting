# -*- coding: utf-8 -*-
"""
Code 2 (Concurrent Real Trades):
 - Reads trades from 'papertrade_{YYYY-MM-DD}.csv'.
 - Places real or paper trades for each CSV row, concurrently.
 - Does NOT skip repeated tickers. Multiple trades for same ticker are allowed.
...
def place_mis_trade(kite_, ticker, transaction_type, quantity):
    ...
    For real trades:
      ...
      6. Wait for fill of either one, then cancel the other.

    This is a blocking function, but we'll call it from a separate thread,
    so the main code can proceed with other trades.
    """
    def _log(msg):
        logging.info(msg)
        print(msg)
        ...
        try:
            ...
            if paper_trade:
                ...
                t = threading.Thread(
                    ...
                    daemon=True  # or False if you want the threads to keep the program alive
                )
                t.start()
                threads.append(t)
