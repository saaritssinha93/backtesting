...
def execute_paper_trades(last_trading_day, kite, api_semaphore, paper_trade=True):
    ...
    for index, row in new_trades_df.iterrows():
        try:
            ...
            # Hardcode quantity = 1 or pull from CSV if desired
            quantity = 1
            ...
            if paper_trade:
                # Avoid duplicate simulation for the same ticker
                ...
                logging.info(
                    f"[SIM] Market {transaction_type} for {ticker} ~{price}, "
                    f"Target={target_price}, StopLoss={stop_loss_price}, ID={simulated_order_id}"
                )
                print(
                    f"[SIM] Market {transaction_type} for {ticker} ~{price}, "
                    f"Target={target_price}, StopLoss={stop_loss_price}, ID={simulated_order_id}"
            ...
            # 2) Live Trading (Market Entry + separate Target & SL) -----------
            else:
                # Avoid duplicate live trades
                if ticker in simulated_tickers:
                    logging.info(f"Ticker {ticker} has already been traded live. Skipping duplicate.")
                    print(f"Ticker {ticker} has already been traded live. Skipping duplicate.")
                    continue

                with api_semaphore:
                    try:
                        # a) Place Market Entry
                        entry_order_id = kite.place_order(
                            ...
                            quantity=quantity,
                            product=product,
                            order_type=kite.ORDER_TYPE_MARKET,  # Market
                            validity=kite.VALIDITY_DAY,
                            tag=tag
                        )
                        logging.info(
                            f"[LIVE] MARKET {transaction_type} => {ticker} x {quantity}, order_id={entry_order_id}"
                        )
                        print(
                            f"[LIVE] MARKET {transaction_type} => {ticker} x {quantity}, order_id={entry_order_id}"
                        )

                        # b) Immediately place Target Order (separate order)
                        #    If trend_type = 'bullish' => target is SELL limit
                        #    If trend_type = 'bearish' => target is BUY limit
                        target_txn_type = 'SELL' if transaction_type == 'BUY' else 'BUY'
                        target_order_id = kite.place_order(
                            variety=variety,
                            exchange=exchange,
                            tradingsymbol=tradingsymbol,
                            transaction_type=target_txn_type,
                            quantity=quantity,
                            product=product,
                            order_type=kite.ORDER_TYPE_LIMIT,
                            price=target_price,  # The limit price for target
                            validity=kite.VALIDITY_DAY,
                            tag=tag
                        )
                        logging.info(
                            f"[LIVE] TARGET => {ticker} x {quantity} @ {target_price}, order_id={target_order_id}"
                        )
                        print(
                            f"[LIVE] TARGET => {ticker} x {quantity} @ {target_price}, order_id={target_order_id}"
                        )

                        # c) Immediately place Stop Loss Order (SL or SL-M):
                        #    If you're long => you place SELL SL
                        #    If you're short => you place BUY SL
                        #    Typically, for an SL order you need both price= and trigger_price= 
                        #    For an SL-M, you only pass trigger_price (and order_type=ORDER_TYPE_SLM).
                        sl_txn_type = 'SELL' if transaction_type == 'BUY' else 'BUY'

                        # Example using an SL-M order:
                        stoploss_order_id = kite.place_order(
                            ...
                            trigger_price=stop_loss_price,    # triggers a market order if reached
                            validity=kite.VALIDITY_DAY,
                            tag=tag
                        )
                        logging.info(
                            f"[LIVE] STOPLOSS => {ticker} x {quantity}, Trigger={stop_loss_price}, order_id={stoploss_order_id}"
                        )
                        print(
                            f"[LIVE] STOPLOSS => {ticker} x {quantity}, Trigger={stop_loss_price}, order_id={stoploss_order_id}"
                        )

                        # d) Mark signal as executed
                        executed_signals.add(signal_id)
                        simulated_tickers.add(ticker)

                    except Exception as e:
                        logging.error(f"Failed to place order(s) for {ticker}: {e}")
                        print(f"Failed to place order(s) for {ticker}: {e}")
