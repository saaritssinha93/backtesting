...
Live parquet variant aligned with `eqidv3_live_combined_analyser_csv.py`:
...
def main() -> None:
    ...
    while True:
        ...
        if live.UPDATE_15M_BEFORE_CHECK:
            try:
                print(f"[UPD ] Running eqidv3 core 15m update at {live.now_ist().strftime('%Y-%m-%d %H:%M:%S%z')}")
