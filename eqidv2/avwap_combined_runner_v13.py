# -*- coding: utf-8 -*-
"""
avwap_combined_runner_v13.py - AVWAP v13 COMBINED runner: V11 SHORT + V9 LONG (hybrid)
==================================================================================
...
# NIFTY50 directional guard (V13)
NIFTY_GUARD_ENABLED = True
NIFTY_GUARD_THRESHOLD_PCT = 0.5
NIFTY_GUARD_TICKERS: Tuple[str, ...] = (
    "NIFTY50",
    "NIFTY_50",
    "NIFTY",
)
...
TEST_SHORT_TARGET_PCT  = 0.0090
TEST_LONG_TARGET_PCT   = 0.0100
...
def _detect_nifty_guard_by_day(cfg: StrategyConfig) -> Tuple[Dict[str, str], str]:
    """
    Build per-day guard map from a NIFTY index 15m parquet.

    Rules:
    - close <= -0.5% vs yesterday close and 15-min trend reducing -> SHORT_ONLY
    - close >= +0.5% vs yesterday close and 15-min trend increasing -> LONG_ONLY
    - else -> BOTH
    """
    ticker_found = ""
    for t in NIFTY_GUARD_TICKERS:
    ...
    try:
        ...
        idx = read_15m_parquet(str(p), cfg.parquet_engine)
        if idx.empty:
            return {}, ""
        ...
        idx["day"] = idx["date"].dt.tz_convert(IST).dt.strftime("%Y-%m-%d")

        daily_close = idx.groupby("day", as_index=True)["close"].last()
        prev_close = daily_close.shift(1)
        gap_pct = ((daily_close - prev_close) / prev_close.replace(0, np.nan)) * 100.0

        trend = idx.groupby("day", as_index=True).apply(
            lambda g: float(g["close"].iloc[-1]) - float(g["close"].iloc[0])
        )

        out: Dict[str, str] = {}
        for day in daily_close.index:
            g = float(gap_pct.get(day, np.nan))
            tr = float(trend.get(day, np.nan))
            mode = "BOTH"
            if np.isfinite(g) and np.isfinite(tr):
                if g <= -float(NIFTY_GUARD_THRESHOLD_PCT) and tr < 0:
                    mode = "SHORT_ONLY"
                elif g >= float(NIFTY_GUARD_THRESHOLD_PCT) and tr > 0:
                    mode = "LONG_ONLY"
            out[str(day)] = mode
...
def _apply_nifty_guard(short_df: pd.DataFrame, long_df: pd.DataFrame, guard_map: Dict[str, str]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Apply per-day NIFTY guard map to short/long trade dataframes."""
    if not guard_map:
        return short_df, long_df

    def _norm_day_col(df: pd.DataFrame) -> pd.Series:
        if df.empty or "trade_date" not in df.columns:
            return pd.Series(dtype=str)
        return pd.to_datetime(df["trade_date"], errors="coerce").dt.strftime("%Y-%m-%d")
    ...
    if not s.empty:
        s_days = _norm_day_col(s)
        s_modes = s_days.map(guard_map).fillna("BOTH")
        s = s[s_modes.ne("LONG_ONLY")].copy()

    if not l.empty:
        l_days = _norm_day_col(l)
        l_modes = l_days.map(guard_map).fillna("BOTH")
        l = l[l_modes.ne("SHORT_ONLY")].copy()

    print(
        "[NIFTY_GUARD] Applied day-side filter: "
        f"SHORT {s_before}->{len(s)} | LONG {l_before}->{len(l)}"
    )
...
def main() -> None:
    ...
    _outputs_dir = _project_root / "outputs_v13"
    ...
    with open(log_path, "w", encoding="utf-8") as _log_fh:
        ...
        try:
            print("=" * 70)
            print("AVWAP v13 COMBINED runner — V11 SHORT + V9 LONG (hybrid)")
            ...
            if NIFTY_GUARD_ENABLED:
                guard_map, guard_src = _detect_nifty_guard_by_day(short_cfg)
                if guard_map:
                    counts = pd.Series(list(guard_map.values())).value_counts()
                    print(
                        "[NIFTY_GUARD] "
                        f"Source={guard_src} | threshold={NIFTY_GUARD_THRESHOLD_PCT:.2f}% | "
                        f"LONG_ONLY={int(counts.get('LONG_ONLY', 0))}, "
                        f"SHORT_ONLY={int(counts.get('SHORT_ONLY', 0))}, "
                        f"BOTH={int(counts.get('BOTH', 0))}"
                    )
                    short_df, long_df = _apply_nifty_guard(short_df, long_df, guard_map)
                else:
                    print("[NIFTY_GUARD] Enabled but no valid NIFTY index parquet found; skipped.")
            ...
            # --- Save CSV ---
            out_csv = _outputs_dir / f"avwap_longshort_trades_v13_ALL_DAYS_{ts}.csv"
