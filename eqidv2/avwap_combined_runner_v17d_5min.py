# -*- coding: utf-8 -*-
"""
avwap_combined_runner_v17d_5min.py - AVWAP v17d COMBINED runner on 5-minute signals
====================================================================================
...
1. All outputs saved to outputs_v17d_5min/
...
# V17D intrabar entry resolution:
# - setup detection stays on completed 5-minute bars
# - actual fills are resolved on 1-minute bars after the signal bar completes
V17D_INTRABAR_ENTRY_ENABLED = _env_flag("EQIDV17D_INTRABAR_ENTRY_ENABLED", True)
V17D_INTRABAR_ENTRY_MAX_ADVERSE_GAP_BPS = _env_float(
    "EQIDV17D_INTRABAR_ENTRY_MAX_ADVERSE_GAP_BPS", 50.0
)
V17D_INTRABAR_ENTRY_WINDOW_A_MIN = max(
    1, int(os.getenv("EQIDV17D_INTRABAR_ENTRY_WINDOW_A_MIN", "5"))
)
V17D_INTRABAR_ENTRY_WINDOW_CONTINUATION_MIN = max(
    1, int(os.getenv("EQIDV17D_INTRABAR_ENTRY_WINDOW_CONTINUATION_MIN", "3"))
)
V17D_INTRABAR_ENTRY_WINDOW_B_HUGE_MIN = max(
    1, int(os.getenv("EQIDV17D_INTRABAR_ENTRY_WINDOW_B_HUGE_MIN", "20"))
)
...
MAX_WORKERS = max(1, int(os.getenv("EQIDV17C_5MIN_MAX_WORKERS", str(DEFAULT_MAX_WORKERS))))
EXECUTOR_MODE = str(os.getenv("EQIDV17C_5MIN_EXECUTOR", "auto")).strip().lower()
...
V17C_SHORT_STRONG_CLOSE_MAX_FRACTION = _env_float(
    "EQIDV17C_SHORT_STRONG_CLOSE_MAX_FRACTION", 0.25
)
V17C_OPPOSING_WICK_MAX_FRACTION = _env_float(
    "EQIDV17C_OPPOSING_WICK_MAX_FRACTION", 0.20
)
V17C_RANGE_EXPANSION_MIN = _env_float(
    "EQIDV17C_RANGE_EXPANSION_MIN", 1.05
)
...
V17C_SHORT_CONTINUATION_ENABLED = _env_flag(
    "EQIDV17C_SHORT_CONTINUATION_ENABLED", False
)
V17C_SHORT_CONTINUATION_ADX_MIN = _env_float(
    "EQIDV17C_SHORT_CONTINUATION_ADX_MIN", 30.0
)
V17C_SHORT_CONTINUATION_ADX_SLOPE_MIN = _env_float(
    "EQIDV17C_SHORT_CONTINUATION_ADX_SLOPE_MIN", 0.50
)
V17C_SHORT_CONTINUATION_VOLUME_MIN_RATIO = _env_float(
    "EQIDV17C_SHORT_CONTINUATION_VOLUME_MIN_RATIO", 1.05
)
V17C_SHORT_CONTINUATION_AVWAP_DIST_ATR_MAX = _env_float(
    "EQIDV17C_SHORT_CONTINUATION_AVWAP_DIST_ATR_MAX", 2.00
)
V17C_SHORT_CONTINUATION_ATR_PCT_MAX = _env_float(
    "EQIDV17C_SHORT_CONTINUATION_ATR_PCT_MAX", 0.0045
)
V17C_SHORT_CONTINUATION_ENTRY_CUTOFF = dtime(13, 30, 0)
...
V17C_SHORT_TARGET_PCT = _env_float("EQIDV17C_SHORT_TARGET_PCT", 0.0080)
...
def _apply_v17c_setup_quality_filters(
    ...
    def _row_ok_short(row) -> bool:
        ...
        if feats is None:
            return False
        return (
            np.isfinite(feats["close_bottom_frac"])
            and feats["close_bottom_frac"] <= V17C_SHORT_STRONG_CLOSE_MAX_FRACTION
            and np.isfinite(feats["lower_wick_frac"])
            and feats["lower_wick_frac"] <= V17C_OPPOSING_WICK_MAX_FRACTION
            and np.isfinite(feats["range_vs_median5"])
            and feats["range_vs_median5"] >= V17C_RANGE_EXPANSION_MIN
            and np.isfinite(feats["volume_ratio20"])
            and feats["volume_ratio20"] >= V17C_SHORT_CONTINUATION_VOLUME_MIN_RATIO
            and np.isfinite(feats["adx_slope2"])
            and feats["adx_slope2"] >= V17C_SHORT_CONTINUATION_ADX_SLOPE_MIN
        )
...
def apply_live_parity_profile(
    ...
    if ENABLE_PLAYBOOK_V11_PROFILE:
        ...
        short_cfg.adx_min = V17C_SHORT_ADX_MIN
        short_cfg.adx_slope_min = 0.40
        short_cfg.volume_min_ratio = V17C_SHORT_VOLUME_MIN_RATIO
        ...
        short_cfg.rsi_max_short = V17C_SHORT_RSI_MAX
        ...
        short_cfg.signal_avwap_dist_atr_max = V17C_SHORT_SIGNAL_AVWAP_DIST_ATR_MAX
...
def _v17d_intrabar_entry_window_minutes(side: str, setup: str) -> int:
    ...
    if setup_u in {
        ...
        return int(V17D_INTRABAR_ENTRY_WINDOW_A_MIN)
    if setup_u in {
        "A_MOD_CLOSE_CONTINUATION_BREAK",
        "C_SHORT_CONTINUATION_BREAK",
    }:
        return int(V17D_INTRABAR_ENTRY_WINDOW_CONTINUATION_MIN)
    if setup_u in {
        "B_HUGE_RED_FAILED_BOUNCE",
        "B_HUGE_GREEN_PULLBACK_HOLD_THEN_BREAK",
        "B_HUGE_C1_CLOSE_RECLAIM_BREAK",
    }:
        return int(V17D_INTRABAR_ENTRY_WINDOW_B_HUGE_MIN)
    return int(V17D_INTRABAR_ENTRY_WINDOW_A_MIN)


def _v17d_adjust_signal_time_for_intrabar_entry(row: pd.Series) -> pd.Timestamp:
    sig_ts = _coerce_ist_timestamp(row.get("signal_time_ist"))
    entry_ts = _coerce_ist_timestamp(row.get("entry_time_ist"))
    setup_u = str(row.get("setup", "") or "").strip().upper()

    if pd.isna(sig_ts):
        if pd.notna(entry_ts):
            sig_ts = entry_ts - pd.Timedelta(minutes=5)
        else:
            return pd.NaT

    adjusted = sig_ts
    if setup_u in {"A_PULLBACK_C2_THEN_BREAK_C2_LOW", "A_PULLBACK_C2_THEN_BREAK_C2_HIGH"}:
        adjusted = sig_ts + pd.Timedelta(minutes=5)
    elif setup_u in {"B_HUGE_RED_FAILED_BOUNCE", "B_HUGE_GREEN_PULLBACK_HOLD_THEN_BREAK"}:
        adjusted = sig_ts + pd.Timedelta(minutes=15)
        if pd.notna(entry_ts):
            adjusted = min(adjusted, entry_ts - pd.Timedelta(minutes=5))
    elif setup_u == "B_HUGE_C1_CLOSE_RECLAIM_BREAK":
        if pd.notna(entry_ts):
            adjusted = entry_ts - pd.Timedelta(minutes=5)
        else:
            adjusted = sig_ts + pd.Timedelta(minutes=5)

    if pd.notna(entry_ts) and adjusted >= entry_ts:
        adjusted = entry_ts - pd.Timedelta(minutes=1)
    return adjusted


def _v17d_find_intrabar_entry_fill(
    ...
    if (
        bars_1m is None
        or bars_1m.empty
        or not np.isfinite(trigger)
        or trigger <= 0
        or pd.isna(signal_time)
        or pd.isna(valid_until)
        or valid_until <= signal_time
    ):
        return {"status": "no_fill"}

    dt = pd.to_datetime(bars_1m.get("datetime"), errors="coerce")
    mask = (dt > signal_time) & (dt <= valid_until)
    window = bars_1m.loc[mask].copy()
    if window.empty:
        return {"status": "no_fill"}

    open_col = "open" if "open" in window.columns else "Open"
    high_col = "high" if "high" in window.columns else "High"
    low_col = "low" if "low" in window.columns else "Low"

    side_u = str(side or "").upper()
    for _, bar in window.sort_values("datetime").iterrows():
        bar_time = _coerce_ist_timestamp(bar.get("datetime"))
        bar_open = float(bar.get(open_col, np.nan))
        ...
        if side_u == "SHORT":
            if bar_open <= trigger:
                adverse_gap_bps = ((trigger - bar_open) / trigger) * 10000.0
                if adverse_gap_bps <= float(V17D_INTRABAR_ENTRY_MAX_ADVERSE_GAP_BPS):
                    return {
                        "status": "fill",
                        "fill_time": bar_time,
                        "fill_price": float(bar_open),
                        "case": "1MIN_GAP_THROUGH",
                    }
                return {
                    "status": "reject_gap",
                    "fill_time": bar_time,
                    "fill_price": float(bar_open),
                    "case": "1MIN_GAP_REJECT",
                    "gap_bps": adverse_gap_bps,
                }
            if bar_low < trigger:
                return {
                    "status": "fill",
                    "fill_time": bar_time,
                    "fill_price": float(trigger),
                    "case": "1MIN_TRIGGER_TOUCH",
                }
        else:
            if bar_open >= trigger:
                adverse_gap_bps = ((bar_open - trigger) / trigger) * 10000.0
                if adverse_gap_bps <= float(V17D_INTRABAR_ENTRY_MAX_ADVERSE_GAP_BPS):
                    return {
                        "status": "fill",
                        "fill_time": bar_time,
...
def _resolve_entries_1min(
    trades_df: pd.DataFrame,
    dir_1m: Path,
    cfg: StrategyConfig,
) -> pd.DataFrame:
    if trades_df.empty or not V17D_INTRABAR_ENTRY_ENABLED:
        return trades_df
    ...
    for _, row in d.iterrows():
        row_out = row.to_dict()
        ticker = str(row_out.get("ticker", "") or "").upper()
        side = str(row_out.get("side", "") or "").upper()
        setup = str(row_out.get("setup", "") or "")
        trigger = float(pd.to_numeric(row_out.get("entry_price", np.nan), errors="coerce"))
        signal_time = _v17d_adjust_signal_time_for_intrabar_entry(row)
        valid_minutes = _v17d_intrabar_entry_window_minutes(side, setup)
        entry_original_time = _coerce_ist_timestamp(row_out.get("entry_time_ist"))
        entry_original_price = float(pd.to_numeric(row_out.get("entry_price", np.nan), errors="coerce"))

        if pd.isna(signal_time) or not np.isfinite(trigger) or trigger <= 0 or valid_minutes <= 0:
            row_out["entry_resolution_case"] = "5M_ORIGINAL_INVALID_SIGNAL"
            kept_rows.append(row_out)
            continue

        valid_until = signal_time + pd.Timedelta(minutes=int(valid_minutes))
        ...
        if df_1m is None or df_1m.empty:
            row_out["signal_time_ist"] = signal_time
            row_out["entry_valid_until_ist"] = valid_until
            row_out["entry_original_time_ist"] = entry_original_time
            row_out["entry_original_price"] = entry_original_price
            row_out["entry_trigger_price"] = trigger
            row_out["entry_resolution_case"] = "5M_FALLBACK_NO_1M"
            kept_rows.append(row_out)
            fallback_no_1m += 1
            continue

        fill = _v17d_find_intrabar_entry_fill(df_1m, side, trigger, signal_time, valid_until)
        status = str(fill.get("status", "") or "").lower()
        if status == "no_fill":
            dropped_no_fill += 1
            continue
        if status == "reject_gap":
            dropped_gap += 1
            continue

        fill_time = _coerce_ist_timestamp(fill.get("fill_time"))
        fill_price = float(fill.get("fill_price", np.nan))
        if pd.isna(fill_time) or not np.isfinite(fill_price) or fill_price <= 0:
            dropped_no_fill += 1
            continue
        ...
        if side == "SHORT":
            sl_price = fill_price * (1.0 + stop_pct)
        ...
        try:
            ...
            position_size_rs = row_out.get("position_size_rs", 0.0)
            risk_rs = row_out.get("risk_per_trade_rs", 0.0)

        row_out["trade_date"] = fill_time.strftime("%Y-%m-%d")
        row_out["signal_time_ist"] = signal_time
        row_out["entry_time_ist"] = fill_time
        row_out["entry_price"] = fill_price
        row_out["sl_price"] = sl_price
        row_out["stop_price"] = sl_price
        row_out["target_price"] = target_price
        row_out["position_size_rs"] = position_size_rs
        row_out["risk_per_trade_rs"] = risk_rs
        row_out["entry_trigger_price"] = trigger
        row_out["entry_valid_until_ist"] = valid_until
        row_out["entry_original_time_ist"] = entry_original_time
        row_out["entry_original_price"] = entry_original_price
        row_out["entry_resolution_case"] = str(fill.get("case", "1MIN_TRIGGER_TOUCH"))
        resolved_rows += 1
        kept_rows.append(row_out)

    out = pd.DataFrame(kept_rows) if kept_rows else d.iloc[0:0].copy()
    print(
        "[ENTRY_1MIN] "
        f"resolved={resolved_rows}/{len(d)} | "
        f"dropped_no_fill={dropped_no_fill} | "
        f"dropped_gap={dropped_gap} | "
        f"fallback_no_1m={fallback_no_1m}"
    )
    return out
...
def main() -> None:
    ...
    _outputs_dir = runtime_dir("outputs_v17d_5min")
    ...
    with open(log_path, "w", encoding="utf-8") as _log_fh:
        ...
        try:
            ...
            print("=" * 70)
            print("AVWAP v17d_5min COMBINED runner — same logic as v17c")
            ...
            # ---- PHASE 1.5: Re-resolve entries using 1-min bars ----
            if V17D_INTRABAR_ENTRY_ENABLED:
                print("\n[PHASE 1.5] Re-resolving entries using 1-min intrabar data...")
                phase15_started = time.perf_counter()
                if not short_df.empty:
                    print(f"  [SHORT] {len(short_df)} trades to re-resolve...")
                    short_df = _resolve_entries_1min(short_df, dir_1m, short_cfg)
                if not long_df.empty:
                    print(f"  [LONG] {len(long_df)} trades to re-resolve...")
                    long_df = _resolve_entries_1min(long_df, dir_1m, long_cfg)
                print(f"[TIMING] Phase 1.5 completed in {time.perf_counter() - phase15_started:.1f}s")
            ...
            # --- Save CSV ---
            out_csv = _outputs_dir / f"avwap_longshort_trades_v17d_5min_ALL_DAYS_{ts}.csv"
            combined.to_csv(out_csv, index=False)
            out_daywise_csv = _outputs_dir / f"avwap_daywise_breakdown_v17d_5min_ALL_DAYS_{ts}.csv"
