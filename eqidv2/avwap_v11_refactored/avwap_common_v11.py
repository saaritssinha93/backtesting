...
class StrategyConfig:
    ...
    # Position sizing by risk (playbook style)
    enable_risk_based_position_sizing: bool = True
    capital_base_rs: float = 1_000_000.0
    risk_per_trade_pct_of_capital: float = 0.0035  # 0.35%
    min_position_size_rs: float = 25_000.0
    max_position_size_rs: float = 250_000.0
    # Partial exits (T1 + runner)
    ...
    # Day mode selector (trend vs reversal)
    enable_mode_selector: bool = True
    opening_range_bars: int = 2
    opening_range_tight_threshold_pct: float = 0.80
    ...
    require_vwap_side_persistence: bool = True
    vwap_side_lookback_bars: int = 5
    vwap_side_min_count: int = 3
    require_structure_filter: bool = True
    structure_lookback_bars: int = 30
    ...
    reversal_requires_sweep: bool = True
...
def default_short_config(**overrides) -> StrategyConfig:
    """Factory for the SHORT side (V11 playbook baseline)."""
    base = dict(
        side="SHORT",
        stop_pct=0.0075,
        target_pct=0.0110,
...
def default_long_config(**overrides) -> StrategyConfig:
    """Factory for the LONG side (V11 playbook baseline)."""
    base = dict(
        side="LONG",
        stop_pct=0.0075,
        target_pct=0.0110,
...
class Trade:
    ...
    day_mode: str = "trend"
    gap_pct_open: float = 0.0
    opening_range_width_pct: float = 0.0
...
def compute_risk_position_size_rs(
    entry_price: float,
    stop_price: float,
    cfg: StrategyConfig,
) -> Tuple[float, float]:
    """
    Compute position size in Rs based on risk-per-trade.
    Returns (position_size_rs, risk_per_trade_rs).
    """
    if not cfg.enable_risk_based_position_sizing:
        return 0.0, 0.0
    if not (np.isfinite(entry_price) and np.isfinite(stop_price) and entry_price > 0):
        return 0.0, 0.0

    per_trade_risk_rs = max(
        0.0, float(cfg.capital_base_rs) * float(cfg.risk_per_trade_pct_of_capital)
    )
    risk_per_share = abs(float(entry_price) - float(stop_price))
    if per_trade_risk_rs <= 0 or risk_per_share <= 0:
        return 0.0, 0.0

    qty = per_trade_risk_rs / risk_per_share
    position_size_rs = qty * float(entry_price)

    min_sz = max(0.0, float(cfg.min_position_size_rs))
    max_sz = max(0.0, float(cfg.max_position_size_rs))
    if min_sz > 0:
        position_size_rs = max(position_size_rs, min_sz)
    if max_sz > 0:
        position_size_rs = min(position_size_rs, max_sz)

    return float(position_size_rs), float(per_trade_risk_rs)


def _count_bool_flips(flags: np.ndarray) -> int:
    """Count side-flips in a boolean sequence."""
    if flags.size <= 1:
        return 0
    flips = 0
    prev = bool(flags[0])
...
def select_day_mode(
    ...
    """
    Select 'trend' or 'reversal' mode using opening-range + gap + VWAP flips.
    Returns (mode, gap_pct_open, opening_range_width_pct).
    """
    gap_pct = 0.0
    or_width_pct = 0.0
    if df_day.empty:
        return "trend", gap_pct, or_width_pct

    prev_close_val = pd.to_numeric(prev_close, errors="coerce")
    first_open = float(pd.to_numeric(df_day.iloc[0].get("open", np.nan), errors="coerce"))
    if np.isfinite(prev_close_val) and np.isfinite(first_open) and prev_close_val > 0:
        gap_pct = abs((first_open - float(prev_close_val)) / float(prev_close_val) * 100.0)

    n = max(1, int(cfg.opening_range_bars))
    seg = df_day.iloc[: min(n, len(df_day))]
    if not seg.empty:
        or_high = float(pd.to_numeric(seg["high"], errors="coerce").max())
        or_low = float(pd.to_numeric(seg["low"], errors="coerce").min())
        if np.isfinite(first_open) and first_open > 0 and np.isfinite(or_high) and np.isfinite(or_low):
            or_width_pct = ((or_high - or_low) / first_open) * 100.0

    if not cfg.enable_mode_selector:
        return "trend", float(gap_pct), float(or_width_pct)

    trend_votes = 0
    reversal_votes = 0

    if or_width_pct <= float(cfg.opening_range_tight_threshold_pct):
        trend_votes += 1
    if or_width_pct >= float(cfg.opening_range_wide_threshold_pct):
        reversal_votes += 1
    if gap_pct >= float(cfg.gap_reversal_threshold_pct):
        reversal_votes += 1

    if "AVWAP" in seg.columns and "close" in seg.columns and len(seg) >= 3:
        close_seg = pd.to_numeric(seg["close"], errors="coerce")
        av_seg = pd.to_numeric(seg["AVWAP"], errors="coerce")
        valid = np.isfinite(close_seg) & np.isfinite(av_seg)
        if bool(valid.any()):
            side_flags = (close_seg[valid] > av_seg[valid]).to_numpy(dtype=bool)
            flips = _count_bool_flips(side_flags)
            if flips >= int(cfg.opening_vwap_flip_threshold):
                reversal_votes += 1
            else:
                trend_votes += 1

    mode = "reversal" if reversal_votes > trend_votes else "trend"
    return mode, float(gap_pct), float(or_width_pct)
...
def prepare_indicators(df: pd.DataFrame, cfg: StrategyConfig) -> pd.DataFrame:
    ...
    if "EMA_200" in out.columns:
        out["EMA200"] = pd.to_numeric(out["EMA_200"], errors="coerce")
    else:
        out["EMA200"] = ensure_ema(out, 200)
...
def vwap_side_persistence_pass(
    df_day: pd.DataFrame, idx: int, side: str, cfg: StrategyConfig
) -> bool:
    """Require minimum number of closes on one side of session VWAP over recent bars."""
    if not cfg.require_vwap_side_persistence:
        return True

    lookback = max(1, int(cfg.vwap_side_lookback_bars))
    need = max(1, int(cfg.vwap_side_min_count))
    need = min(need, lookback)

    start = max(0, idx - lookback + 1)
    seg = df_day.iloc[start : idx + 1]
    if seg.empty or "AVWAP" not in seg.columns:
        return False

    close = pd.to_numeric(seg["close"], errors="coerce")
    avwap = pd.to_numeric(seg["AVWAP"], errors="coerce")
    valid = np.isfinite(close) & np.isfinite(avwap)
    if not bool(valid.any()):
        return False

    side_u = str(side).upper()
    if side_u == "LONG":
        cond = (close > avwap) & valid
    else:
        cond = (close < avwap) & valid
    return int(cond.sum()) >= need


def _recent_pivot_lows(df_day: pd.DataFrame, idx: int, lookback: int) -> List[float]:
    lows = pd.to_numeric(df_day["low"], errors="coerce").to_numpy()
    pivots: List[float] = []
    start = max(1, idx - lookback + 1)
    end = min(idx - 1, len(lows) - 2)
    if end < start:
        return pivots
    for j in range(start, end + 1):
        a, b, c = lows[j - 1], lows[j], lows[j + 1]
        if np.isfinite(a) and np.isfinite(b) and np.isfinite(c) and (b < a) and (b <= c):
            pivots.append(float(b))
    return pivots


def _recent_pivot_highs(df_day: pd.DataFrame, idx: int, lookback: int) -> List[float]:
    highs = pd.to_numeric(df_day["high"], errors="coerce").to_numpy()
    pivots: List[float] = []
    start = max(1, idx - lookback + 1)
    end = min(idx - 1, len(highs) - 2)
    if end < start:
        return pivots
    for j in range(start, end + 1):
        a, b, c = highs[j - 1], highs[j], highs[j + 1]
        if np.isfinite(a) and np.isfinite(b) and np.isfinite(c) and (b > a) and (b >= c):
            pivots.append(float(b))
    return pivots


def swing_structure_pass(
    df_day: pd.DataFrame, idx: int, side: str, cfg: StrategyConfig
) -> bool:
    """
    Lightweight structure filter:
    - LONG: latest pivot low > previous pivot low (higher-low structure)
    - SHORT: latest pivot high < previous pivot high (lower-high structure)
    """
    if not cfg.require_structure_filter:
        return True

    lookback = max(8, int(cfg.structure_lookback_bars))
    side_u = str(side).upper()
    if side_u == "LONG":
        piv = _recent_pivot_lows(df_day, idx, lookback)
        if len(piv) < 2:
            return False
        return piv[-1] > piv[-2]

    piv = _recent_pivot_highs(df_day, idx, lookback)
    if len(piv) < 2:
        return False
    return piv[-1] < piv[-2]
