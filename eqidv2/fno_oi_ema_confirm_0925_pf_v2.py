"""Optimise the V2 top-N 09:25 FNO EMA/OI confirmation setup.
...
* one ranking scan is performed when confirmation is known at 09:26; and
* between one and five contracts may be selected per side per session.
...
SESSION = "fno_oi_ema_confirm_0925_pf_v2"
SIGNAL_SLOT = 925
CONFIRMATION_END = 926
RESULT_DIR = common.FNO_ROOT / "strategy_research"
REPORT_PATH = common.LATEST_DIR / "latest_fno_oi_ema_confirm_0925_v2_pf.md"
FORCE_REPORT_PATH = common.LATEST_DIR / "latest_fno_oi_ema_confirm_0925_v2_force_daily_pf.md"
DAYWISE_REPORT_PATH = common.LATEST_DIR / "latest_fno_oi_ema_confirm_0925_v2_daywise_detailed.md"
FORCE_DAYWISE_REPORT_PATH = common.LATEST_DIR / "latest_fno_oi_ema_confirm_0925_v2_force_daily_daywise_detailed.md"
...
def build_force_daily_selections(
    ...
    for picker in PICKERS:
        ranked_counts = select_up_to_per_day(
            eligible, orders[picker], SLOT_DAY_IDX, max_per_side
        )
...
def daily_curve(
    ...
    for day in calendar_days:
        ...
        symbols = (
            signals.iloc[indices]["tradingsymbol"].astype(str).tolist()
            if selections else []
        )
        sids = signals.iloc[indices]["sid"].astype(int).astype(str).tolist() if selections else []
        details = []
        for symbol, result in zip(symbols, values):
            result_text = "NO_FILL" if not math.isfinite(float(result)) else f"{float(result):+.3f}%"
            details.append(f"{symbol}={result_text}")
...
def trade_audit(
    ...
    audit = signals.iloc[indices][
        [
            "day", "sid", "tradingsymbol", "side", "price_change_pct",
            "oi_change_pct", "volume_ratio", "body_ratio", "wick_ratio",
            "traded_value",
        ]
    ].copy()
    audit.insert(0, "model", model)
    audit.insert(2, "signal_end", "09:25")
    audit.insert(3, "confirmation_end", "09:26")
    audit.insert(4, "rank_within_side", audit.groupby("day", sort=False).cumcount() + 1)
...
def render_daywise_report(
    ...
    mode = "V2 Force-Daily" if meta["force_daily"] else "V2 Filtered"
    table_rule = (
        f"top 1 through top {meta['max_per_side']} confirmed contracts are tested for each side/session"
        if meta["force_daily"]
        else f"filters and ranking may select up to {meta['max_per_side']} LONG and {meta['max_per_side']} SHORT contracts per session"
    )
    lines = [
...
def render_report(
    ...
    frequency = (
        f"top 1 through top {meta['max_per_side']} confirmed contracts tested per side/session without threshold filters"
        if meta["force_daily"]
        else f"up to {meta['max_per_side']} filtered contracts selected per side/session"
    )
    lines = [
        "# FNO EMA/OI 09:25/09:26 V2 Top-5 PF Optimisation",
        "",
        f"- Generated: {common.now_ist().isoformat(timespec='seconds')}",
        ...
        "## Best configurations",
        "",
        "| Side | Selection | Max/side | Train robust PF | Train PF | Test PF | All trade PF | All day PF | All trades | All net % | "
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--force-daily",
        action="store_true",
        help="Rank confirmed candidates for each side/day without threshold filters.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    file_prefix = (
        "ema_confirm_0925_v2_force_daily"
        if args.force_daily
        else "ema_confirm_0925_v2_once"
    )
    ...
    try:
        ...
        portfolio_daily = pd.DataFrame()
        if not daily.empty:
            if (daily["selections"] > args.max_per_side).any():
                raise AssertionError("V2 selected more contracts than --max-per-side.")
