...
- Robust missing/freshness detection for weekly candles
...
Outputs (Parquet):
- stocks_indicators_weekly_eq / <TICKER>_stocks_indicators_weekly.parquet

Usage examples:
- Fetch weekly:
    python trading_data_continous_run_historical_alltf_v3_parquet_stocksonly_weekly.py weekly
- Same (all aliases to weekly):
    python trading_data_continous_run_historical_alltf_v3_parquet_stocksonly_weekly.py all

Notes:
- Fixed historical window start: 2025-08-25 (IST), until latest completed weekly candle.
...
MARKET_CLOSE_TIME_DAILY = time(15, 30)      # daily candle considered complete after market close

# Keep an internal timestamp mode key to minimize code changes.
DEFAULT_TIMESTAMP_MODE = "end"
...
def setup_logger() -> logging.Logger:
    ...
    fh = logging.FileHandler("stocks_fetcher_weekly_run.log", mode="w", encoding="utf-8")
    fh.setFormatter(fmt)
...
def setup_kite_session() -> KiteConnect:
    """
    Set up primary Kite session + optional app pool.
    Uses the same credential naming convention as live executors:
    ...
    for app_name, key_file, token_file in specs:
        if not (os.path.exists(key_file) and os.path.exists(token_file)):
            if app_name == "app1":
                logger.error("Primary credentials missing: %s and/or %s", key_file, token_file)
            else:
                logger.warning("[KITE] %s skipped (missing %s/%s).", app_name, key_file, token_file)
            continue
        ...
        try:
            ...
            logger.info("[KITE] Session ready: %s | user=%s", app_name, user_name)
            ...
            if app_name == "app1":
                logger.error("Primary Kite session setup failed (%s): %s", app_name, e)
            else:
                logger.warning("[KITE] %s disabled due to setup error: %s", app_name, e)

    if primary is None and sessions:
        primary = sessions[0]
    ...
    logger.info("[KITE] Session pool active apps: %d", len(kite_pool))
    return primary
...
def get_kite_client_rr() -> KiteConnect:
    ...
    raise RuntimeError("Kite session pool is empty; call setup_kite_session() first.")
...
def last_completed_daily_stamp(now_ist: datetime, holidays: set[date]) -> datetime:
    """
    Return the timestamp (IST midnight) for the latest completed daily candle.
    - Non-trading day => previous trading day.
    - Trading day before close => previous trading day.
    - Trading day after/equal close => today.
    """
    ...
    elif now_ist.time() < MARKET_CLOSE_TIME_DAILY:
        d = _prev_trading_day(d, holidays)

    return IST_TZ.localize(datetime(d.year, d.month, d.day, 0, 0, 0))

def last_completed_weekly_stamp(now_ist: datetime, holidays: set[date]) -> datetime:
    """
    Return the timestamp label (W-FRI) of the latest completed weekly candle.
    """
    d = last_completed_daily_stamp(now_ist, holidays).date()
    days_since_friday = (d.weekday() - 4) % 7
    week_end = d - timedelta(days=days_since_friday)
    return IST_TZ.localize(datetime(week_end.year, week_end.month, week_end.day, 0, 0, 0))
...
def expected_last_stamp(mode: str, now_ist: datetime, holidays: set[date], intraday_ts: str) -> dict:
    ...
    if mode != "weekly":
        raise ValueError(f"Unsupported mode for weekly script: {mode}")

    exp_ts = last_completed_weekly_stamp(now_ist, holidays)
    return {"kind": "ts", "value": exp_ts, "step_min": 0}
...
def _fetch_historical_daily_base_df(kite, token, start_dt_ist, end_dt_ist, logger, intraday_ts):
...
def _resample_weekly_from_daily(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame()

    req = ["date", "open", "high", "low", "close", "volume"]
    w = df.copy()
    w["date"] = _to_ist(w["date"])
    w = w.sort_values("date").reset_index(drop=True)
    w = w.set_index("date")
    agg = (
        w.resample("W-FRI", label="right", closed="right")
         .agg({
             "open": "first",
             "high": "max",
             "low": "min",
...
def fetch_historical_weekly_df(kite, token, start_dt_ist, end_dt_ist, logger, intraday_ts):
    base_daily = _fetch_historical_daily_base_df(kite, token, start_dt_ist, end_dt_ist, logger, intraday_ts)
    if base_daily.empty:
        return pd.DataFrame()
    out = _resample_weekly_from_daily(base_daily)
    out = out[out["date"] <= end_dt_ist].reset_index(drop=True)
    return out
...
# ========= CHANGE FEATURES (weekly) =========

def add_change_features_weekly(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Keep column parity with intraday outputs for downstream readers.
    df["Intra_Change"] = df["Daily_Change"]
    return df
...
def _compute_common_features(df: pd.DataFrame, mode: str) -> pd.DataFrame:
    ...
    # Keep a short recent window for compatibility with downstream strategy code.
    ...
    df = add_change_features_weekly(df)
    return df
...
def process_ticker(
    ...
    try:
        ...
        kite_client = kite
        try:
            kite_client = get_kite_client_rr()
        except Exception:
            # Fallback to passed client if pool is unavailable.
            pass

        if mode == "weekly":
            fetched = fetch_historical_weekly_df(kite_client, token, inc_start, end_dt_ist, logger, intraday_ts)
        else:
...
def run_mode(
    ...
    end_dt = last_completed_daily_stamp(now_ist, holidays)

    logger.info("=== MODE=%s | timestamp_mode=%s | Window: %s -> %s (IST) ===",
                mode, DEFAULT_TIMESTAMP_MODE, start_dt.strftime("%Y-%m-%d %H:%M"), end_dt.strftime("%Y-%m-%d %H:%M"))
    ...
    verify_expected_ts = last_completed_weekly_stamp(now_ist, holidays)
...
def parse_args():
    ...
    p.add_argument("mode", nargs="?", default="weekly", help="weekly|all")
    p.add_argument("--max-workers", type=int, default=DEFAULT_MAX_WORKERS)
    p.add_argument("--no-skip", action="store_true",
                   help="Disable freshness skip (will refetch/recompute even if fresh)")
    p.add_argument("--intraday-ts", default=DEFAULT_TIMESTAMP_MODE, choices=["end"],
                   help="Timestamp mode (weekly script uses candle end semantics only)")
    ...
    p.add_argument("--report-dir", default="reports/stocks_missing_reports_weekly",
