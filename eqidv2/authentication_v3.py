# -*- coding: utf-8 -*-
"""
authentication_v3.py

Faster version of authentication_v2.py. Logic and behaviour are identical;
the only changes are performance optimisations:

  1. Secondary apps (2-8) authenticated in PARALLEL via ThreadPoolExecutor
     instead of sequentially — browser logins and profile() checks overlap.
  2. Same-day state-file trust: if auth_v2_state.json already records
     session_date_ist_appN == today the profile() API call is skipped
     entirely (Kite access tokens don't expire intraday).
  3. ChromeDriver path cached at module level so ChromeDriverManager.install()
     is called only once per process regardless of how many browser sessions open.
  4. State-file writes protected by a threading.Lock to prevent concurrent
     read-modify-write races when apps authenticate in parallel.

Files used: same as v2 (api_key.txt … api_key8.txt, access_token*.txt, etc.)
"""
...
# ---------------------------------------------------------------------------
# OPT 3: cache ChromeDriver path — install() called once per process
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# OPT 4: state-file lock — prevents concurrent read-modify-write races
# ---------------------------------------------------------------------------
...
def _do_browser_login_for_request_token(kite: KiteConnect, user_id: str, password: str, totp_secret: str) -> str:
    # OPT 3: reuse cached ChromeDriver path — no repeat version-check network call
...
def _seed_additional_session_for_today(primary_parts: list[str], force_login: bool, app_idx: int) -> None:
    ...
    # OPT 2: trust same-day state — skip profile() API call if already confirmed today
    if not force_login:
        with _state_lock:
            st = _load_state()
        if st.get(f"session_date_ist_app{app_idx}") == today:
            existing_access = _read_first_line(acc_file)
            if existing_access:
                kite_n.set_access_token(existing_access)
                print(
                    f"[{tag}] Same-day state confirmed for app{app_idx} — skipping profile() check.",
                    flush=True,
                )
                return
...
def main() -> None:
    ...
    # OPT 1: pre-install ChromeDriver once before any parallel browser sessions open
    try:
        _get_chromedriver_path()
    except Exception as e:
        print(f"[WARN] ChromeDriver pre-install failed ({e}). Will retry per-session.", flush=True)

    # OPT 1: authenticate secondary apps (2-8) in parallel
