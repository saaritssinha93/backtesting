...
SIGNAL_CSV_COLUMNS = [
    ...
    "bar_closed_at_ist", "decision_ready_at_ist",
    "entry_engine_ready_at_ist", "signal_written_at_ist",
    "candidate_snapshot_sha256", "runtime_manifest_path",
...
def _save_state(state: Dict[str, Any]) -> None:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            print(
                f"[WARN] State save contention ({attempt}/{max_attempts}): {exc!r}. "
    ...
    print(
        f"[ERROR] Unable to persist state after {max_attempts} attempts: {last_exc!r}. "
        "Continuing without state update for this scan.",
        flush=True,
    )
...
def _all_day_runner_parity_signals_for_ticker(
    ...
    if _runner_cfg is not None:
        apply_profile = getattr(_runner_cfg, "apply_live_parity_profile", None)
        if callable(apply_profile):
            ...
            short_cfg.lag_bars_short_a_mod_break_c1_low = int(
                getattr(
                    ...
                    short_cfg.lag_bars_short_a_mod_break_c1_low,
                )
            )
            ...
            short_cfg.lag_bars_short_b_huge_failed_bounce = int(
                getattr(
                    ...
                    short_cfg.lag_bars_short_b_huge_failed_bounce,
                )
            )
            long_cfg.lag_bars_long_a_mod_break_c1_high = int(
                getattr(
                    ...
                    long_cfg.lag_bars_long_a_mod_break_c1_high,
                )
            )
