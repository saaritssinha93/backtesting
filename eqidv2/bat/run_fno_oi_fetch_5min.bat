...
set "SCRIPT_NAME=fno_oi_fetch_5min.py"
set "LOG_FILE=%LOG_DIR%\fno_oi_fetch_5min.log"
set "STATUS_FILE=%LOG_DIR%\fno_oi_fetch_5min.supervisor.status"
set "HEARTBEAT_FILE=%LOG_DIR%\fno_oi_fetch_5min.supervisor.heartbeat"
set "FRESHNESS_FILE=%RUNTIME_STATUS_DIR%\fno_oi_fetch_5min.heartbeat"
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%SUPERVISOR_PS1%" ^
  -Name "%SCRIPT_NAME%" ^
  -FilePath "%PYTHON_EXE%" ^
  -ArgumentList "-u","%BASE_DIR%\%SCRIPT_NAME%","--boundary-buffer-sec","3","--request-interval-sec","0.36","--min-coverage","0.99","--bootstrap-days","60","--slot-retry-attempts","2","--slot-retry-delay-sec","2" ^
  -WorkDir "%BASE_DIR%" ^
  -LogFile "%LOG_FILE%" ^
