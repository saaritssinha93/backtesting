...
set "SCRIPT_NAME=authentication.py"
set "LOG_FILE=%LOG_DIR%\authentication_runner.log"
set "ALERT_LOG=%ALERT_DIR%\CRITICAL_authentication_runner.log"
set "STATUS_FILE=%LOG_DIR%\authentication_runner.status"
set "RUN_OUT=%TEMP%\authentication_runner_%RANDOM%_%RANDOM%.log"
...
if /I "%FORCE_FAIL%"=="1" (
  >"%RUN_OUT%" echo FORCE_FAIL=1 -> Simulating authentication failure
  set "EXIT_CODE=911"
) else (
...
if not "%EXIT_CODE%"=="0" (
  ...
  eventcreate /L APPLICATION /T ERROR /ID 9002 /SO EQIDV2 /D "CRITICAL: authentication.py failed with exit code %EXIT_CODE%. See %LOG_FILE%" >nul 2>&1
  msg %USERNAME% /TIME:15 "CRITICAL: authentication.py failed (exit=%EXIT_CODE%) - check %LOG_FILE%" >nul 2>&1

  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "try { $ws = New-Object -ComObject WScript.Shell; [void]$ws.Popup('CRITICAL FAILURE in authentication.py`nExitCode: %EXIT_CODE%`nSee log: %LOG_FILE%', 10, 'EQIDV2 ALERT', 16) } catch { }"
