...
set "EQIDV7_SWEEP_LONG_PENDING_POLL_ENABLED=1"
set "EQIDV7_SWEEP_LONG_PENDING_POLL_INTERVAL_SEC=5"
...
set "SCRIPT_NAME=eqidv2_live_combined_analyser_csv_v7_sweep_long.py"
...
set "STATUS_FILE=%RUNTIME_STATUS_DIR%\eqidv2_live_combined_analyser_csv_v7_sweep_long.status"
set "HEARTBEAT_FILE=%RUNTIME_STATUS_DIR%\eqidv2_live_combined_analyser_csv_v7_sweep_long.heartbeat"
...
if not "%EXIT_CODE%"=="0" (
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "try { $ws = New-Object -ComObject WScript.Shell; [void]$ws.Popup('CRITICAL FAILURE in eqidv2_live_combined_analyser_csv_v7_sweep_long.py`nExitCode: %EXIT_CODE%`nSee log: %LOG_FILE%', 10, 'EQIDV2 ALERT', 16) } catch { }"
