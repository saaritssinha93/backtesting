...
set "SCRIPT_NAME=authentication_backup.py"
set "LOG_FILE=%LOG_DIR%\authentication_backup_runner.log"
set "ALERT_LOG=%ALERT_DIR%\CRITICAL_authentication_backup_runner.log"
set "STATUS_FILE=%LOG_DIR%\authentication_backup_runner.status"
...
if /I "%FORCE_FAIL%"=="1" (
  echo FORCE_FAIL=1 ^> Simulating authentication failure
  echo FORCE_FAIL=1 ^> Simulating authentication failure>>"%LOG_FILE%"
  set "EXIT_CODE=911"
) else (
...
if not "%EXIT_CODE%"=="0" (
  ...
  set "ALERT_FILE=%ALERT_DIR%\CRITICAL_authentication_FAILED_!TS!.txt"
  ...
  eventcreate /L APPLICATION /T ERROR /ID 9002 /SO EQIDV2 /D "CRITICAL: authentication_backup.py failed with exit code %EXIT_CODE%. See %LOG_FILE%" >nul 2>&1
  msg %USERNAME% /TIME:15 "CRITICAL: authentication_backup.py failed (exit=%EXIT_CODE%) - check %LOG_FILE%" >nul 2>&1

  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "try { $ws = New-Object -ComObject WScript.Shell; [void]$ws.Popup('CRITICAL FAILURE in authentication_backup.py`nExitCode: %EXIT_CODE%`nSee log: %LOG_FILE%', 10, 'EQIDV2 ALERT', 16) } catch { }"
