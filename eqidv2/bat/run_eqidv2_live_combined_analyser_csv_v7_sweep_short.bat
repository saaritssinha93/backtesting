...
set "EQIDV7_SWEEP_STALE_ONLY_RETRY=1"
...
set "SCRIPT_NAME=eqidv2_live_combined_analyser_csv_v7_sweep_short.py"
set "LOG_FILE=%LOG_DIR%\eqidv2_live_combined_analyser_csv_v7_sweep_short.log"
set "ALERT_LOG=%ALERT_DIR%\CRITICAL_eqidv2_live_combined_analyser_csv_v7_sweep_short.log"
set "STATUS_FILE=%RUNTIME_STATUS_DIR%\eqidv2_live_combined_analyser_csv_v7_sweep_short.status"
set "HEARTBEAT_FILE=%RUNTIME_STATUS_DIR%\eqidv2_live_combined_analyser_csv_v7_sweep_short.heartbeat"
...
echo [INFO] Scan tuning: initial_delay=%EQIDV2_INITIAL_DELAY_SECONDS%s, scans_per_slot=%EQIDV2_NUM_SCANS_PER_SLOT%, interval=%EQIDV2_SCAN_INTERVAL_SECONDS%s, stale_only_retry=%EQIDV7_SWEEP_STALE_ONLY_RETRY%, short_target_pct=%EQIDV7_SWEEP_SHORT_TARGET_PCT%>>"%LOG_FILE%"
for /f %%a in ('powershell -NoProfile -Command "Get-Date -Format yyyy-MM-dd_HH:mm:ss"') do set "RUN_TS=%%a"
...
if not "%EXIT_CODE%"=="0" (
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "try { $ws = New-Object -ComObject WScript.Shell; [void]$ws.Popup('CRITICAL FAILURE in eqidv2_live_combined_analyser_csv_v7_sweep_short.py`nExitCode: %EXIT_CODE%`nSee log: %LOG_FILE%', 10, 'EQIDV2 ALERT', 16) } catch { }"
