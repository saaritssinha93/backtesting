...
set "EQIDV15_LONG_PENDING_POLL_ENABLED=1"
set "EQIDV15_LONG_PENDING_POLL_INTERVAL_SEC=5"
set "EQIDV15_LONG_STOP_PCT=0.0075"
set "EQIDV15_LONG_TARGET_PCT=0.011"
set "LOG_DIR=%BASE_DIR%\logs"
set "ALERT_DIR=%LOG_DIR%\alerts"
set "SCRIPT_NAME=eqidv2_live_combined_analyser_csv_v15_long.py"
set "LOG_FILE=%LOG_DIR%\eqidv2_live_combined_analyser_csv_v15_long.log"
set "ALERT_LOG=%ALERT_DIR%\CRITICAL_eqidv2_live_combined_analyser_csv_v15_long.log"
set "STATUS_FILE=%LOG_DIR%\eqidv2_live_combined_analyser_csv_v15_long.status"
...
echo [INFO] Scan tuning: initial_delay=%EQIDV2_INITIAL_DELAY_SECONDS%s, scans_per_slot=%EQIDV2_NUM_SCANS_PER_SLOT%, interval=%EQIDV2_SCAN_INTERVAL_SECONDS%s, stale_only_retry=%EQIDV15_STALE_ONLY_RETRY%, pending_poll=%EQIDV15_LONG_PENDING_POLL_ENABLED%, pending_poll_interval=%EQIDV15_LONG_PENDING_POLL_INTERVAL_SEC%s, long_stop_pct=%EQIDV15_LONG_STOP_PCT%, long_target_pct=%EQIDV15_LONG_TARGET_PCT%>>"%LOG_FILE%"
...
if not "%EXIT_CODE%"=="0" (
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "try { $ws = New-Object -ComObject WScript.Shell; [void]$ws.Popup('CRITICAL FAILURE in eqidv2_live_combined_analyser_csv_v15_long.py`nExitCode: %EXIT_CODE%`nSee log: %LOG_FILE%', 10, 'EQIDV2 ALERT', 16) } catch { }"
