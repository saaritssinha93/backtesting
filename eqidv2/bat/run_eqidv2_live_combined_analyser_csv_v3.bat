...
set "SCRIPT_NAME=eqidv2_live_combined_analyser_csv_v3.py"
set "LOG_FILE=%LOG_DIR%\eqidv2_live_combined_analyser_csv_v3.log"
set "ALERT_LOG=%ALERT_DIR%\CRITICAL_eqidv2_live_combined_analyser_csv_v3.log"
set "STATUS_FILE=%LOG_DIR%\eqidv2_live_combined_analyser_csv_v3.status"
...
if not "%EXIT_CODE%"=="0" (
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "try { $ws = New-Object -ComObject WScript.Shell; [void]$ws.Popup('CRITICAL FAILURE in eqidv2_live_combined_analyser_csv_v3.py`nExitCode: %EXIT_CODE%`nSee log: %LOG_FILE%', 10, 'EQIDV2 ALERT', 16) } catch { }"
