...
set "SCRIPT_NAME=fundamental_price_action_v1_session.py"
set "SCRIPT_PATH=%BASE_DIR%\%SCRIPT_NAME%"
set "STATUS_FILE=%RUNTIME_STATUS_DIR%\fundamental_price_action_v1.status"
set "HEARTBEAT_FILE=%RUNTIME_STATUS_DIR%\fundamental_price_action_v1.heartbeat"
...
set "LOG_FILE=%LOG_DIR%\fundamental_price_action_v1_%TODAY_IST%.log"
...
if !NOW_HHMM! GEQ %END_CUTOFF_HHMM% (
  echo [%DATE% %TIME%] SKIP fundamental_price_action_v1 ^(current HHmm=!NOW_HHMM!, cutoff=%END_CUTOFF_HHMM%^)>>"%LOG_FILE%"
  call :WRITE_STATUS SKIPPED_CUTOFF 0
  endlocal & exit /b 0
)

echo [%DATE% %TIME%] START fundamental_price_action_v1>>"%LOG_FILE%"
echo [INFO] Auto-restart enabled: max_restarts=%MAX_RESTARTS%, retry_delay=%RESTART_DELAY_SEC%s, cutoff=%END_CUTOFF_HHMM%>>"%LOG_FILE%"

:RUN_LOOP
"%PYTHON_EXE%" -u "%SCRIPT_PATH%" --start-time 09:15 --first-entry-slot 09:55 --entry-cutoff 14:30 --end-time 15:30 %* >>"%LOG_FILE%" 2>&1
set "EXIT_CODE=%ERRORLEVEL%"

echo [%DATE% %TIME%] END fundamental_price_action_v1 ^(exit=!EXIT_CODE!^)>>"%LOG_FILE%"

if "!EXIT_CODE!"=="0" goto DONE

REM A crashed worker leaves the strategy stopped for the rest of the session,
REM so restart it in-place. The Python session resumes from the latest
REM completed slot; it never replays the morning backlog.
for /f %%a in ('powershell -NoProfile -Command "(Get-Date).ToString('HHmm')"') do set "NOW_HHMM=%%a"
if !NOW_HHMM! GEQ %END_CUTOFF_HHMM% (
...
>>"%STATUS_FILE%" echo session=fundamental_price_action_v1
...
>>"%HEARTBEAT_FILE%" echo session=fundamental_price_action_v1
