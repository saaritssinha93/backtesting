...
if "%EQIDV15_SHORT_SHARD_ID%"=="" (
  echo [ERROR] EQIDV15_SHORT_SHARD_ID is required.
  endlocal & exit /b 1
)

set "SHARD_ID=%EQIDV15_SHORT_SHARD_ID%"
if "%EQIDV15_SHORT_SHARD_COUNT%"=="" (
  set "EQIDV15_SHORT_SHARD_COUNT=10"
)
set /a SHARD_NUM=1%SHARD_ID%-100
set /a POST_READY_STAGGER_SEC=!SHARD_NUM!-1
...
set "EQIDV15_SHARED_SCAN_WAIT_SECONDS=90"
set "EQIDV15_SHARED_SCAN_WAIT_POLL_SECONDS=0.5"
set "EQIDV15_FAST_REUSE_PREVIOUS_SCAN=1"
set "EQIDV15_SHORT_STOP_PCT=0.0066"
set "EQIDV15_SHORT_TARGET_PCT=0.011"
set "EQIDV15_SHARD_SCAN_MAX_WORKERS=4"
set "EQIDV15_SHARD_SCAN_BLOCK_SIZE=27"
set "LOG_DIR=%BASE_DIR%\logs"
set "ALERT_DIR=%LOG_DIR%\alerts"
set "RUNTIME_STATUS_DIR=%EQIDV2_RUNTIME_ROOT%\runtime_status"
set "SCRIPT_NAME=eqidv2_live_combined_analyser_csv_v15_short_shard_%SHARD_ID%.py"
set "LOG_FILE=%LOG_DIR%\eqidv2_live_combined_analyser_csv_v15_short_shard_%SHARD_ID%.log"
set "ALERT_LOG=%ALERT_DIR%\CRITICAL_eqidv2_live_combined_analyser_csv_v15_short_shard_%SHARD_ID%.log"
set "STATUS_FILE=%RUNTIME_STATUS_DIR%\eqidv2_live_combined_analyser_csv_v15_short_shard_%SHARD_ID%.status"
set "HEARTBEAT_FILE=%RUNTIME_STATUS_DIR%\eqidv2_live_combined_analyser_csv_v15_short_shard_%SHARD_ID%.heartbeat"
...
echo [INFO] Shard: id=!SHARD_ID!/!EQIDV15_SHORT_SHARD_COUNT! ^| signal_csv=signals_YYYY-MM-DD_v15_short.csv>>"%LOG_FILE%"
echo [INFO] Scan tuning: initial_delay=!EQIDV2_INITIAL_DELAY_SECONDS!s, slot_offset=!EQIDV2_SLOT_START_OFFSET_SECONDS!s, post_ready_stagger=!EQIDV2_POST_READY_STAGGER_SECONDS!s, scans_per_slot=!EQIDV2_NUM_SCANS_PER_SLOT!, interval=!EQIDV2_SCAN_INTERVAL_SECONDS!s, block_parallel=!EQIDV2_BLOCK_PARALLEL_SCAN_ENABLED!, block_size=!EQIDV15_SHARD_SCAN_BLOCK_SIZE!, max_workers=!EQIDV15_SHARD_SCAN_MAX_WORKERS!, ready_marker=!EQIDV2_SLOT_READY_MARKER_ENABLED!, ready_poll=!EQIDV2_SLOT_READY_POLL_ENABLED!, ready_max_wait=!EQIDV2_SLOT_READY_MAX_WAIT_SECONDS!s, ready_poll_s=!EQIDV2_SLOT_READY_POLL_SECONDS!s, ready_sample=!EQIDV2_SLOT_READY_SAMPLE_SIZE!, ready_ratio=!EQIDV2_SLOT_READY_MIN_FRESH_RATIO!, stale_only_retry=!EQIDV15_STALE_ONLY_RETRY!, stale_retry_max_tickers=!EQIDV15_STALE_RETRY_MAX_TICKERS!, stale_retry_max_ratio=!EQIDV15_STALE_RETRY_MAX_RATIO!, short_stop_pct=!EQIDV15_SHORT_STOP_PCT!, short_target_pct=!EQIDV15_SHORT_TARGET_PCT!>>"%LOG_FILE%"
for /f %%a in ('powershell -NoProfile -Command "Get-Date -Format yyyy-MM-dd_HH:mm:ss"') do set "RUN_TS=%%a"
