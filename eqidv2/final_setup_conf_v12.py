"""
V12 EXPERIMENTAL SETUP BOOK.

Copied from the latest V11 working setup book. V12 research owns this module;
V7 live and V11 must never load it.
...
# The late BB10 setup is defined in the primary configuration record and copied
# into this launcher-selected V7/V11 parity book. deepcopy prevents either book
# from mutating the other's nested policy dictionaries at runtime.
from copy import deepcopy as _deepcopy
from final_setup_conf_v12_base import FINAL_SETUP_CONF as _PRIMARY_FINAL_SETUP_CONF
...
# ===========================================================================
# V12 RELAXED-GATE EXPERIMENT
#
# Every original condition is retained. Numeric thresholds are widened in the
# permissive direction, time windows are expanded, and Top-N caps are raised.
# Detector-only setups remain detector-only. Binary regime terms cannot be
# partially relaxed and therefore remain unchanged.
# ===========================================================================
V12_RELAXED_CONSTRAINTS = {
    "A_PULLBACK_C2_THEN_BREAK_C2_LOW": {
        "mask_terms": [["quality_score", ">=", 100.0]],
        "pre_momentum_terms": [["sig5_adx_calc", ">=", 18.0]],
        "entry_guards": {},
    },
    "D_EMA20_REJECTION": {
        "mask_terms": [["ema20_dist_atr", "<=", -0.20]],
        "pre_momentum_terms": [],
        "entry_guards": {"max_slot": "13:00"},
    },
    "E_ORB_BREAKOUT_LONG": {
        "mask_terms": [["rs_pct", ">=", 4.0], ["vwap_dist_atr", "<=", 1.25]],
        "pre_momentum_terms": [],
        "entry_guards": {},
    },
    "G_HIGHER_HIGH_BREAK": {
        "mask_terms": [["atr_pct", ">=", 0.00170], ["upper_wick_pct", ">=", 0.015]],
        "pre_momentum_terms": [["sig5_vol_ratio20", "<=", 4.50]],
        "entry_guards": {"top_n": 5},
    },
    "L_DOUBLE_BOTTOM_VWAP": {
        "mask_terms": [],
        "pre_momentum_terms": [["sig5_rsi_dir", ">=", 55.0]],
        "entry_guards": {"min_slot": "09:45", "max_slot": "12:30", "top_n": 2},
    },
    "B_HUGE_RED_FAILED_BOUNCE": {
        "mask_terms": [],
        "pre_momentum_terms": [
    ...
    "C_OR_BREAKDOWN": {
        "mask_terms": [],
        "pre_momentum_terms": [["sig5_adx_calc", ">=", 32.0], ["pre1_adx", "<=", 26.0]],
        "entry_guards": {},
    },
    "A_MOD_BREAK_C1_LOW": {
        "mask_terms": [["vol_ratio", ">=", 1.60]],
        "pre_momentum_terms": [["pre5_mom_r", ">=", 0.35], ["pre3_range_r", "<=", 0.28]],
        "entry_guards": {},
    },
    "G_LOWER_LOW_BREAK": {
        "mask_terms": [["vol_ratio", ">=", 3.20], ["quality_score", ">=", 60.0]],
        "pre_momentum_terms": [["sig5_rsi_dir", ">=", 62.0]],
        "entry_guards": {},
    },
    "S9_MIDDAY_LOSE": {
        "mask_terms": [],
    ...
    "DOC5D_AVWAP_RECLAIM_LONG": {
        "mask_terms": [["avwap_dist_atr", ">=", 0.80]],
        "pre_momentum_terms": [],
        "entry_guards": {"min_slot": "10:30", "top_n": 4},
    },
    "L_LATE_BB10_COMPRESSION_BREAKOUT": {
        "mask_terms": [["market_breadth", ">=", 0.40], ["nifty_ema_up", ">=", 1.0]],
        "pre_momentum_terms": [],
        "entry_guards": {"min_slot": "13:45", "max_slot": "14:45"},
    },
    # Preserve this setup exactly. Its multi-state hourly-prefilter contract is
    # detector-owned and must never be approximated by V12's generic relaxer.
...
if set(V12_RELAXED_CONSTRAINTS) != set(FINAL_SETUP_CONF):
    raise RuntimeError("V12 relaxed constraint map must cover every active setup exactly")

for _name, _cfg in FINAL_SETUP_CONF.items():
    _cfg["v12_original_constraints"] = {
        "mask_terms": _deepcopy(_cfg.get("mask_terms", [])),
        "pre_momentum_terms": _deepcopy(_cfg.get("pre_momentum_terms", [])),
        "entry_guards": _deepcopy(_cfg.get("entry_guards", {})),
    }
    _relaxed = V12_RELAXED_CONSTRAINTS[_name]
    _cfg["mask_terms"] = _deepcopy(_relaxed["mask_terms"])
    _cfg["pre_momentum_terms"] = _deepcopy(_relaxed["pre_momentum_terms"])
    _cfg["entry_guards"] = _deepcopy(_relaxed["entry_guards"])
    _cfg["v12_experiment"] = {
        "name": "relaxed_all_active_setups_v3_frequency",
        "scope": "multi_day_research_required",
