...
import avwap_combined_runner_v15 as base


ROOT = Path(__file__).resolve().parent
OUT_DIR = ROOT / "outputs_v15_sltarget_search"
ROUNDTRIP_COST_PCT_FALLBACK = 0.16


def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Search v15 short/long SL+target settings")
...
def _latest_v15_csv() -> Path:
    files = sorted(
        (ROOT / "outputs_v15").glob("avwap_longshort_trades*_ALL_DAYS_*.csv"),
        key=lambda p: p.stat().st_mtime,
    )
    if not files:
        raise FileNotFoundError("No v15 trade CSV found in outputs_v15")
    return files[-1]
