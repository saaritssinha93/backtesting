# -*- coding: utf-8 -*-
"""
avwap_combined_runner_v18_5min.py - AVWAP v18 COMBINED runner on 5-minute signals
===================================================================================
...
1. All outputs saved to outputs_v18_5min/
...
V18_MORNING_SESSION_WINDOW = (dtime(9, 30, 0), dtime(11, 0, 0))
V18_AFTERNOON_SESSION_WINDOW = (dtime(13, 0, 0), dtime(14, 30, 0))
...
def _apply_v18_session_shortlists(df: pd.DataFrame, side_label: str) -> pd.DataFrame:
    ...
    """
    if df.empty:
        print(f"[V18_SHORTLIST] {side_label}: no rows to filter.")
        return df

    side_key = "long" if str(side_label).upper() == "LONG" else "short"
    shortlist_cache: Dict[Tuple[str, str, str], set] = {}
    keep_mask: List[bool] = []
    outside_window = 0
    no_date = 0
    missing_shortlist = 0
    ...
    for row in df.itertuples(index=False):
        ...
        if cache_key not in shortlist_cache:
            shortlist_cache[cache_key] = set(
                get_two_session_shortlist(date_key, session_name, side_key, as_set=True) or []
            )
        allowed = shortlist_cache[cache_key]
        ticker_val = str(getattr(row, "ticker", "")).strip()
        keep = bool(ticker_val and ticker_val in allowed)
    ...
    print(
        f"[V18_SHORTLIST] {side_label}: {len(df)}->{len(filtered)} "
...
def main() -> None:
    ...
    _outputs_dir = runtime_dir("outputs_v18_5min")
    ...
    with open(log_path, "w", encoding="utf-8") as _log_fh:
        ...
        try:
            ...
            print("=" * 70)
            print("AVWAP v18_5min COMBINED runner — v17c logic + two-session history shortlist")
            ...
            print("\n[V18] Applying session-wise historical shortlist universe...")
            ...
            # --- Save CSV ---
            out_csv = _outputs_dir / f"avwap_longshort_trades_v18_5min_ALL_DAYS_{ts}.csv"
            combined.to_csv(out_csv, index=False)
            out_daywise_csv = _outputs_dir / f"avwap_daywise_breakdown_v18_5min_ALL_DAYS_{ts}.csv"
