...
def _latest_entry_signals_for_ticker(
    ...
    if (
        not allow_signal_today(state, ticker, "SHORT", today_str, SHORT_CAP_PER_TICKER_PER_DAY)
        and not allow_signal_today(state, ticker, "LONG", today_str, LONG_CAP_PER_TICKER_PER_DAY)
    ):
        checks.append(
            {
                "ticker": ticker,
