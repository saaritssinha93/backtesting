...
Default behavior is fast: reuse the latest resolved all-setups cache.
Use --refresh_expansion to rescan raw 5-min/1-min data from scratch.

Outputs:
  C:\\TradingData\\eqidv2\\outputs_ID_v4_5min
"""
...
OUT_ROOT = Path(r"C:\TradingData\eqidv2\outputs_ID_v4_5min")
...
def _refresh_v2_baseline(timeout_sec: int = 0) -> None:
    ...
    print("[v4] refreshing v2 cascade baseline...")
...
def _run_native_all_setups(
    ...
    print("[v4] loading market context for native all-setups scan...")
    market_ctx = v2._load_market_context()
    print(f"[v4] native scan universe={len(universe)} workers={workers} cost_bps={cost_bps}")
    ...
    if errors:
        print(f"[v4][WARN] errored tickers={len(errors)} first5={errors[:5]}")
    ...
    for k, (ticker, ticker_candidates) in enumerate(selected_by_ticker.items(), 1):
        ...
        if k % 100 == 0 or k == len(selected_by_ticker):
            print(f"  [v4 resolve {k:4d}/{len(selected_by_ticker)}] trades={len(trades)}")
    ...
    print(f"[v4] native all-setups trades written: {trades_path}")
    return trades_path
...
def _run_v4(args: argparse.Namespace) -> int:
    ...
    if args.refresh_expansion:
        ...
        if not expansion_csv.exists():
            raise SystemExit(f"missing expansion cache: {expansion_csv}; rerun with --refresh_expansion")
    ...
    text = text.replace("AVWAP ID 5-min v3 expansion backtest", "AVWAP ID 5-min v4 expansion backtest")
    text = text.replace("V3 expanded:", "V4 expanded:")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="AVWAP ID 5-min v4 raw-data expansion runner")
    ...
    ap.add_argument("--refresh_expansion", action="store_true")
