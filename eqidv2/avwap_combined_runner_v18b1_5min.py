# -*- coding: utf-8 -*-
"""
avwap_combined_runner_v18b1_5min.py - AVWAP v18b1 COMBINED runner on 5-minute signals
===================================================================================
...
1. All outputs saved to outputs_v18b1_5min/
...
try:
    ...
    from filtered_stocks_two_session_history import get_shortlist as get_two_session_shortlist
...
V18_MORNING_SESSION_WINDOW = (dtime(9, 30, 0), dtime(12, 30, 0))
V18_AFTERNOON_SESSION_WINDOW = (dtime(13, 0, 0), dtime(15, 0, 0))
...
V18B1_ENABLE_SHORT_PULLBACK = False
V18B1_SHORT_CONTINUATION_MAX_RANK_PCT = 0.20
V18B1_SHORT_CONTINUATION_ENTRY_CUTOFF = dtime(13, 45, 0)
...
NIFTY_RS_BOTH_MODE_THRESHOLD_LONG_PCT = 0.55   # v18b tuned: widen BOTH-mode participation
NIFTY_RS_BOTH_MODE_THRESHOLD_SHORT_PCT = 0.55
...
V17C_SHORT_CONTINUATION_ADX_SLOPE_MIN = _env_float(
    "EQIDV17C_SHORT_CONTINUATION_ADX_SLOPE_MIN", 0.15
)
V17C_SHORT_CONTINUATION_VOLUME_MIN_RATIO = _env_float(
    "EQIDV17C_SHORT_CONTINUATION_VOLUME_MIN_RATIO", 0.75
)
...
V16_LONG_QS_DEAD_LO         = 7.80  # v18b tuned: narrower long QS dead zone
V16_LONG_QS_DEAD_HI         = 7.95
V16_LONG_QS_ABS_MAX         = 10.0  # absolute cap — QS 10+ = 30% win (exhausted momentum)
V16_LONG_AVWAP_DIST_ATR_MAX = 3.25  # v18b tuned: allow slightly wider but still controlled chase distance
...
V16_LONG_AVWAP_DIST_DEAD_LO = 1.15  # v18b tuned
V16_LONG_AVWAP_DIST_DEAD_HI = 1.25
...
def _apply_v18_session_shortlists(df: pd.DataFrame, side_label: str) -> pd.DataFrame:
    ...
    """
    if df.empty:
        print(f"[V18B_SHORTLIST] {side_label}: no rows to filter.")
        return df
                ...
                if side_upper == "SHORT":
                    if meta["rank_pct"] <= V18B_SHORT_COMBINED_MAX_RANK_PCT:
                        if (
                            ...
                            or (
                                setup_val == "C_SHORT_CONTINUATION_BREAK"
                                and meta["rank_pct"] <= V18B1_SHORT_CONTINUATION_MAX_RANK_PCT
                            )
    ...
    print(
        f"[V18B_SHORTLIST] {side_label}: {len(df)}->{len(filtered)} "
...
def _apply_v18b1_trade_cleanup(df: pd.DataFrame, side_label: str) -> pd.DataFrame:
    if df is None or df.empty:
        print(f"[V18B1_CLEANUP] {side_label}: no rows to clean.")
        return df

    filtered = df.copy()
    removed_short_pullback = 0
    removed_late_short_cont = 0

    if str(side_label).upper() == "SHORT":
        if not V18B1_ENABLE_SHORT_PULLBACK and "setup" in filtered.columns:
            pullback_mask = (
                filtered["setup"].astype(str).str.upper()
        ...
        if "setup" in filtered.columns and not filtered.empty:
            if "entry_time_ist" in filtered.columns:
                entry_ts = _normalize_ist_series(filtered["entry_time_ist"])
            elif "entry_time" in filtered.columns:
                entry_ts = _normalize_ist_series(filtered["entry_time"])
            else:
                entry_ts = pd.Series(pd.NaT, index=filtered.index)

            cont_mask = (
                filtered["setup"].astype(str).str.upper()
                == "C_SHORT_CONTINUATION_BREAK"
            )
            late_mask = cont_mask & entry_ts.apply(
                lambda ts: pd.notna(ts)
                and ts.time() >= V18B1_SHORT_CONTINUATION_ENTRY_CUTOFF
            )
            removed_late_short_cont = int(late_mask.sum())
            if removed_late_short_cont:
                filtered = filtered.loc[~late_mask].copy()

    print(
        f"[V18B1_CLEANUP] {side_label}: {len(df)}->{len(filtered)} "
        f"(-{len(df) - len(filtered)} removed | "
        f"short_pullback={removed_short_pullback}, "
        f"late_short_cont={removed_late_short_cont})"
    )
    return filtered
...
def main() -> None:
    ...
    _outputs_dir = runtime_dir("outputs_v18b1_5min")
    _outputs_dir.mkdir(parents=True, exist_ok=True)

    ts = now_ist().strftime("%Y%m%d_%H%M%S")
    log_path = _outputs_dir / f"avwap_combined_runner_v18b1_{ts}.txt"
    ...
    with open(log_path, "w", encoding="utf-8") as _log_fh:
        ...
        try:
            ...
            print("=" * 70)
            print("AVWAP v18b1_5min COMBINED runner — staged shortlist expansion + continuation cleanup")
            ...
            if ENABLE_PLAYBOOK_V11_PROFILE:
                ...
                print("[PROFILE] V18b1: staged short expansion with tighter continuation rescue on fast prebuilt history.")
            ...
            print("\n[V18b1] Applying session-wise historical shortlist universe...")
            short_df = _apply_v18_session_shortlists(short_df, "SHORT")
            long_df = _apply_v18_session_shortlists(long_df, "LONG")
            print("\n[V18b1] Applying post-shortlist trade cleanup...")
            short_df = _apply_v18b1_trade_cleanup(short_df, "SHORT")
            long_df = _apply_v18b1_trade_cleanup(long_df, "LONG")
            ...
            # --- Save CSV ---
            out_csv = _outputs_dir / f"avwap_longshort_trades_v18b1_5min_ALL_DAYS_{ts}.csv"
            combined.to_csv(out_csv, index=False)
            out_daywise_csv = _outputs_dir / f"avwap_daywise_breakdown_v18b1_5min_ALL_DAYS_{ts}.csv"
