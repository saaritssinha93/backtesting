...
def runtime_dir(*parts: str) -> Path:
    return _ORIG_V19A_RUNTIME_DIR("outputs_v19a4_5min")


# ---------------------------------------------------------------------------
# V19A4 Area-3 fix: session-specific rank cutoffs
# SHORT morning:   rank_pct <= 0.50 (morning rank 50-75% PF=1.197 — marginal)
# SHORT mid:       rank_pct <= 0.35 (mid rank 35-50% PF=0.776 — losing, keep cut)
# SHORT afternoon: NO cutoff        (n too small, global cut hurt PF 2.821→2.442)
# LONG:            no cut           (all buckets profitable)
# ---------------------------------------------------------------------------
V19A4_SHORT_MORNING_MAX_RANK_PCT = 0.50
V19A4_SHORT_MID_MAX_RANK_PCT     = 0.35

_ORIG_V19A4_CLEANUP = _apply_v18c5_trade_cleanup


def _apply_v18c5_trade_cleanup(df: pd.DataFrame, side_label: str) -> pd.DataFrame:
    df = _ORIG_V19A4_CLEANUP(df, side_label)
    if str(side_label).upper() != "SHORT":
        return df
    ...
    # Morning: cut rank_pct > 0.50
    morning_ok = (df["shortlist_session"] != "morning") | (
        df["shortlist_rank_pct"].isna() | (df["shortlist_rank_pct"] <= V19A4_SHORT_MORNING_MAX_RANK_PCT)
    )
    # Mid: cut rank_pct > 0.35
    mid_ok = (df["shortlist_session"] != "mid") | (
        df["shortlist_rank_pct"].isna() | (df["shortlist_rank_pct"] <= V19A4_SHORT_MID_MAX_RANK_PCT)
    )
    ...
    if removed > 0:
        print(f"[V19A4] SHORT rank cutoff: -{removed} trades (morning>50%, mid>35%, afternoon=all)")
    return df
...
def _rename_new_outputs(output_dir: Path, before: set[str]) -> List[Path]:
    ...
    for path in output_dir.iterdir():
        ...
        new_name = (
            path.name
            .replace("v18c5_5min", "v19a4_5min")
            .replace("v19a3_5min", "v19a4_5min")
            .replace("v19a2_5min", "v19a4_5min")
            .replace("v19a1_5min", "v19a4_5min")
            .replace("v19a_5min", "v19a4_5min")
            .replace("avwap_combined_runner_v18c3_", "avwap_combined_runner_v19a4_")
            .replace("avwap_combined_runner_v19a3_", "avwap_combined_runner_v19a4_")
            .replace("avwap_combined_runner_v19a2_", "avwap_combined_runner_v19a4_")
            .replace("avwap_combined_runner_v19a1_", "avwap_combined_runner_v19a4_")
            .replace("avwap_combined_runner_v19a_", "avwap_combined_runner_v19a4_")
        )
...
def main() -> None:
    output_dir = runtime_dir("outputs_v19a4_5min")
    output_dir.mkdir(parents=True, exist_ok=True)
    before = {p.name for p in output_dir.iterdir() if p.is_file()}
    print("[V19A4] Starting standalone backtest with three-window shortlist (v3_3times)...")
    print("[V19A4] Session-specific rank cutoffs (Area-3 fix):")
    print(f"[V19A4]   SHORT morning  max rank_pct = {V19A4_SHORT_MORNING_MAX_RANK_PCT:.0%}")
    print(f"[V19A4]   SHORT mid      max rank_pct = {V19A4_SHORT_MID_MAX_RANK_PCT:.0%}")
    print("[V19A4]   SHORT afternoon = no cutoff (all ranks allowed)")
    print("[V19A4]   LONG            = no cutoff")
    print("[V19A4] Inherited from v19a2:")
    print("[V19A4]   LONG RECLAIM        SL=0.70%  TGT=1.40%")
    print("[V19A4]   SHORT signals blocked 10:35-11:05")
    print("[V19A4]   LONG  signal  blocked 09:45")
    _ORIG_V19A_MAIN()
    renamed = _rename_new_outputs(output_dir, before)
    if renamed:
        print("[V19A4] Output files:")
        for path in renamed:
            print(f"[V19A4]   {path}")
