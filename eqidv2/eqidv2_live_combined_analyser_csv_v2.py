    ...
    for idx, t in enumerate(tickers, start=1):
        ...
        try:
            ...
            if LATEST_SLOT_ONLY and (target_slot_end is not None):
                ...
                # Some builders store candle START timestamps (e.g., 09:30) while the scheduler
                # computes the candle END slot (e.g., 09:45). If so, allow a -15m shift.
                expected_start_ts = expected_ts - pd.Timedelta(minutes=15)

                drift_end = abs((last_ts - expected_ts).total_seconds())
                drift_start = abs((last_ts - expected_start_ts).total_seconds())

                if (drift_end > float(LATEST_SLOT_TOLERANCE_SEC)) and (drift_start > float(LATEST_SLOT_TOLERANCE_SEC)):
                    all_checks.append({
