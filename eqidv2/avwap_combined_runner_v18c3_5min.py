# -*- coding: utf-8 -*-
"""
avwap_combined_runner_v18c3_5min.py - AVWAP v18c3 COMBINED runner on 5-minute signals
===================================================================================
...
1. All outputs saved to outputs_v18c3_5min/
...
V18C3_DATA_5M_DIR = Path(
    os.getenv(
        "EQIDV18C3_DATA_5M_DIR",
...
V18C3_ENABLE_SHORT_PULLBACK = False
V18C3_ENABLE_LONG_CLOSE_CONTINUATION = False
V18C3_SHORT_CONT_LATE_CUTOFF = dtime(13, 45, 0)
V18C3_LONG_RESCUE_MAX_TIER = 1
V18C3_LONG_LATE_A_MOD_CUTOFF = dtime(13, 45, 0)
V18C3_LONG_LATE_A_MOD_BLOCK_TIERS = {1, 3}
V18C3_MID_START = dtime(10, 30, 0)
V18C3_MID_END = dtime(11, 59, 59)
V18C3_PM_START = dtime(12, 0, 0)
V18C3_PM_END = dtime(13, 44, 59)
V18C3_EARLY_END = dtime(10, 29, 59)
V18C3_SHORT_CONT_T2_OVERSOLD_RSI_MAX = 19.0
V18C3_SHORT_CONT_T2_OVERSOLD_AVWAP_MIN_ATR = 1.20
...
def _apply_v18c3_trade_cleanup(df: pd.DataFrame, side_label: str) -> pd.DataFrame:
    if df is None or df.empty:
        print(f"[V18C3_CLEANUP] {side_label}: no rows to clean.")
        return df
    ...
    if str(side_label).upper() == "SHORT":
        if not V18C3_ENABLE_SHORT_PULLBACK:
        ...
        cont_late_mask = cont_mask & entry_time_s.ge(V18C3_SHORT_CONT_LATE_CUTOFF)
        ...
        cont_t2_oversold_mask = (
            ...
            & pd.to_numeric(filtered.get("rsi_signal", np.nan), errors="coerce").le(
                V18C3_SHORT_CONT_T2_OVERSOLD_RSI_MAX
            )
            & pd.to_numeric(filtered.get("avwap_dist_atr_signal", np.nan), errors="coerce").ge(
                V18C3_SHORT_CONT_T2_OVERSOLD_AVWAP_MIN_ATR
            )
        )
        ...
        cont_t2_short_mid_mask = (
            ...
            & source_s.eq("short")
            & tier_s.eq(2)
            & entry_time_s.ge(V18C3_MID_START)
            & entry_time_s.le(V18C3_MID_END)
        )
        ...
        cont_t2_combined_early_mask = (
            ...
            & tier_s.eq(2)
            & entry_time_s.le(V18C3_EARLY_END)
        )
        ...
        amod_t2_combined_pm_mask = (
            ...
            & source_s.eq("combined")
            & tier_s.eq(2)
            & entry_time_s.ge(V18C3_PM_START)
        )
        ...
        if not V18C3_ENABLE_LONG_CLOSE_CONTINUATION:
        ...
        rescue_mask = source_s.eq("combined_rescue") & tier_s.gt(float(V18C3_LONG_RESCUE_MAX_TIER))
        ...
        late_amod_mask = (
            ...
            & tier_s.isin(sorted(V18C3_LONG_LATE_A_MOD_BLOCK_TIERS))
            & entry_time_s.ge(V18C3_LONG_LATE_A_MOD_CUTOFF)
        )
    ...
    print(
        f"[V18C3_CLEANUP] {side_label}: {len(df)}->{len(filtered)} "
...
def apply_live_parity_profile(
    ...
    if ENABLE_PLAYBOOK_V11_PROFILE:
        ...
        long_cfg.enable_setup_a_close_continuation_break = V18C3_ENABLE_LONG_CLOSE_CONTINUATION
...
def main() -> None:
    ...
    _outputs_dir = runtime_dir("outputs_v18c3_5min")
    ...
    with open(log_path, "w", encoding="utf-8") as _log_fh:
        ...
        try:
            ...
            print("=" * 70)
            print("AVWAP v18c3_5min COMBINED runner — lower-DD cleanup package")
            ...
            short_df = _apply_v18c3_trade_cleanup(short_df, "SHORT")
            long_df = _apply_v18c3_trade_cleanup(long_df, "LONG")
            ...
            # --- Save CSV ---
            out_csv = _outputs_dir / f"avwap_longshort_trades_v18c3_5min_ALL_DAYS_{ts}.csv"
            combined.to_csv(out_csv, index=False)
            out_daywise_csv = _outputs_dir / f"avwap_daywise_breakdown_v18c3_5min_ALL_DAYS_{ts}.csv"
