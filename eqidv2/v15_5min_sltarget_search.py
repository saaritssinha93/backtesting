...
import avwap_combined_runner_v15_5min as base


ROOT = Path(__file__).resolve().parent
OUT_DIR = ROOT / "outputs_v15_5min_sltarget_search"
ROUNDTRIP_COST_PCT_FALLBACK = 0.16


def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Search v15_5min short/long SL+target settings")
...
def _latest_v15_csv() -> Path:
    files = sorted(
        (ROOT / "outputs_v15_5min").glob("avwap_longshort_trades_v15_5min_ALL_DAYS_*.csv"),
        key=lambda p: p.stat().st_mtime,
    )
    if not files:
        raise FileNotFoundError("No v15_5min trade CSV found in outputs_v15_5min")
    return files[-1]
...
def _build_trade_paths(trades: pd.DataFrame, cache: Dict[str, pd.DataFrame]) -> List[dict]:
    rows: List[dict] = []
    for row in trades.itertuples(index=False):
        ticker = str(row.ticker)
        df_1m = cache.get(ticker)
...
def main() -> None:
    ...
    cache = _load_1m_cache(trades["ticker"].astype(str).unique())
    ...
    summary = {
        "source_v15_5min_csv": str(trades_csv),
        "baseline": {
