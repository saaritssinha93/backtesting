# -*- coding: utf-8 -*-
"""
EQIDV2 Off-market "row-by-row" signal scan for ONE date (default: today IST)
...
def main() -> None:
    ...
    import eqidv2_live_combined_analyser_csv_v2 as live

    # Point it at the same dir/suffix the user wants (for tickers list)
    live.DIR_15M = args.data_dir
    live.SUFFIX_15M = args.suffix

    # Patch allow_signal_today to accept the legacy 'sid' kwarg used inside _latest_entry_signals_for_ticker.
    # We'll preserve true per-side caps by delegating to the original allow_signal_today.
    _allow_orig = live.allow_signal_today

    def _allow_sid(state: Dict[str, Any], ticker: str, sid: str = None) -> bool:
        today = sid or target_day_str
        ok_short = _allow_orig(state, ticker, "SHORT", today, int(getattr(live, "SHORT_CAP_PER_TICKER_PER_DAY", 1)))
        ok_long = _allow_orig(state, ticker, "LONG", today, int(getattr(live, "LONG_CAP_PER_TICKER_PER_DAY", 1)))
        return ok_short or ok_long

    live.allow_signal_today = _allow_sid  # type: ignore
    ...
    for tkr in tickers:
        ...
        for k in range(max(int(args.min_bars) - 1, 0), len(df_day)):
            ...
            for s in sigs:
                ...
                if not _allow_orig(state, s.ticker, s.side, target_day_str, 1):
                    continue
