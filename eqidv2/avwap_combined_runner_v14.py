# -*- coding: utf-8 -*-
"""
avwap_combined_runner_v14.py - AVWAP v14 COMBINED runner: V11 SHORT + V9 LONG (hybrid)
==================================================================================
...
NIFTY_CONTEXT_OR_END_TIME = dtime(10, 15, 0)
NIFTY_CONTEXT_CONFIRM_TIME = dtime(10, 30, 0)
NIFTY_CONTEXT_MIN_DAYMOVE_PCT = 0.20
NIFTY_RS_FILTER_ENABLED = True
NIFTY_RS_LOOKBACK_BARS = 3
NIFTY_RS_THRESHOLD_PCT = 0.15
        ...
        for row in d.itertuples(index=False):
            ...
            if mode != "BOTH":
                ...
                if np.isfinite(stock_ret) and np.isfinite(nifty_ret):
                    ...
                    if side_u == "LONG":
                        keep = rel_val >= float(NIFTY_RS_THRESHOLD_PCT)
                    else:
                        keep = rel_val <= -float(NIFTY_RS_THRESHOLD_PCT)
...
def main() -> None:
    ...
    _outputs_dir = _project_root / "outputs_v14"
    ...
    with open(log_path, "w", encoding="utf-8") as _log_fh:
        ...
        try:
            print("=" * 70)
            print("AVWAP v14 COMBINED runner — V11 SHORT + V9 LONG (hybrid)")
            ...
            if NIFTY_CONTEXT_ENABLED:
                ...
                if mode_map:
                    print(
                        "[NIFTY_CONTEXT] "
                        f"Source={context_src} | confirm={NIFTY_CONTEXT_CONFIRM_TIME.strftime('%H:%M')} | "
            ...
            # --- Save CSV ---
            out_csv = _outputs_dir / f"avwap_longshort_trades_v14_ALL_DAYS_{ts}.csv"
