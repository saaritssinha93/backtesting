# -*- coding: utf-8 -*-
"""
Sharded wrappers for EQIDV2 LIVE Scanner V15 LONG.

Purpose:
- keep the existing v15 long scanner untouched
...
import eqidv2_live_combined_analyser_csv_v15_long as base_long
from eqidv2_runtime_paths import report_subdir, runtime_dir


ROOT = Path(__file__).resolve().parent
SHARD_DIR = ROOT / "shards" / "v15_long"
...
def _manifest_path(shard_id: int, shard_count: int) -> Path:
    return SHARD_DIR / f"v15_long_shard_{shard_id:02d}_of_{shard_count:02d}.txt"


def load_v15_long_shard_tickers(shard_id: int, shard_count: int = 10) -> List[str]:
...
def verify_v15_long_shards(shard_count: int = 10) -> Dict[str, object]:
    manifests: Dict[int, List[str]] = {}
    all_seen: Dict[str, int] = {}
    overlaps: Dict[str, List[int]] = {}

    for shard_id in range(1, int(shard_count) + 1):
        tickers = load_v15_long_shard_tickers(shard_id, shard_count)
...
def configure_v15_long_shard(shard_id: int, shard_count: int = 10) -> List[str]:
    assigned_tickers = load_v15_long_shard_tickers(shard_id, shard_count)

    base_long._apply_v15_long_overrides()

    suffix = f"s{int(shard_id):02d}_of_{int(shard_count):02d}"
    v2 = base_long.v2

    v2.REPORTS_DIR = report_subdir(f"eqidv2_reports_v15_long_{suffix}")
    v2.REPORTS_DIR.mkdir(parents=True, exist_ok=True)

    v2.OUT_CHECKS_DIR = runtime_dir(f"out_eqidv2_live_checks_15m_v15_long_{suffix}")
    v2.OUT_SIGNALS_DIR = runtime_dir(f"out_eqidv2_live_signals_15m_v15_long_{suffix}")
    v2.OUT_CHECKS_DIR.mkdir(parents=True, exist_ok=True)
    v2.OUT_SIGNALS_DIR.mkdir(parents=True, exist_ok=True)

    v2.STATE_FILE = ROOT / "logs" / f"eqidv2_avwap_live_state_v11_v15_long_{suffix}.json"
    ...
    v2.SCAN_MAX_WORKERS = min(
        ...
        _env_int("EQIDV15_LONG_SHARD_SCAN_MAX_WORKERS", default_workers, min_value=1),
    )
    v2.SCAN_BLOCK_SIZE = min(
        len(assigned_tickers),
        _env_int("EQIDV15_LONG_SHARD_SCAN_BLOCK_SIZE", default_block_size, min_value=1),
    )

    return assigned_tickers


def run_v15_long_shard_main(shard_id: int, shard_count: int = 10) -> None:
    assigned_tickers = configure_v15_long_shard(shard_id, shard_count)
    base_long._refresh_v15_nifty_context()
    print(
        f"[V15_LONG_SHARD] shard={shard_id}/{shard_count} | "
        f"tickers={len(assigned_tickers)} | "
        f"first={assigned_tickers[0]} | last={assigned_tickers[-1]} | "
        f"state={base_long.v2.STATE_FILE.name} | "
        f"block_size={base_long.v2.SCAN_BLOCK_SIZE} | "
        f"max_workers={base_long.v2.SCAN_MAX_WORKERS} | "
