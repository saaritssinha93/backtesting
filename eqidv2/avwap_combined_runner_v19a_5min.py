# -*- coding: utf-8 -*-
"""
avwap_combined_runner_v19a_5min.py - AVWAP v19a standalone 5-minute runner
============================================================================

Standalone v19a runner with the v18c5 engine plus the v18c6/v19a behaviour
inlined directly into this file.
...
- v18c6-style RS context upgrades:
    short continuation uses faster 4-bar RS context
    long reclaim uses faster 4-bar RS context
- v18c6-style exit levels:
    SHORT all setups: SL=0.85%, TGT=1.40%
    LONG standard:    SL=0.70%, TGT=1.30%
    LONG reclaim:     SL=0.85%, TGT=1.20%
"""
...
try:
    ...
    print("[V19A] Using shortlist v3_3times (filtered_stocks_two_session_history_v3_3times)")
except ImportError:
    ...
    print(
        "[V19A] WARNING: filtered_stocks_two_session_history_v3_3times not found. "
...
V19A_SHORT_STOP_PCT = 0.0055
V19A_SHORT_TARGET_PCT = 0.0180
V19A_LONG_STOP_PCT = 0.0065
V19A_LONG_TARGET_PCT = 0.0150

V19A_SHORT_AMOD_STOP_PCT = 0.0065
V19A_SHORT_AMOD_TARGET_PCT = 0.0190
V19A_SHORT_BHUGE_STOP_PCT = 0.0085
V19A_SHORT_BHUGE_TARGET_PCT = 0.0140

V19A_LONG_RECLAIM_STOP_PCT = 0.0070
V19A_LONG_RECLAIM_TARGET_PCT = 0.0185
...
def runtime_dir(*parts: str) -> Path:
    return _ORIG_V19A_RUNTIME_DIR("outputs_v19a_5min")
...
def _rename_new_outputs(output_dir: Path, before: set[str]) -> List[Path]:
    ...
    for path in output_dir.iterdir():
        ...
        new_name = (
            path.name
            .replace("v18c5_5min", "v19a_5min")
            .replace("avwap_combined_runner_v18c3_", "avwap_combined_runner_v19a_")
        )
...
def main() -> None:
    output_dir = runtime_dir("outputs_v19a_5min")
    output_dir.mkdir(parents=True, exist_ok=True)
    before = {p.name for p in output_dir.iterdir() if p.is_file()}
    print("[V19A] Starting standalone backtest with three-window shortlist (v3_3times)...")
    _ORIG_V19A_MAIN()
    renamed = _rename_new_outputs(output_dir, before)
    if renamed:
        print("[V19A] Output files:")
        for path in renamed:
            print(f"[V19A]   {path}")
