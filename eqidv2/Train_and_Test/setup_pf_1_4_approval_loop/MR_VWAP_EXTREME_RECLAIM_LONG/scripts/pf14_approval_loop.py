...
def main() -> int:
    ...
    # ---- card baseline (from final_setup_conf if importable; else raw-pool exit/defaults) ----
    def pool_exit_default(col, fallback):
        if col not in pool.columns:
            return fallback
        s = pd.to_numeric(pool[col], errors="coerce").dropna()
        return fallback if s.empty else round(float(s.median()), 2)

    base_cfg = {"sl": pool_exit_default("v7_signal_sl_pct", 0.70),
                "tgt": pool_exit_default("v7_signal_target_pct", 0.80),
                "mask_terms": [], "premom_terms": [], "guard": None, "status": "OK",
    ...
    try:
        ...
        if raw:
            best_found = raw.get("best_found") or {}
            former = raw.get("former_config") or {}
            exit_cfg = raw.get("exit") or best_found.get("exit") or former.get("exit") or {}
            mask_terms = [tuple(t) for t in (raw.get("mask_terms") or best_found.get("mask_terms") or [])]
            premom_terms = [tuple(t) for t in (raw.get("pre_momentum_terms") or best_found.get("pre_momentum_terms") or [])]
            guard = raw.get("entry_guards") or best_found.get("entry_guards") or None
            base_cfg = {"sl": float(exit_cfg.get("sl_pct", 0.70)), "tgt": float(exit_cfg.get("tgt_pct", 1.50)),
                        ...
                        "guard": guard, "status": "OK",
                        "max_positions": 20, "daily_loss_rs": 0.0}
            if not exit_cfg:
                base_cfg["sl"] = pool_exit_default("v7_signal_sl_pct", 0.70)
                base_cfg["tgt"] = pool_exit_default("v7_signal_target_pct", 0.80)
