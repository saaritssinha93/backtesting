...
def main() -> int:
    ...
    base_cfg = {"sl": 1.00, "tgt": 1.50, "mask_terms": [("regime", "!=", "BULL")],
    ...
    try:
        ...
        if raw:
            base_cfg = {"sl": float(raw["exit"]["sl_pct"]), "tgt": float(raw["exit"]["tgt_pct"]),
    ...
    def cand_from(cfg, label):
        ...
        if not c["test_ran"]:                      # also score test for reporting (band may miss)
            mTE, dTE = full_metrics(setup, cfg, TEST); c["test"], c["test_detail"] = mTE, dTE
    ...
    # also any individual trial whose FIT&VAL both look band-ish -> confirm the top 15 by score
    top_tr = sorted([t for t in trial_rows if t["fit_n"] >= MIN_FITVAL and t["val_n"] >= MIN_FITVAL],
                    key=lambda t: -t["score"])[:15]
