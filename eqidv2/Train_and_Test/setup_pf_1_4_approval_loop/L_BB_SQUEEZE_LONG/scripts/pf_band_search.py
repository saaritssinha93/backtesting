...
CARD_BASELINES = {
    ...
    # L_BB_SQUEEZE_LONG baseline = the RESEARCH_WATCH "robustness_first_best" documented config
    # (regime==NEUTRAL & pre3_range_r<=0.887 @ 0.90/2.50). REJECT (enabled=False): ungated heavy loser
    # (train 0.69 / test 0.72), best config p 0.121 (not significant), ~55 raw test trades.
    "L_BB_SQUEEZE_LONG": {
        "sl": 0.90, "tgt": 2.50, "mask_terms": [["regime", "==", "NEUTRAL"]],
        "premom_terms": [["pre3_range_r", "<=", 0.887]],
        "guard": None, "max_positions": 20, "daily_loss_rs": 0.0,
    },
}
# This setup's documented edge is a categorical regime filter, so let the search explore it too.
REGIME_CHOICES = [None, "NEUTRAL", "TREND", "!BULL", "!BEAR"]
...
def _suggest(trial, mask_quant, pm_quant, max_mask=3, max_pm=2):
    ...
    # categorical regime filter (this setup's documented edge): None / ==X / !=X
