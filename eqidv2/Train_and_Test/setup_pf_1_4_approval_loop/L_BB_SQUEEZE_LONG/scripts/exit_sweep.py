...
# L_BB_SQUEEZE_LONG documented gates (RESEARCH_WATCH robustness_first_best).
CARD_MASK = [["regime", "==", "NEUTRAL"]]
CARD_PREMOM = [["pre3_range_r", "<=", 0.887]]


def _base(kind):
    if kind == "card":
        return {"mask_terms": CARD_MASK, "premom_terms": CARD_PREMOM, "guard": None,
...
def main() -> int:
    ...
    for kind in ("raw", "card"):
        ...
        md += [f"## Base: {kind.upper()}" + ("  (detection only — pure exit effect, full sample)" if kind == "raw"
               else "  (documented gates kept: regime==NEUTRAL & pre3_range_r≤0.887)"), ""]
