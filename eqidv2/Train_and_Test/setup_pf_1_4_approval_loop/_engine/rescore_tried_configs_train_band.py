...
TT_DIR = HERE.parent.parent
REPO = TT_DIR.parent
for p in (REPO, TT_DIR, HERE):
...
def eval_metrics(setup: str, cfg, df, full: bool = False):
...
def fmt_cfg(setup: str, side: str, cfg):
    block = eng.cfg_to_conf_block(setup, side, cfg)
...
def main() -> int:
    ...
    ap.add_argument("--pf_hi", type=float, default=1.70)
    ...
    side = args.side.strip().upper() if args.side else (
        pool["side"].mode().iloc[0] if len(pool) else "LONG"
    )
    ...
    for i, cfg in enumerate(cfgs.values(), 1):
        mtr = eval_metrics(setup, cfg, train, full=False)
        row = {
            "idx": i,
            "config": fmt_cfg(setup, side, cfg),
            "train_n": mtr["n"],
            "train_pf": mtr["net_pf"],
        ...
        if mtr["n"] >= args.min_train_trades and args.pf_lo <= float(mtr["net_pf"]) <= args.pf_hi:
            train_band.append((cfg, row))

    confirmed = []
    for cfg, row in train_band:
        mtr = eval_metrics(setup, cfg, train, full=True)
        if not (mtr["n"] >= args.min_train_trades and args.pf_lo <= float(mtr["net_pf"]) <= args.pf_hi):
            continue
        mte = eval_metrics(setup, cfg, test, full=True)
        rec = dict(row)
        rec.update({
    ...
    summary = {
        ...
        "best_confirmed": confirmed_df.iloc[0].to_dict() if not confirmed_df.empty else None,
        "passing": passing,
    }
    ...
    lines = [
        ...
        "",
        f"Rescored every unique config logged by {len(args.trials)} trial file(s) on TRAIN first.",
        ...
        f"- TEST PF > {args.test_pf_min:.2f} configs: {len(passing)}",
        "",
    ]
    if best:
        lines += [
    ...
    if confirmed:
        top = confirmed_df.iloc[0].to_dict()
        lines += [
