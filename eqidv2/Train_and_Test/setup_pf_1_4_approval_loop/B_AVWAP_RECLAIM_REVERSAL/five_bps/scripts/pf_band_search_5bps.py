...
CARD_BASELINES = {
    ...
    # B_AVWAP_RECLAIM_REVERSAL card (§2): mask = un-inverted near-VWAP edge vwap_dist_atr<=1.0; exit 0.70/1.50.
    # (Live overlay uses the INVERTED vwap_dist_atr>=0.60 — see cross-check §5.2; a winner must NOT rely on that.)
    "B_AVWAP_RECLAIM_REVERSAL": {
        "sl": 0.70, "tgt": 1.50, "mask_terms": [["vwap_dist_atr", "<=", 1.0]],
        "premom_terms": [], "guard": None, "max_positions": 20, "daily_loss_rs": 0.0,
    },
}
# Let the search also explore the categorical regime filter (detection is regime!=BEAR).
REGIME_CHOICES = [None, "!BEAR", "!BULL", "NEUTRAL", "TREND"]
...
def _suggest(trial, mask_quant, pm_quant, max_mask=3, max_pm=2):
    ...
    if rg:
        mask_terms = list(mask_terms) + ([["regime", "!=", rg[1:]]] if rg.startswith("!") else [["regime", "==", rg]])
...
def write_failure_md(path, setup, label, m):
    L = [f"# FAILURE_ANALYSIS — {setup}", "",
         f"Loss diagnosis for **{label}** (full-TRAIN book, 5 bps/leg, net of cost).", ""]
...
def write_candidates_md(path, setup, cands):
    L = [f"# CANDIDATE_CONFIGS — {setup}", "",
         ...
         "and trade/day/symbol-dominance stability) are listed. Net of cost @5 bps/leg.", ""]
    if not cands:
...
def write_final_md(path, setup, side, cands, rerun_cmd, baseline_cmd, sess):
    ...
    L += ["## Verdict: **APPROVAL REQUIRED** (best candidate cleared the gate; awaiting your explicit OK)", "",
          ...
          "- B_AVWAP_RECLAIM_REVERSAL has a known live-overlay contradiction: conf/card uses near-VWAP `vwap_dist_atr<=1.0`, while the legacy overlay used inverted `vwap_dist_atr>=0.60`; do not promote any config that relies on the inverted overlay.", "",
...
def main() -> int:
    ...
    baseline_cmd = (f"py -3.12 Train_and_Test/setup_loop_runner.py --setup {setup} --pool {args.pool} "
                    ...
                    f"--test_start {args.test_start} --test_end <latest> --slippage_bps {args.slippage_bps}")
    ...
    # ---- baseline metrics at requested 5 bps + verification pass ----
    base_tr15 = full_metrics(setup, baseline, TRAIN); base_te15 = full_metrics(setup, baseline, TEST)
    print(f"[pf-band] BASELINE @5bps TRAIN {_fmt_m(base_tr15)}")
    print(f"[pf-band] BASELINE @5bps TEST  {_fmt_m(base_te15)}")
    ...
    summary = {"setup": setup, "side": args.side, "optimizer": engine, "windows": sess,
               "baseline_train_5bps_run": {k: v for k, v in base_tr15.items() if k != "detail"},
               "baseline_test_5bps_run": {k: v for k, v in base_te15.items() if k != "detail"},
