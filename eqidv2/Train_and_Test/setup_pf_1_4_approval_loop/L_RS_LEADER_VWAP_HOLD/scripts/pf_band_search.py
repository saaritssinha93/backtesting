...
Run (from repo root):
  py -3.12 Train_and_Test/setup_pf_1_4_approval_loop/L_RS_LEADER_VWAP_HOLD/scripts/pf_band_search.py \
...
def write_baseline_md(path, setup, side, baseline, sess, base_tr15, base_te15, base_tr5, base_te5):
    L = [f"# BASELINE_RESULT — {setup} ({side})", "",
         ...
         "- **Detection (raw, unchanged):** `rs_pct>=0.75`, `stock_ret>=0.30`, `close>EMA20`, `EMA20>=EMA50`, `ema20_slope_3bar>0`, `low<=VWAP+0.30*ATR`, `close>VWAP`, `close>open`, `close_loc>=0.60`, `close>prev_bar_high`, `vol_ratio>=1.3`, `ADX>=20`, `50<=RSI<=72`, `regime!=BEAR`; scan window 09:45-14:00 IST.", "",
...
def write_failure_md(path, setup, label, m):
    ...
    L += ["", "## Notes",
          ...
          "A high SL share with low target share = fake VWAP hold / no follow-through (RS leader signal "
          "is being faded). Pre-momentum / volume / near-VWAP gates aim to remove the no-follow-through subset; the "
...
def write_final_md(path, setup, side, cands, rerun_cmd, baseline_cmd, sess):
    ...
    L += ["## Verdict: **APPROVAL REQUIRED** (best candidate cleared the gate; awaiting your explicit OK)", "",
          ...
          "- L_RS_LEADER_VWAP_HOLD is scanner-source and was demoted after live paper failure on 2026-06-16..2026-06-22; any re-promotion needs explicit approval plus fresh live-paper confirmation.", "",
...
def main() -> int:
    ...
    split_note = (
        f"requested split used: TRAIN {args.train_start}..session before {args.test_start}; "
        f"TEST {args.test_start}..latest"
    )
    if not TRAIN_s or not TEST_s:
        need = int(args.fallback_train_sessions) + int(args.fallback_test_sessions)
        if len(sessions) < max(4, need):
            n_test = max(1, len(sessions) // 4)
            n_train = len(sessions) - n_test
        else:
            n_train = int(args.fallback_train_sessions)
            n_test = int(args.fallback_test_sessions)
        TEST_s = sessions[-n_test:]
        TRAIN_s = sessions[-(n_train + n_test):-n_test]
        split_note = (
            f"requested TEST start {args.test_start} unavailable for {setup}; "
            f"pool sessions end {sessions[-1].date()}. Used nearest available rolling split: "
            f"TRAIN last {len(TRAIN_s)} sessions before TEST, TEST last {len(TEST_s)} sessions."
        )
        print(f"[pf-band] WARNING {split_note}")
    if not TRAIN_s or not TEST_s:
        print(f"[pf-band] cannot form split after fallback: train_n={len(TRAIN_s)} test_n={len(TEST_s)} "
    ...
    for cfg in ordered:
        ...
        gate_rows.append({
            "sl": _ser_cfg(cfg)["sl"],
            "tgt": _ser_cfg(cfg)["tgt"],
            "mask_terms": json.dumps(_ser_cfg(cfg)["mask_terms"]),
            "premom_terms": json.dumps(_ser_cfg(cfg)["premom_terms"]),
            "entry_guards": json.dumps(_ser_cfg(cfg)["entry_guards"]),
            "max_positions": _ser_cfg(cfg)["max_positions"],
            "daily_loss_rs": _ser_cfg(cfg)["daily_loss_rs"],
            "train_trades": g["train"]["trades"],
            "train_pf": g["train"]["net_pf"],
            "train_net": g["train"]["net_pnl"],
            "train_tpd": g["train"]["trades_per_day"],
            "train_trade_dom": g["train"]["trade_dom_gross"],
            "train_day_dom": g["train"]["day_dom"],
            "train_sym_dom": g["train"]["sym_dom"],
            "test_scored": bool(g["test"] is not None),
            "test_trades": test_m.get("trades", ""),
            "test_pf": test_m.get("net_pf", ""),
            "test_net": test_m.get("net_pnl", ""),
            "test_tpd": test_m.get("trades_per_day", ""),
            "test_trade_dom": test_m.get("trade_dom_gross", ""),
            "test_day_dom": test_m.get("day_dom", ""),
            "test_sym_dom": test_m.get("sym_dom", ""),
            "passed": g["pass"],
            "reject_reasons": "; ".join(g["reasons"]),
            "cfg_json": json.dumps(_ser_cfg(cfg), sort_keys=True),
        })
        if g["pass"]:
            cands.append({"cfg": _ser_cfg(cfg), "train": g["train"], "test": g["test"],
                          "risk": "nearest-available TEST window; scanner-source setup demoted after live paper failure.",
                          "rec": "APPROVAL REQUIRED"})
    gate_df = pd.DataFrame(gate_rows)
    if not gate_df.empty:
        gate_df.to_csv(outdir / "gate_evaluations.csv", index=False)
    ...
    for i, c in enumerate(cands, 1):
        (outdir / "candidates" / f"{setup}_candidate_{i:03d}.json").write_text(
            json.dumps(_json_safe({"setup": setup, "side": args.side, "windows": sess, "config": c["cfg"],
    ...
    near_miss = None
    if gate_rows:
        scored = [r for r in gate_rows if r["train_trades"] >= args.min_train_trades]
        pool_rows = scored or gate_rows
        near_miss = sorted(pool_rows, key=lambda r: (
            bool(r["test_scored"]),
            float(r["test_pf"] or -1),
            float(r["train_pf"] or 0),
            int(r["train_trades"] or 0),
        ), reverse=True)[0]
    ...
    summary = {"setup": setup, "side": args.side, "optimizer": engine, "windows": sess,
               ...
               "near_miss": near_miss}
    (outdir / "search_summary.json").write_text(
