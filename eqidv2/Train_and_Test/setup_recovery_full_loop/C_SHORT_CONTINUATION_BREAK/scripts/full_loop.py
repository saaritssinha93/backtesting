r"""full_loop.py — Stages 3-8 for C_SHORT_CONTINUATION_BREAK (research-only; PRE-collapse pool basis).
...
def main() -> int:
    ...
    VERSIONS = {
        "V0_raw": base_cfg(sl=0.90, tgt=1.50),
        "V1_wide_exit": base_cfg(sl=0.90, tgt=2.00),
        "V2_fresh_break": base_cfg(sl=0.90, tgt=1.50, prefilter_terms=[("fire_seq", "<=", 1.0)]),
        "V3_fresh_morning": base_cfg(sl=0.90, tgt=1.50, prefilter_terms=[("fire_seq", "<=", 1.0)],
                                     guard={"max_slot": "12:30"}),
        "V5_not_overext": base_cfg(sl=0.90, tgt=1.50,
                                   mask_terms=[("vwap_dist_atr", ">=", q("vwap_dist_atr", 0.25) or -3.0)]),
        "V6_vol_band": base_cfg(sl=0.90, tgt=1.50,
                                prefilter_terms=[("vol_ratio", ">=", 1.8), ("vol_ratio", "<=", 3.2)]),
        "V7_candle_quality": base_cfg(sl=0.90, tgt=1.50,
                                      prefilter_terms=[("body_pct", ">=", 0.60)]),
        "V8_bear_tape": base_cfg(sl=0.90, tgt=1.50, mask_terms=[("market_ret_pct", "<=", 0.0)]),
        "V9_ranked_top2": base_cfg(sl=0.90, tgt=1.50, guard={"top_n": 2}),
        "V10_late_drift": base_cfg(sl=0.90, tgt=1.50, guard={"min_slot": "13:00"}),
        "V11_broad_gate": base_cfg(sl=0.90, tgt=1.50,
                                   premom_terms=[("sig5_adx_calc", ">=", 30.0), ("pre1_adx", "<=", 25.0)]),
        "V12_fresh_vol": base_cfg(sl=0.90, tgt=1.50,
                                  prefilter_terms=[("fire_seq", "<=", 1.0), ("vol_ratio", ">=", 2.0)]),
    }
    stage3 = {}
