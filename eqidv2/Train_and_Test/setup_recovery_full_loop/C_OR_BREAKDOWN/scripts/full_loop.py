r"""full_loop.py — Stages 3-8 for C_OR_BREAKDOWN (research-only).
...
def main() -> int:
    ...
    VERSIONS = {
        "V0_raw": base_cfg(),
        "V1_conf_gate": base_cfg(premom_terms=[("sig5_adx_calc", ">=", 39.670518),
                                               ("pre1_adx", "<=", 21.368044)]),
        ...
        "V4_trend_struct": base_cfg(prefilter_terms=[("ema20_slope", "<=", 0.0), ("adx", ">=", 25.0)]),
        "V5_not_overext": base_cfg(prefilter_terms=[("rsi", ">=", 25.0)],
                                   mask_terms=[("vwap_dist_atr", ">=", q("vwap_dist_atr", 0.25) or -2.0)]),
        "V6_vol_band": base_cfg(prefilter_terms=[("vol_ratio", ">=", 1.8), ("vol_ratio", "<=", 3.2)]),
        "V7_candle_quality": base_cfg(prefilter_terms=[("body_pct", ">=", 0.60),
                                                       ("lower_wick_pct", "<=", 0.25)]),
        "V8_bear_tape": base_cfg(mask_terms=[("market_ret_pct", "<=", 0.0)]),
