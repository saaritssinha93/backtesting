...
FINAL_SETUP_CONF = {
    ...
    # Added at the user's explicit instruction despite NOT clearing the gate. Detector:
    # avwap_5min_ID_v2_backtesting._scan_day (flag ENABLE_S9_MIDDAY_LOSE, default OFF;
    # auto-enabled under the conf book). Shared by v11 backtest AND v7 live. Full research:
    # Train_and_Test/long_setup_discovery_from_raw_data/claude_engine/. NOT a fast scalp
    # (~56% EOD exits, ~3.7h hold). Keep in sync with the repo-root final_setup_conf.py.
    # =======================================================================
    "S9_MIDDAY_LOSE": {
        "side": "SHORT",
        "detection": {
            ...
            "idea": ("late-morning (10:20-11:00 IST) failed bounce that loses session VWAP: prior bar "
                     ...
                     "positive 3-bar push, with ATR room (atr_pct>=0.30%); regime != BULL."),
            "conditions": [
                ("close", "<", "open"),
                ("close", "<", "VWAP"),
                ("close", "<", "prev_bar_low"),
        ...
        "provenance": {
            ...
            "gate_miss": ("3-month PF ~1.00 @5bps (break-even); OOS April lost (PF 0.65, -Rs20k); "
                          "loses at 15bps (PF 0.71); 8-week edge did not persist; EOD-dominated, ~3.7h hold."),
            "kept_because": "user explicitly directed promotion to v7 live + v11 on 2026-06-30 despite REJECT.",
            "risk_label": ("DO NOT SIZE UP. Cost-fragile (needs <=5bps/leg to be ~break-even). Monitor live; "
                           "demote if the cost wall holds. Detector default-OFF outside the conf book."),
        },
    },
}
