...
def train_band_ok(m: dict[str, Any]) -> bool:
    return m["trades"] > 0 and m["net_pnl"] > 0 and float(m["net_pf"]) >= 1.30


def approval_ok(train: dict[str, Any], test: dict[str, Any], min_train: int, min_test: int) -> tuple[bool, list[str]]:
    reasons = []
    if train["net_pf"] < 1.30:
        reasons.append("TRAIN PF < 1.30")
    if test["net_pf"] <= 1.40:
...
def write_artifacts(
    ...
    for stage, g in pd.DataFrame(csv_rows).groupby("stage", sort=False):
        tested = len(g)
        train_band = int((pd.to_numeric(g["train_pf"], errors="coerce") >= 1.30).sum())
    ...
    (setup_dir / "PARAMETER_SWEEP_SUMMARY.md").write_text(
        "\n".join(
            [
                ...
                "",
                md_table(sweep_rows, ["Stage", "Iterations", "TRAIN PF >= 1.30", "TEST PF > 1.40"]),
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    if not best.get("test"):
        fail_notes.append("The closest train-side candidate was not evaluated on TEST because full TRAIN PF did not reach 1.30 with positive net.")
    else:
