r"""recreate_pool.py — recreate the B_HUGE_GREEN_PULLBACK_HOLD_THEN_BREAK research pool for the
...
Basis:
RAW candidates from a RESEARCH-MODE rerun of the SAME production scanner
(`avwap_5min_ID_v11_backtesting.py --mode historical_all_available`) with
EQIDV2_SIGNAL_DISCOVERY_V7_SELECTION_MODE=research_catalog so the v8-exit-rule
allowlist (v6.SETUP_EXIT_RULES) does not drop catalog detectors that lack a v6 exit
entry. The detector itself runs UNMODIFIED inside candidate_scan.v2._scan_day —
identical detection logic, features and quality scores as production; only the
post-scan setup-name filter is widened. Scan output shared by both catalog setups at
B_HUGE_FAILED_BOUNCE/pools/_research_scan_catalog_20260301_20260701/.

Output (same filename convention the tuner's load_pool() expects):
  pools/B_HUGE_GREEN_PULLBACK_HOLD_THEN_BREAK/historical_all_available_pre_dedupe_live_candidates.csv
  pools/pool_manifest.json
  pools/sessions_coverage.csv

Run from repo root:
  py -3.12 Train_and_Test\setup_pf_1_4_full_loop\B_HUGE_GREEN_PULLBACK_HOLD_THEN_BREAK\scripts\recreate_pool.py
"""
...
WORK = _HERE.parent                              # .../B_HUGE_GREEN_PULLBACK_HOLD_THEN_BREAK
POOLS = WORK / "pools"
FULL_LOOP = WORK.parent                          # .../setup_pf_1_4_full_loop

ROOT = Path(r"C:\TradingData\eqidv2")
SOURCES = [
    ("research_scan_catalog_20260301_20260701",
     [str(FULL_LOOP / "B_HUGE_FAILED_BOUNCE" / "pools" / "_research_scan_catalog_20260301_20260701" / "historical_all_available_raw_candidates.csv")]),
]
