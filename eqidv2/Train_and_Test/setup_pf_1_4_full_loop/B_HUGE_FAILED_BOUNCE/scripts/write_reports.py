...
B_HUGE_FAILED_BOUNCE from the artifacts produced by recreate_pool.py + run_full_loop.py.

RESEARCH-ONLY. Writes ONLY inside Train_and_Test/setup_pf_1_4_full_loop/B_HUGE_FAILED_BOUNCE/.
...
def main() -> int:
    ...
    pr = [f"# {SETUP} ({SIDE}) — POOL_RECREATION_REPORT", "", HDR, "",
          ...
          "- **5-minute signal generation:** RESEARCH-MODE rerun of the production scanner "
          "(`avwap_5min_ID_v11_backtesting.py --mode historical_all_available`, "
          "EQIDV2_SIGNAL_DISCOVERY_V7_SELECTION_MODE=research_catalog to bypass the v8-exit-rule "
          "allowlist that drops catalog detectors; detector itself UNMODIFIED in "
          "candidate_scan.v2._scan_day) on data root "
    ...
    inv = [f"# {SETUP} ({SIDE}) — PARAMETER_INVENTORY", "", HDR, "",
           ...
           "- **entry trigger (detection, read-only):** catalog detector `huge_candle_failed_bounce`: prev bar range >=1.80x prev ATR (huge bar), prev bar RED (`prev_close<prev_open`), short structure (`close<open`, `close_loc<=0.40`), `close<prev_bar_low`, `close<VWAP`, `vol_ratio>=1.2`",
