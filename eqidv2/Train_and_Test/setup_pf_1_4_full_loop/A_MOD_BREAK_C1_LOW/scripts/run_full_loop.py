r"""run_full_loop.py — A_MOD_BREAK_C1_LOW (SHORT) full PF-band optimization loop.
================================================================================
RESEARCH-ONLY. No live trades, no order placement, no final_setup_conf.py edits.
All artifacts stay under Train_and_Test/setup_pf_1_4_full_loop/A_MOD_BREAK_C1_LOW/.

Campaign windows (user-mandated):
  TRAIN = completed sessions 2026-03-01 .. 2026-05-30
  TEST  = completed sessions 2026-06-01 .. 2026-07-02 (Jul 2 drops out if 1-min
          exit data for it is incomplete — the loop reports requested vs actual)
...
Staged protocol (anti-overfit):
  Stage 1  baseline  — current conf config on FIT/VAL/TRAIN/TEST
  Stage 2  failure study — classify baseline TRAIN losers
  Stage 3  sweeps    — one-knob-at-a-time sweeps from baseline, scored on FIT+VAL
  Stage 4  search    — Optuna TPE (fallback: seeded random) combination search on
                       FIT/VAL band objective; NEVER sees TEST
  Stage 5  finalize  — top distinct configs confirmed on full TRAIN; TEST scored
                       ONCE per finalist whose TRAIN lands in the band
  Stage 6  rescue    — if nothing passes: premom-off re-search, term
                       simplification, time-window restriction (TEST budget-capped)
...
Run from repo root:
  py -3.12 Train_and_Test\setup_pf_1_4_full_loop\A_MOD_BREAK_C1_LOW\scripts\run_full_loop.py ^
...
# populated numeric mask features for this setup's RAW pool (verified: everything
# else in the wide schema is empty for A_MOD rows)
MASK_FEATS = ["rs_pct", "vol_ratio", "atr_pct", "body_pct", "close_loc", "vwap_dist_atr",
              ...
              "wick_skew_pct"]
REGIME_TERMS = [("regime", "==", "BEAR"), ("regime", "!=", "BEAR"),
                ("regime", "==", "NEUTRAL"), ("regime", "!=", "TREND")]
...
def main() -> int:
    ...
    def sweep(group, knob, old, new, cfg):
        ...
        except Exception as e:   # never let one bad variant kill the campaign
            sweep_rows.append({"group": group, "knob": knob, "old": old, "new": new,
                               "fit_n": None, "fit_pf": None, "val_n": None, "val_pf": None,
                               "score": None, "vs_baseline": f"error:{type(e).__name__}"})
            return -9.0
    ...
    for fi, cand in enumerate(finalists, 1):
        ...
        try:
            ...
            print(f"[stage5] finalist {fi} errored: {type(e).__name__}: {e}")
