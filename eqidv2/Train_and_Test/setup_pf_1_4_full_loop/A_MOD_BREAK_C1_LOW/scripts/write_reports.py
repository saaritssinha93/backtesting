...
A_MOD_BREAK_C1_LOW from the artifacts produced by recreate_pool.py + run_full_loop.py.

RESEARCH-ONLY. Writes ONLY inside Train_and_Test/setup_pf_1_4_full_loop/A_MOD_BREAK_C1_LOW/.
...
def main() -> int:
    ...
    pr += ["", "## Data quality notes", "",
           "- The pool is RAW candidates (pre-gate) — the same basis `build_unified_pool.py` uses for this "
           "mined-short readmit setup, so the campaign tunes the same object that runs in v11/live.",
    ...
    inv = [f"# {SETUP} ({SIDE}) — PARAMETER_INVENTORY", "", HDR, "",
           ...
           "- **entry trigger (detection, read-only):** 5-min bar breaks the prior C1 (first-candle) LOW "
           "with a moderate impulse — reason tag `moderate_impulse_break_prior_low`, produced by the "
           "production scanner (`_scan_day`); momentum continuation DOWN out of a tight pre-break range.",
