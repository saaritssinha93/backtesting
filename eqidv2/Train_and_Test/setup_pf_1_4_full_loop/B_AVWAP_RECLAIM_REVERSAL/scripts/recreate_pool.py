r"""recreate_pool.py — recreate the B_AVWAP_RECLAIM_REVERSAL research pool for the
...
Output (same filename convention the tuner's load_pool() expects):
  pools/B_AVWAP_RECLAIM_REVERSAL/historical_all_available_pre_dedupe_live_candidates.csv
  pools/pool_manifest.json
  pools/sessions_coverage.csv

Run from repo root:
  py -3.12 Train_and_Test\setup_pf_1_4_full_loop\B_AVWAP_RECLAIM_REVERSAL\scripts\recreate_pool.py
"""
...
WORK = _HERE.parent                              # .../B_AVWAP_RECLAIM_REVERSAL
POOLS = WORK / "pools"
