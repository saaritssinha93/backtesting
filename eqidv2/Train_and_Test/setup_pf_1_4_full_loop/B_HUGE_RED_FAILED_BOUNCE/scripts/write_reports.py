...
B_HUGE_RED_FAILED_BOUNCE from the artifacts produced by recreate_pool.py + run_full_loop.py.

RESEARCH-ONLY. Writes ONLY inside Train_and_Test/setup_pf_1_4_full_loop/B_HUGE_RED_FAILED_BOUNCE/.
...
Run from repo root:
  py -3.12 Train_and_Test\setup_pf_1_4_full_loop\B_HUGE_RED_FAILED_BOUNCE\scripts\write_reports.py
"""
...
def main() -> int:
    ...
    inv = [f"# {SETUP} ({SIDE}) — PARAMETER_INVENTORY", "", HDR, "",
           ...
           "- **entry trigger (detection, read-only):** after a huge RED bar, price bounces weakly and fails -> resume down: huge prev red bar + short structure + breakdown of prior bar low below VWAP — reason tag `huge_red_failed_bounce_breakdown`",
