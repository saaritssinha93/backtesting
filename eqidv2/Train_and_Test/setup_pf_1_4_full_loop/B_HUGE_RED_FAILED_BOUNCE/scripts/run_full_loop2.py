r"""run_full_loop.py — B_HUGE_RED_FAILED_BOUNCE (SHORT) full PF-band optimization loop — ROUND 2 (enriched features).
================================================================================
RESEARCH-ONLY. No live trades, no order placement, no final_setup_conf.py edits.
All artifacts stay under Train_and_Test/setup_pf_1_4_full_loop/B_HUGE_RED_FAILED_BOUNCE/.
...
Run from repo root:
  py -3.12 Train_and_Test\setup_pf_1_4_full_loop\B_HUGE_RED_FAILED_BOUNCE\scripts\run_full_loop.py ^
...
def main() -> int:
    ...
    ap.add_argument("--time_budget_min", type=float, default=75.0)
    ...
    print(f"[full-loop][ROUND2] {SETUP} ({SIDE}) — band [{PF_LO},{PF_HI}], TEST>{TEST_PF_MIN}")
    ...
    for lbl in ("TRAIN", "TEST"):
        ...
        if det is not None and not det.empty:
            det.to_csv(OUT / f"baseline_trades_{lbl.lower()}.csv", index=False)
    ...
    if not args.skip_sweeps:
        ...
        pd.DataFrame(sweep_rows).to_csv(OUT / "sweeps.csv", index=False)
    ...
    def _suggest_cfg(trial):
        ...
        rg = trial.suggest_categorical("regime_term", ["none", "!=BULL", "==NEUTRAL", "!=TREND", "==BEAR"])
        if rg != "none":
            op, val = rg[:2], rg[2:]
    ...
    tdf.to_csv(OUT / "trials.csv", index=False)
    ...
    for fi, cand in enumerate(finalists, 1):
        ...
        if in_band and test_evals < args.max_test_evals:
            ...
            det = mTE["detail"]
            if det is not None and not det.empty:
                det.to_csv(OUT / f"finalist_{fi:02d}_trades_test.csv", index=False)
            mTR["detail"].to_csv(OUT / f"finalist_{fi:02d}_trades_train.csv", index=False)
        else:
    ...
    # ---------------- persist everything ------------------------------------------
    pd.DataFrame(iter_rows).to_csv(OUT / "iteration_log.csv", index=False)
    summary = {
    ...
    (OUT / "run_summary.json").write_text(json.dumps(tt._json_sanitize(summary), indent=2, default=str),
                                           encoding="utf-8")
