"""
AVWAP ID 5-min v12 experimental live-strategy backtester.

V12 is an isolated research copy of V11. It keeps the same candidate source and
...
Default output:
  C:\\TradingData\\eqidv2_experiments\\outputs_ID_v12_5min
...
# V12 owns a separate setup book. Importing this research backtester must not
# silently fall back to the V7/V11 production configuration.
V12_FINAL_SETUP_CONF_MODULE_ENV = "EQIDV2_V12_FINAL_SETUP_CONF_MODULE"
V12_FINAL_SETUP_CONF_MODULE = os.getenv(
    V12_FINAL_SETUP_CONF_MODULE_ENV,
    "final_setup_conf_v12",
).strip() or "final_setup_conf_v12"
_V12_PREVIOUS_SHARED_CONF_TARGET = os.environ.get("EQIDV2_FINAL_SETUP_CONF_MODULE")
os.environ["EQIDV2_FINAL_SETUP_CONF_MODULE"] = V12_FINAL_SETUP_CONF_MODULE
...
# The temporary selector above lets imported V7 helpers initialize against the
# isolated V12 book when conf mode is active, but importing this research module
# must not change the process-wide V7/V11 setup-book selection afterwards.
if _V12_PREVIOUS_SHARED_CONF_TARGET is None:
    os.environ.pop("EQIDV2_FINAL_SETUP_CONF_MODULE", None)
else:
    os.environ["EQIDV2_FINAL_SETUP_CONF_MODULE"] = _V12_PREVIOUS_SHARED_CONF_TARGET
del _V12_PREVIOUS_SHARED_CONF_TARGET


OUT_ROOT = Path(r"C:\TradingData\eqidv2_experiments\outputs_ID_v12_5min")
...
def _first_1m_entry(
    ...
    *,
    ...
    if not_before > latest_allowed:
        return None
    # Historical V12 bars are normalized and monotonically sorted.  Binary
    # search is exactly equivalent to materializing a full-frame boolean mask,
    # but avoids an O(all two-month bars) scan for every candidate.
    if bars.index.is_monotonic_increasing:
        position = int(bars.index.searchsorted(not_before, side="left"))
        if position >= len(bars):
            return None
        entry_ts = pd.Timestamp(bars.index[position])
        if entry_ts > latest_allowed:
            return None
        entry_px = float(bars.iloc[position]["open"])
    else:
...
@lru_cache(maxsize=1)
def _historical_nifty_daily_closes() -> pd.DataFrame:
    """Return completed NIFTY sessions used by the V12 sizing regime.

    This cache is V12-only.  The shared V7 helper intentionally remains
    untouched because changing it would alter the live/V11 production path.
    """
    frame = pd.read_parquet(NIFTY_5MIN_PARQUET, columns=["date", "close"])
    timestamps = pd.to_datetime(frame["date"], errors="coerce")
    valid = timestamps.notna() & pd.to_numeric(frame["close"], errors="coerce").notna()
    frame = frame.loc[valid, ["close"]].copy()
    frame["date"] = timestamps.loc[valid]
    daily = (
    ...
    return daily.dropna(subset=["trade_date", "close"]).sort_values(
        "trade_date"
...
def _historical_nifty_short_mult(trade_day: str) -> float:
    """Size an intraday short using only sessions completed before trade_day.

    Using the last 5-minute row *on* ``trade_day`` would expose a morning entry
    to that session's eventual close.  The strict ``< trade_day`` boundary is
    therefore part of the V12 research contract.
    """
    if not NIFTY_REGIME_GATE_ENABLED:
        return 1.0
    try:
        daily = _historical_nifty_daily_closes()
        if daily.empty:
            return 1.0
        day = pd.Timestamp(trade_day)
        trade_dates = daily["trade_date"]
        timezone = getattr(trade_dates.dt, "tz", None)
        if timezone is not None and day.tzinfo is None:
            day = day.tz_localize(timezone)
        elif timezone is None and day.tzinfo is not None:
            day = day.tz_localize(None)
        elif timezone is not None and day.tzinfo is not None:
            day = day.tz_convert(timezone)
        completed = daily.loc[trade_dates < day.normalize()].tail(
            int(NIFTY_MA_DAYS) + 3
        ).copy()
        if len(completed) < int(NIFTY_MA_DAYS) + 1:
            return 1.0
        completed["ma"] = completed["close"].rolling(int(NIFTY_MA_DAYS)).mean()
        ...
        return float(NIFTY_REGIME_SHORT_SIZE_MULT) if bullish else 1.0
    except Exception:
        return 1.0
...
def _selected_strategy_features(signals: pd.DataFrame) -> pd.DataFrame:
    ...
    # D_EMA20_REJECTION's approved mask uses ``ema20_dist_atr`` but scanner
    # candidate rows historically did not serialize EMA20/ATR.  Materialize the
    # causal completed-signal value from the historical 5-minute indicator file
    # when it is absent; otherwise the mask evaluates NaN and can never pass.
    if "ema20_dist_atr" not in work.columns:
        work["ema20_dist_atr"] = np.nan
    missing_ema = (
        work.get("setup", pd.Series("", index=work.index)).astype(str).eq("D_EMA20_REJECTION")
        & _selected_strategy_numeric(work, "ema20_dist_atr").isna()
    )
    ema_col_pos = work.columns.get_loc("ema20_dist_atr")
    for pos in np.flatnonzero(missing_ema.to_numpy(dtype=bool)):
        row = work.iloc[int(pos)]
        ticker = str(row.get("ticker", "")).upper().strip()
        signal_ts = signal_times[int(pos)]
        bars = _load_5m_ind_bars(ticker) if ticker and pd.notna(signal_ts) else None
        if bars is None or bars.empty or not {"date", "EMA_20", "ATR"}.issubset(bars.columns):
            continue
        same_day = bars.loc[
            (bars["date"].dt.date == signal_ts.date()) & (bars["date"] <= signal_ts)
        ].tail(1)
        if same_day.empty:
            continue
        bar = same_day.iloc[-1]
        close_value = _safe_float(row.get("signal_close", bar.get("close")))
        ema20_value = _safe_float(bar.get("EMA_20"))
        atr_value = _safe_float(bar.get("ATR"))
        if np.isfinite(close_value) and np.isfinite(ema20_value) and np.isfinite(atr_value) and atr_value > 0:
            work.iat[int(pos), ema_col_pos] = float((close_value - ema20_value) / atr_value)
    return work
...
def _load_final_setup_conf_module():
    """Load V12's isolated setup book without mutating the V7/V11 selector."""
    target = setup_conf_loader.configured_target(
        env_names=(V12_FINAL_SETUP_CONF_MODULE_ENV,),
        default=V12_FINAL_SETUP_CONF_MODULE,
    )
    try:
        ...
        raise SystemExit(
            "[v12 final_setup_conf] failed to load "
            f"{V12_FINAL_SETUP_CONF_MODULE_ENV}={target!r}: {exc}"
        ) from exc

    print(
        f"[v12 final_setup_conf] config module={target} file={getattr(module, '__file__', 'unknown')}",
        flush=True,
    )
    return module
...
def _final_setup_conf_mask(signals: pd.DataFrame) -> pd.Series:
    ...
    for name, cfg in _fc.FINAL_SETUP_CONF.items():
        ...
        # The approved-book schema has always carried a per-slot Top-N guard.
        # Apply it here, before the V7 ticker/day de-duplication, using the same
        # vwap-distance ranking as the train/test research engine.  The old V12
        # path silently ignored this key.
...
def _capture_v11_worker_scan_state() -> dict[str, object]:
    """Return the scanner switches that spawned Windows workers must inherit.

    ``_activate_final_setup_conf`` mutates these module globals in the parent
    process.  Windows ``spawn`` imports a fresh module in every worker, so the
    mutations are otherwise lost and the child scans a different setup book.
    Keep the payload deliberately small and pickle-safe.
    """
    return {
        "allowed_setups": tuple(sorted(str(x) for x in candidate_scan.ALLOWED_SETUPS)),
        "filter_to_v8_exit_setups": bool(candidate_scan.FILTER_TO_V8_EXIT_SETUPS),
        "enable_s9_midday_lose": bool(candidate_scan.v2.ENABLE_S9_MIDDAY_LOSE),
        "enable_doc5d_avwap_reclaim": bool(candidate_scan.v2.ENABLE_DOC5D_AVWAP_RECLAIM),
    }


def _apply_v11_worker_scan_state(scan_state: dict[str, object] | None) -> None:
    """Apply the parent scanner state inside a historical-scan worker."""
    if not scan_state:
        return
    candidate_scan.ALLOWED_SETUPS = frozenset(
        str(x) for x in scan_state.get("allowed_setups", ())
    )
    candidate_scan.FILTER_TO_V8_EXIT_SETUPS = bool(
        scan_state.get("filter_to_v8_exit_setups", False)
    )
    candidate_scan.v2.ENABLE_S9_MIDDAY_LOSE = bool(
        scan_state.get("enable_s9_midday_lose", False)
    )
    candidate_scan.v2.ENABLE_DOC5D_AVWAP_RECLAIM = bool(
        scan_state.get("enable_doc5d_avwap_reclaim", False)
    )


def _v11_worker_init(
    data_root: str = "",
    ...
    _apply_v11_worker_scan_state(scan_state)
    if _V11_WORKER_MARKET_CTX is None:
        ...
        if scan_state:
            worker_overrides.update({
                "ENABLE_S9_MIDDAY_LOSE": bool(
                    scan_state.get("enable_s9_midday_lose", False)
                ),
                "ENABLE_DOC5D_AVWAP_RECLAIM": bool(
                    scan_state.get("enable_doc5d_avwap_reclaim", False)
                ),
            })
        candidate_scan.v2._init_worker(worker_overrides)
        try:
...
def _trim_historical_scan_input(df: pd.DataFrame, requested_days: tuple[str, ...]) -> pd.DataFrame:
    """Keep requested sessions plus one prior session before feature preparation.

    Historical indicator parquet files contain many months.  Preparing all of
    them for every replayed date is wasteful.  Scanner features reset by
    session; one prior session is retained for previous-day levels.  Stored
    ATR/EMA/RSI/ADX values remain the point-in-time values from the source.
    """
    if df is None or df.empty or "date" not in df.columns or not requested_days:
        return df
    dates = _normalise_date_series(df["date"])
    requested = sorted({pd.Timestamp(day).date() for day in requested_days})
    requested_set = set(requested)
    available_prior = sorted({value for value in dates.dt.date.dropna().unique() if value < requested[0]})
    keep_days = requested_set | ({available_prior[-1]} if available_prior else set())
    out = df.loc[dates.dt.date.isin(keep_days)].copy()
    out["date"] = dates.loc[out.index]
    out["date_only"] = out["date"].dt.date
    return out.reset_index(drop=True)


_V12_HISTORICAL_5M_COLUMNS = (
    "date", "open", "high", "low", "close", "volume",
    "gap_filled", "opening_snapshot", "source_1m_count",
...
def _load_historical_5m_window(ticker: str, requested_days: tuple[str, ...]) -> pd.DataFrame | None:
    """Read only the Parquet date window needed by an isolated V12 replay."""
    if not requested_days:
        return None
    path = candidate_scan.LIVE_5M_DIR / f"{str(ticker).upper()}_stocks_indicators_5min.parquet"
    if not path.exists():
        return None
    days = sorted(pd.Timestamp(day).date() for day in requested_days)
    # Fifteen calendar days safely captures the immediately preceding NSE
    # session across weekends and ordinary holiday clusters.
    start = pd.Timestamp(days[0], tz="Asia/Kolkata") - pd.Timedelta(days=15)
    end = pd.Timestamp(days[-1], tz="Asia/Kolkata") + pd.Timedelta(days=1)
    filters = [("date", ">=", start), ("date", "<", end)]
    try:
        frame = pd.read_parquet(path, columns=list(_V12_HISTORICAL_5M_COLUMNS), filters=filters)
    except Exception:
        try:
    ...
    if frame is None or frame.empty or "date" not in frame.columns:
        return None
    frame = frame.copy()
    frame["date"] = _normalise_date_series(frame["date"])
    ...
    return _trim_historical_scan_input(frame, requested_days)
...
def _scan_one_ticker_day_candidates(payload: tuple) -> list[dict]:
    ...
    try:
        df = _load_historical_5m_window(ticker, (str(day),))
...
def _scan_historical_full_day_candidates(
    *,
    ...
    worker_scan_state = _capture_v11_worker_scan_state()
    rows: list[dict] = []
    if worker_count <= 1:
        _v11_worker_init(str(data_root), worker_scan_state)
        ...
        with ProcessPoolExecutor(
            ...
            initializer=_v11_worker_init,
            initargs=(str(data_root), worker_scan_state),
        ) as ex:
    ...
    if not raw.empty and "setup" in raw.columns:
        ...
        if custom_mask.any():
            ...
            alignment = candidate_scan.late_bb10.market_alignment_for_slots(
                tickers,
                custom_slots,
                lambda ticker: _load_historical_5m_window(ticker, (str(day),)),
            )
...
def _run_historical_all_available(args: argparse.Namespace) -> int:
    ...
    for i, day in enumerate(dates, 1):
        ...
        if bool(getattr(args, "candidate_pool_only", False)):
            selected_strategy_signals = pd.DataFrame()
            entry_engine_raw = pd.DataFrame()
            entry_engine_rejects = pd.DataFrame()
            entry_engine_signals = pd.DataFrame()
            selected_strategy_rejects = pd.DataFrame()
            selected_strategy_stats = {
                "selected_strategy_profile": str(args.selected_strategy_profile),
                "selected_strategy_input_signals": 0,
                "selected_strategy_preselect_signals": 0,
                "selected_strategy_signals": 0,
                "selected_strategy_rejects": 0,
    ...
    inputs_text = (
        ...
        "selected_strategy=post-v7-entry honest production profile filters; pass --selected_strategy_profile none for raw v11\n"
        "entry_engine_dedupe=best setup per ticker/slot, then one ticker per day matching v7 entry-engine guard\n"
        f"candidate_pool_only={bool(getattr(args, 'candidate_pool_only', False))}\n"
        f"pnl_model={_pnl_model_description()}\n"
    )

    if bool(getattr(args, "candidate_pool_only", False)):
        pool_summary = {
            "mode": "historical_all_available_candidate_pool_only",
            "dates": int(len(dates)),
            "first_date": dates[0],
            "last_date": dates[-1],
            "raw_candidates": int(len(raw_all)),
            "pre_dedupe_live_candidates": int(len(pre_dedupe_all)),
            "production_consumption_allowed": False,
        }
        (out_dir / "candidate_pool_summary.json").write_text(
            json.dumps(pool_summary, indent=2, sort_keys=True), encoding="utf-8"
        )
        (out_dir / "inputs.txt").write_text(inputs_text, encoding="utf-8")
        print(
            f"[v11 all_available] candidate-pool-only wrote {len(pre_dedupe_all):,} "
            f"pre-dedupe rows to {out_dir}",
            flush=True,
        )
        return 0
...
def _write_empty_outputs(
    ...
    *,
    ...
    text = "\n".join([
        "=" * 100,
        "AVWAP ID 5-min v12 experimental backtest",
...
def _write_parity_debug_csv(
    ...
    """Write v12_ID_parity_debug.csv — per-candidate audit trail across all pipeline stages.
    ...
    fname = f"v12_ID_parity_debug{'_' + label if label else ''}.csv"
...
def _write_outputs(
    ...
    _build_v11_id_trades(trades).to_csv(out_dir / "v12_ID_trades.csv", index=False)
    daily_out.to_csv(out_dir / "v12_ID_daily_summary.csv", index=False)
    by_setup.to_csv(out_dir / "v12_ID_setup_summary.csv", index=False)

    text = v6._summary_text(summary, by_setup, Path(source))
    text = text.replace("AVWAP ID 5-min v6 backtest", "AVWAP ID 5-min v12 experimental backtest")
    text = text.replace(
    ...
    print(text)
    (out_dir / "summary.txt").write_text(text + "\n", encoding="utf-8")
    (out_dir / "v12_ID_summary.csv").write_text(text + "\n", encoding="utf-8")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="AVWAP ID 5-min v12 isolated experimental strategy backtest")
    ap.add_argument(
        "--mode",
        choices=[
            "live_parity",
    ...
    ap.add_argument(
        "--candidate_pool_only",
        action="store_true",
        help="stop after writing scanner/live-gate candidate pools; skip entry and exit resolution",
    )
    ...
    print(f"[v12] frozen_runtime_manifest={manifest_path}", flush=True)
