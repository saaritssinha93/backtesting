...
**ONLY for intraday 1-hour timeframe** (Parquet storage).
...
Outputs (Parquet):
- stocks_indicators_1hr_eq / <TICKER>_stocks_indicators_1hr.parquet

Usage examples:
- Fetch 1-hour:
    python trading_data_continous_run_historical_alltf_v3_parquet_stocksonly_1hr.py 1hr
- Same (all aliases to 1hr):
    python trading_data_continous_run_historical_alltf_v3_parquet_stocksonly_1hr.py all

Notes:
- Intraday timestamps can be stored as candle "end" (recommended) or "start".
- Fixed historical window start: 2025-08-25 (IST), until latest completed 1-hour candle.
...
# Directories (only intraday 1h)
DIRS = {
    "1hr": {"cache": "stocks_cache_1hr_eq", "out": "stocks_indicators_1hr_eq"},
}
...
def setup_logger() -> logging.Logger:
    ...
    fh = logging.FileHandler("stocks_fetcher_1hr_run.log", mode="w", encoding="utf-8")
    fh.setFormatter(fmt)
...
def fetch_historical_1hr_df(kite, token, start_dt_ist, end_dt_ist, logger, intraday_ts):
...
def _incremental_start_from_existing(mode: str, out_path: str, default_start: datetime) -> datetime:
    ...
    if mode == "1hr":
        back = timedelta(minutes=60 * warm)
    else:
...
def process_ticker(
    ...
    try:
        ...
        if mode == "1hr":
            fetched = fetch_historical_1hr_df(kite_client, token, inc_start, end_dt_ist, logger, intraday_ts)
        else:
...
def parse_args():
    ...
    p.add_argument("mode", nargs="?", default="1hr", help="1hr|all")
    ...
    p.add_argument("--report-dir", default="reports/stocks_missing_reports_1hr",
