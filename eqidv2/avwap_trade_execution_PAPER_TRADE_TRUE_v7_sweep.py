...
Usage:
    python avwap_trade_execution_PAPER_TRADE_TRUE_v7_sweep.py
    python avwap_trade_execution_PAPER_TRADE_TRUE_v7_sweep.py --no-ltp
    python avwap_trade_execution_PAPER_TRADE_TRUE_v7_sweep.py --capital 500000
"""
...
EXECUTED_SIGNALS_FILE = os.path.join(SIGNAL_DIR, "executed_signals_paper_v7_sweep.json")
SUMMARY_FILE = os.path.join(SIGNAL_DIR, "paper_trade_summary_v7_sweep.json")
OPEN_TRADES_STATE_PATTERN = "open_trades_state_{}_v7_sweep.json"
KILL_SWITCH_COMMAND_FILE = os.path.join(SIGNAL_DIR, "kill_switch_true_v7_sweep.json")
...
MAX_CONCURRENT_TRADES = int(os.getenv("EQIDV2_PAPER_V7_SWEEP_MAX_CONCURRENT_TRADES", "0"))
...
LONG_TARGET_PCT = float(os.getenv("EQIDV7_SWEEP_LONG_TARGET_PCT", "0.011"))    # 1.10%
...
RISK_LIMITS_ENABLED = str(os.getenv("EQIDV2_PAPER_V7_SWEEP_ENABLE_RISK_LIMITS", "0")).strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
}
MAX_OPEN_POSITIONS = int(os.getenv("EQIDV2_PAPER_V7_SWEEP_MAX_OPEN_POSITIONS", "999"))
MAX_CAPITAL_DEPLOYED_RS = float(os.getenv("EQIDV2_PAPER_V7_SWEEP_MAX_CAPITAL_DEPLOYED_RS", "500000"))
...
def _normalize_signal(raw: dict) -> dict:
    ...
    # Enforce V7_SWEEP target policy from executor side as well:
...
def read_signals_csv_multi(csv_paths: Sequence[str]) -> List[dict]:
    """
    Read and merge signals from multiple CSV files (v7_sweep_short + v7_sweep_long).
...
def _restore_intraday_runtime_state(
    ...
    # Build today's signal lookup by signal_id from both v7_sweep_short + v7_sweep_long CSVs
...
def main():
    ...
    # Resolve today's signal CSVs (v7_sweep_short + v7_sweep_long)
    today_str = datetime.now(IST).strftime("%Y-%m-%d")
    ...
    def on_csv_change():
        nonlocal executed
        log.info("Signal CSV changed - processing new signals from v7_sweep_short + v7_sweep_long...")
