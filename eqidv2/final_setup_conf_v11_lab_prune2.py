...
Use this file for V11 backtesting experiments only. The live/paper v7 path
continues to import the root final_setup_conf.py unless that is changed
separately.

Lab policy 2026-07-10:
  - Start from the v11_rnd research copy, not the live root file.
  - Park recent long-side damage first; do not touch V7 live/paper wiring.
  - Any parked setup is moved to RESEARCH_WATCH_CONF with enabled=False so it
    remains visible and reversible.
  - Prune2 additionally parks weak recent pullback shorts and tightens only
    existing active rules that showed clean loss pockets in the wide lab run.
...
FINAL_SETUP_CONF = {
    ...
    "C_OR_BREAKDOWN": {
        ...
        "mask_terms": [
            ["signal_minute", "<=", 690],  # R&D 2026-07-08: 11:30 IST entry cap (kills afternoon bleed; IS+OOS validated)
            ["body_pct", "<=", 0.70],      # V11 lab prune2: avoid large-body breakdown chase bars.
        ],
        "pre_momentum_terms": [
...
# ===========================================================================
# 2026-07-08 R&D DEMOTION (config-only, REVERSIBLE) — last-2-weeks v11 study.
# Baseline conf book (11 setups) was a heavy net loser at realistic statutory+5bps
# costs on BOTH the tuning window (2026-06-23..07-07: -Rs19,823, PF 0.70) and an
# out-of-sample window (2026-05-26..06-20: -Rs11,897, PF 0.92) — death-by-cost at
# 14-16 trades/day. Two robust, OOS-validated fixes were applied:
#   (1) an 11:30 IST entry cap on the 6 momentum setups (edited into mask_terms above)
#       — the afternoon book was the entire structural loss on both windows; and
#   (2) demoting the 4 setups below.
# Result: IS +Rs6,127 (PF 1.60) / OOS +Rs8,338 (PF 1.35), ~2.5 trades/day, both windows
# day-spread (5/9 and 9/15 positive days). The surviving edge is concentrated in
# E_ORB_BREAKOUT_LONG (IS +2,564 / OOS +12,633); the rest is de-risked to marginal.
# To REVERSE: delete this block AND the "R&D 2026-07-08" cap edits above.
# ===========================================================================
# USER DIRECTION 2026-07-08: DOC5D/S9/D_EMA20 kept ACTIVE ("add irrespectively") despite documented
# loser/reject/failed-gate provenance (each fired 0 trades in both study windows -> validated PnL
# unchanged). Only G_HIGHER_HIGH_BREAK is parked (INACTIVE but present in RESEARCH_WATCH_CONF).
_RND_DEMOTION_2026_07_08 = {
    # Unstable/noise: loses -Rs3,502 IS but wins +Rs6,263 OOS (sign-flips across windows);
    # config's own provenance flags it FAILED the selection gate (dominance-cap breach).
    "G_HIGHER_HIGH_BREAK": "unstable sign-flip P&L + documented failed-gate force-promote; user-parked 2026-07-08",
}
for _name, _why in _RND_DEMOTION_2026_07_08.items():
    _cfg = FINAL_SETUP_CONF.pop(_name, None)
    if _cfg is None:
        continue
    _cfg = dict(_cfg); _cfg["enabled"] = False
    _prov = dict(_cfg.get("provenance", {}))
    _prov["rnd_demotion_2026_07_08"] = {"reason": _why,
        "re_validation_trigger": "must show positive net at statutory+5bps on a fresh IS+OOS pair before re-promotion"}
    _cfg["provenance"] = _prov
    RESEARCH_WATCH_CONF[_name] = _cfg


# ===========================================================================
# 2026-07-10 V11 LAB PRUNE (config-only, V11-LAB-ONLY, REVERSIBLE).
#
# First "do less, lose less" test requested after the 2026-07-03..2026-07-09
# baseline. The recent live-parity V11 working book lost Rs -6,865 over the last
# 5 completed sessions, with the damage concentrated in:
#   - L_DOUBLE_BOTTOM_VWAP: 21 trades / Rs -9,411
#   - G_HIGHER_HIGH_BREAK: 5 trades / Rs -5,186
#
# G_HIGHER_HIGH_BREAK is already parked by the 2026-07-08 R&D demotion in this
# lab source; it remains listed here as a guard against accidental reactivation.
# L_DOUBLE_BOTTOM_VWAP is parked for this lab run so the next baseline tests the
# surviving short-heavy book without that long-side drag.
# ===========================================================================
_V11_LAB_DEMOTION_2026_07_10 = {
    "L_DOUBLE_BOTTOM_VWAP": {
        "reason": "last-5-session V11 live-parity long-side drag; park before retuning",
        "evidence": {
            "window": "2026-07-03..2026-07-09",
            "trades": 21,
            "net_rs": -9411,
            "diagnosis": "recent long reclaim/reversal behavior not surviving current tape",
        },
    },
    "G_HIGHER_HIGH_BREAK": {
        "reason": "guardrail: keep documented failed-gate/unstable long breakout parked in lab",
        "evidence": {
            "window": "2026-07-03..2026-07-09",
            "trades": 5,
            "net_rs": -5186,
            "diagnosis": "recent higher-high breakouts are stale/chase-prone and negative",
        },
    },
}
for _name, _lab in _V11_LAB_DEMOTION_2026_07_10.items():
    _cfg = FINAL_SETUP_CONF.pop(_name, None)
    if _cfg is None:
        continue
    _cfg = dict(_cfg)
    _cfg["enabled"] = False
    _prov = dict(_cfg.get("provenance", {}))
    _prov["v11_lab_demotion_2026_07_10"] = {
        "reason": _lab["reason"],
        "evidence": _lab["evidence"],
        "re_validation_trigger": (
            "fresh V11 lab run must show positive net after costs across both a recent "
            "last-5-session holdout and a wider historical window before reactivation"
        ),
    }
    _cfg["provenance"] = _prov
    RESEARCH_WATCH_CONF[_name] = _cfg
...
# ===========================================================================
# 2026-07-10 V11 LAB PRUNE2 (config-only, V11-LAB-ONLY, REVERSIBLE).
#
# Second refine pass after the 2026-05-21..2026-07-09 wide live-parity lab run.
# This is intentionally conservative: remove one recent loser and tighten two
# existing active masks where the failed trades formed simple, explainable loss
# pockets. This file is for V11 backtesting experiments only.
#
# Direct edits above:
#   - C_OR_BREAKDOWN: add body_pct <= 0.70 to avoid large-body chase bars.
#   - E_ORB_BREAKOUT_LONG: tighten vwap_dist_atr <= 0.90 to avoid extended ORB
#     entries. This removes the July 8 extension losers in the prior lab sample.
# ===========================================================================
_V11_LAB_PRUNE2_DEMOTION_2026_07_10 = {
    "A_PULLBACK_C2_THEN_BREAK_C2_LOW": {
        "reason": "wide lab run showed weak recent contribution; park before adding new risk",
        "evidence": {
            "window": "2026-05-21..2026-07-09 live-parity snapshots",
            "trades": 4,
            "win_rate_pct": 25.0,
            "net_rs": -2094.25,
            "diagnosis": "not enough current edge after costs; setup was negative while stronger shorts carried the book",
        },
    },
}
for _name, _lab in _V11_LAB_PRUNE2_DEMOTION_2026_07_10.items():
    _cfg = FINAL_SETUP_CONF.pop(_name, None)
    if _cfg is None:
        continue
    _cfg = dict(_cfg)
    _cfg["enabled"] = False
    _prov = dict(_cfg.get("provenance", {}))
    _prov["v11_lab_prune2_demotion_2026_07_10"] = {
        "reason": _lab["reason"],
        "evidence": _lab["evidence"],
        "re_validation_trigger": (
            "fresh V11 lab run must show positive net after costs on both last-5 "
            "and wide live-parity windows before reactivation"
        ),
    }
    _cfg["provenance"] = _prov
    RESEARCH_WATCH_CONF[_name] = _cfg

_V11_LAB_PRUNE2_RULE_EDITS_2026_07_10 = {
    "C_OR_BREAKDOWN": {
        "change": "add mask body_pct <= 0.70",
        "evidence": (
            "prior wide lab sample kept all observed C_OR winners while removing "
            "most high-body_pct C_OR losers"
        ),
    },
    "E_ORB_BREAKOUT_LONG": {
        "change": "tighten vwap_dist_atr <= 0.90 from <= 0.979716",
        "evidence": (
            "prior wide lab sample kept observed winners and removed July 8 "
            "extended ORB losers"
        ),
    },
}
for _name, _edit in _V11_LAB_PRUNE2_RULE_EDITS_2026_07_10.items():
    if _name not in FINAL_SETUP_CONF:
        continue
    _cfg = dict(FINAL_SETUP_CONF[_name])
    _prov = dict(_cfg.get("provenance", {}))
    _prov["v11_lab_prune2_rule_edit_2026_07_10"] = {
        "change": _edit["change"],
        "evidence": _edit["evidence"],
        "re_validation_requirement": (
            "must improve or preserve PF, net P&L, drawdown, and day spread in "
            "the rerun; otherwise revert the direct edit above"
        ),
    }
    _cfg["provenance"] = _prov
    FINAL_SETUP_CONF[_name] = _cfg
