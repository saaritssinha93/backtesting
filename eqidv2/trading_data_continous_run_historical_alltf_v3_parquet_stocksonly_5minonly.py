...
âœ… ONLY for intraday 5-minute timeframe (Parquet storage)

This is a streamlined, faster, and robust rewrite of the "all timeframes" fetcher.
It intentionally removes ALL other timeframes (15m/1h/3h/daily/weekly).
...
Outputs (Parquet):
- runtime 5m folder / <TICKER>_stocks_indicators_5min.parquet

Usage:
    python trading_data_continous_run_historical_alltf_v3_parquet_stocksonly_5minonly.py
Options:
    --max-workers 8
...
# 5m * 400 = 6000 minutes â‰ˆ 100 hours of intraday data (good for stable indicators).
WARMUP_BARS_5M = 400
...
def setup_logger() -> logging.Logger:
    logger = logging.getLogger("stocks_fetcher_5m")
    ...
    fh = logging.FileHandler("stocks_fetcher_5m_run.log", mode="w", encoding="utf-8")
    fh.setFormatter(fmt)
...
def _intraday_end_shift_minutes(interval: str) -> int:
    return 5 if interval == "5minute" else 0
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if attempt == max_retries:
                    logger.warning("5m chunk fetch failed %s â†’ %s: %s", s, e, ex)
                else:
...
# =======================
# INDICATORS (same feature parity, 5m-only)
# =======================
...
def _compute_features_5m(df: pd.DataFrame) -> pd.DataFrame:
...
def _load_existing_ohlc(out_path: str, intraday_ts: str) -> pd.DataFrame:
    ...
    try:
        ...
        df["date"] = _to_ist(df["date"])
        # If stored as "start", convert to "end" by shifting +STEP_MIN.
        if intraday_ts.lower() == "end":
...
def _incremental_start_from_existing(out_path: str, default_start: datetime) -> datetime:
    ...
    back = timedelta(minutes=STEP_MIN * WARMUP_BARS_5M)
    s = (last_ts - back)
...
def process_ticker(
    ...
    out_path = os.path.join(OUT_DIR, f"{ticker}_stocks_indicators_5min.parquet")
    ...
    try:
        ...
        logger.exception("[5MIN] %s fetch failed: %s", ticker, e)
    ...
    try:
        ...
        new_rows_path = None
        if new_rows_count > 0:
            rep_dir = os.path.join(report_dir, "missing_rows", "5min")
            os.makedirs(rep_dir, exist_ok=True)
            new_rows_path = os.path.join(rep_dir, f"{ticker}_missing_rows_5min.parquet")
            _finalize_and_save(new_rows, new_rows_path)

            if print_missing_rows:
                show = new_rows.tail(print_missing_rows_max)
                logger.info("[5MIN] %s NEW ROWS (last %d):\n%s",
        ...
        logger.exception("[5MIN] %s indicator/save failed: %s", ticker, e)
        return UpdateReport(ticker, "failed", out_path, existed_before,
                            last_before_ts.strftime("%Y-%m-%d %H:%M:%S") if last_before_ts is not None else None,
                            exp_str, 0, None, None, None)
...
def main():
    ...
    logger.info("=== 5MIN ONLY | intraday_ts=%s | Window: %s â†’ %s (IST) ===",
                args.intraday_ts,
    ...
    if skip_if_fresh:
        for t in syms:
            t = t.upper()
            out_path = os.path.join(OUT_DIR, f"{t}_stocks_indicators_5min.parquet")
        ...
        logger.info("[5MIN] Missing files: %d", len(missing_files))
        if missing_files:
            rep_dir = os.path.join(args.report_dir, "missing_files")
            os.makedirs(rep_dir, exist_ok=True)
            miss_file_path = os.path.join(rep_dir, "missing_files_5min.txt")
            Path(miss_file_path).write_text("\n".join(missing_files), encoding="utf-8")
            logger.info("[5MIN] Missing files list saved: %s", miss_file_path)

        logger.info("[5MIN] Stale/missing symbols to process: %d", len(to_process))
    else:
        to_process = [t.upper() for t in syms]
        logger.info("[5MIN] no-skip enabled => processing all symbols: %d", len(to_process))

    if not to_process:
        logger.info("[5MIN] Nothing to do â€” all symbols are fresh.")
        return
    ...
    logger.info("[5MIN] Processing symbols=%d with max_workers=%d ...", len(work_items), max_workers)
    ...
    if updated_reports:
        logger.info("[5MIN] Updated symbols: %d", len(updated_reports))
        for r in sorted(updated_reports, key=lambda x: x.ticker):
            logger.info(
                "[5MIN] %s %s | last_before=%s | expected=%s | new_rows=%d | new_range=%s â†’ %s | new_rows_store=%s",
                r.ticker, r.status, r.last_before, r.expected, r.new_rows_count, r.new_first, r.new_last, r.new_rows_path
            )
    else:
        logger.info("[5MIN] No new rows appended (everything noop).")

    if failed:
        logger.warning("[5MIN] Failed symbols: %d (see stocks_fetcher_5m_run.log)", failed)
