# -*- coding: utf-8 -*-
"""
two_session_shortlist_test_v2.py
Two-Session Intraday Stock Shortlisting Framework -- Manual Test Runner

Implements: shortlist v2

Changes from v1:
- RVOL acceleration factor (rvol_accel = today_rvol / 3d_rvol): added to scoring (8% weight)
- Morning RS carry-forward in afternoon scoring (morning_rs_long/short as 10% weight)
- SHORT TV minimum restored to Rs.25L (from Rs.20L) for liquidity
- Gap quality filter for morning LONG: gaps > 0.5% must hold through first 3 bars (no fill)
- Setup-specific tier assignment uses component scores (score_mod/cont/huge) not generic score
...
MORNING_RVOL_MIN         = 0.70          # unchanged from v1
MORNING_ORP_ATR_MIN      = 0.07          # unchanged from v1
MORNING_CLV_MIN          = 0.55          # unchanged from v1
MORNING_TV_MIN_LONG      = 2_000_000    # Rs.20 lakhs (LONG — unchanged)
MORNING_TV_MIN_SHORT     = 2_500_000    # Rs.25 lakhs (SHORT — V18c5: restored for liquidity)
MORNING_TV_MIN           = MORNING_TV_MIN_LONG   # backward-compat alias (LONG default)

AFTERNOON_RVOL_MIN       = 0.60          # unchanged from v1
AFTERNOON_ORP_ATR_MIN    = 0.04          # unchanged from v1
AFTERNOON_CLV_MIN        = 0.45          # unchanged from v1
AFTERNOON_TV_MIN_LONG    = 1_500_000    # Rs.15 lakhs (LONG — unchanged)
AFTERNOON_TV_MIN_SHORT   = 2_000_000    # Rs.20 lakhs (SHORT — V18c5: higher liquidity bar)
AFTERNOON_TV_MIN         = AFTERNOON_TV_MIN_LONG   # backward-compat alias (LONG default)

# V18c5: RVOL acceleration lookback (rvol_accel = today_rvol / mean of last N days rvol)
RVOL_ACCEL_LOOKBACK      = 3    # compare today's RVOL against 3-day average
# V18c5: Gap fill detection for morning LONG quality filter
GAP_FILL_DETECT_PCT      = 0.50  # gaps > 0.5% must survive for 3 bars (no fill)
...
MORNING_WEIGHTS = dict(
    rvol=0.10,        # reduced 0.13→0.10 to make room for rvol_accel
    tv_ratio=0.09,    # reduced 0.10→0.09
    orp_atr=0.13,
    clv=0.14,
    gap_hold=0.07,
    rs=0.15,          # reduced 0.16→0.15 to make room for rvol_accel
    vwap_edge=0.12,
    ema20_edge=0.05,
    body_eff=0.09,    # reduced 0.10→0.09
    rvol_accel=0.06,  # V18c5 NEW: RVOL acceleration (today vs 3d avg)
)
AFTERNOON_WEIGHTS = dict(
    rvol=0.10,        # reduced 0.12→0.10
    tv_ratio=0.10,    # reduced 0.12→0.10
    orp_atr=0.10,     # reduced 0.12→0.10
    clv=0.12,         # reduced 0.14→0.12
    rs=0.16,          # reduced 0.20→0.16
    vwap_edge=0.14,   # reduced 0.16→0.14
    ema20_edge=0.06,
    body_eff=0.07,    # reduced 0.08→0.07
    rvol_accel=0.05,  # V18c5 NEW: RVOL acceleration
    morning_rs=0.10,  # V18c5 NEW: morning RS carry-forward for afternoon context
)
