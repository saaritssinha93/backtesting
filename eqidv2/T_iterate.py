"""
T_iterate.py — aggressive iteration + anti-overfit search for the T*/MR* family (the
tier123_balanced overlay setups). MANY iterations. Data source: the tier123 probe
(outputs_ID_v11_tier123_new_setup_probe/tier123_standalone_trades.csv).

Four setups (span 2025-06..2026-05-19; test = 2026-05-01..05-19 only, MR thin):
  - T_TREND_DAY_EMA_STAIR_LONG  (LONG, 2498)   exit 0.70/1.00
  - T_TREND_DAY_EMA_STAIR_SHORT (SHORT, 2498)  exit 0.70/1.00
  - MR_CONTROLLED_VWAP_EXTREME_FADE_LONG  (LONG, 287, test 11)  exit 0.70/0.80
  - MR_CONTROLLED_VWAP_EXTREME_FADE_SHORT (SHORT, 240, test 12) exit 0.70/0.80
Prior tier123 research: all 4 are HOLDOUT losers (PF 0.50-0.88).

Per setup: build entry->EOD PATHS + premom feats; then baseline + greedy(robust) + exhaustive
2-term + 60k random 3-term + FORCED momentum/ADX grid, exit co-optimized. Reports train-PF>=2
(honest test) + robustness-first (min h1/h2/test). Day-block bootstrap AND day-concentration
(top-1-day share) computed on finalists. NET of cost.
Run:  py -3.12 T_iterate.py
"""
...
PROBE = Path(r"C:\TradingData\eqidv2\outputs_ID_v11_tier123_new_setup_probe\tier123_standalone_trades.csv")
PROP = Path(r"C:\TradingData\eqidv2\outputs_ID_v11_traintest_pool\proposals")
CACHE = Path(r"C:\TradingData\eqidv2\v11_T_paths_cache.pkl")
TRAIN_END, TEST_START = "2026-04-30", "2026-05-01"
CFG = CostConfig()
SETUPS = {  # side, production exit, train sample cap
    "T_TREND_DAY_EMA_STAIR_LONG": ("LONG", (0.70, 1.00), 1200),
    "T_TREND_DAY_EMA_STAIR_SHORT": ("SHORT", (0.70, 1.00), 1200),
    "MR_CONTROLLED_VWAP_EXTREME_FADE_LONG": ("LONG", (0.70, 0.80), 10**9),
    "MR_CONTROLLED_VWAP_EXTREME_FADE_SHORT": ("SHORT", (0.70, 0.80), 10**9),
}
POOL_FEATS = ["vol_ratio", "rs_pct", "market_ret_pct", "vwap_dist_atr", "close_loc",
              "quality_score", "ranker_score", "atr_pct", "body_pct", "signal_minute"]
...
def build_paths():
    df = pd.read_csv(PROBE, low_memory=False)
    ...
    for setup, (side, _ex, cap_tr) in SETUPS.items():
        ...
        for r in e.itertuples():
            ...
            paths.append(rec)
        out[setup] = paths
        print(f"[T_iterate] {setup}: {len(paths)} paths ({len(e)} sampled)")
    return out
...
def load_or_build():
    if CACHE.exists():
        with open(CACHE, "rb") as fh:
            out = pickle.load(fh)
        print("[T_iterate] loaded cache: " + ", ".join(f"{k}={len(v)}" for k, v in out.items()))
        return out
    out = build_paths()
...
def _topday(dates, net):
    s = pd.Series(net, index=pd.to_datetime(dates)).groupby(level=0).sum()
    tot = s.sum()
    return round(float(s.max() / tot * 100), 0) if tot > 0 else float('nan')
...
def search(setup, recs):
    ...
    mid = np.sort(dts[is_tr])[is_tr.sum() // 2] if is_tr.sum() else TRAIN_END   # dynamic median split
    ...
    def stats(mask, net, boot=False):
        ...
        if boot:
            out["test_dbp"] = round(_dbp(td.to_numpy(), t), 3)
            out["top1day"] = _topday(td.to_numpy(), t)
        return out
    ...
    for i in fin:
        ...
        for term in res.loc[i, "terms"].split("; "):
            for op in (">=", "<=", "=="):
                if op in term:
                    f, v = term.split(op); f, v = f.strip(), v.strip()
                    col = df[f]
...
def main():
    ...
    for setup in SETUPS:
        ...
        if len(res):
            outs.append(res)
    if outs:
        pd.concat(outs, ignore_index=True).to_csv(PROP / "T_iterate_results.csv", index=False)
    print(f"\n[T_iterate] done -> {PROP}\\T_iterate_results.csv")
    return 0
