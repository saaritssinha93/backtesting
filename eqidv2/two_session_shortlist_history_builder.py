...
import two_session_shortlist_test as shortlist_base
...
DEFAULT_OUTPUT_PATH = SCRIPT_DIR / "filtered_stocks_two_session_history.py"
...
def _apply_morning_shortlist(raw_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if not long_df.empty:
        long_df["score"] = (
            shortlist_base.MORNING_WEIGHTS["rvol"] * long_df.groupby("scan_date")["rvol"].transform(_safe_rank)
            + shortlist_base.MORNING_WEIGHTS["orp_atr"] * long_df.groupby("scan_date")["orp_atr"].transform(_safe_rank)
            + shortlist_base.MORNING_WEIGHTS["clv"] * long_df.groupby("scan_date")["clv_long"].transform(_safe_rank)
            + shortlist_base.MORNING_WEIGHTS["gap_hold"] * long_df.groupby("scan_date")["gap_hold_long"].transform(_safe_rank)
        )
    ...
    if not short_df.empty:
        short_df["score"] = (
            shortlist_base.MORNING_WEIGHTS["rvol"] * short_df.groupby("scan_date")["rvol"].transform(_safe_rank)
            + shortlist_base.MORNING_WEIGHTS["orp_atr"] * short_df.groupby("scan_date")["orp_atr"].transform(_safe_rank)
            + shortlist_base.MORNING_WEIGHTS["clv"] * short_df.groupby("scan_date")["clv_short"].transform(_safe_rank)
            + shortlist_base.MORNING_WEIGHTS["gap_hold"] * short_df.groupby("scan_date")["gap_hold_short"].transform(_safe_rank)
        )
...
def _apply_afternoon_shortlist(raw_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if not long_df.empty:
        long_df["score"] = (
            shortlist_base.AFTERNOON_WEIGHTS["rvol"] * long_df.groupby("scan_date")["rvol"].transform(_safe_rank)
            + shortlist_base.AFTERNOON_WEIGHTS["orp_atr"] * long_df.groupby("scan_date")["orp_atr"].transform(_safe_rank)
            + shortlist_base.AFTERNOON_WEIGHTS["clv"] * long_df.groupby("scan_date")["clv_long"].transform(_safe_rank)
        )
        long_df = long_df.sort_values(["scan_date", "score", "ticker"], ascending=[True, False, True])
    ...
    if not short_df.empty:
        short_df["score"] = (
            shortlist_base.AFTERNOON_WEIGHTS["rvol"] * short_df.groupby("scan_date")["rvol"].transform(_safe_rank)
            + shortlist_base.AFTERNOON_WEIGHTS["orp_atr"] * short_df.groupby("scan_date")["orp_atr"].transform(_safe_rank)
            + shortlist_base.AFTERNOON_WEIGHTS["clv"] * short_df.groupby("scan_date")["clv_short"].transform(_safe_rank)
        )
        short_df = short_df.sort_values(["scan_date", "score", "ticker"], ascending=[True, False, True])
        if shortlist_base.AFTERNOON_TOP_N:
...
def _rows_to_score_maps(df: pd.DataFrame) -> Dict[date, Dict[str, float]]:
    ...
    for scan_date, group in df.groupby("scan_date", sort=True):
        ...
        for row in group.itertuples(index=False):
            ...
            if not ticker:
                continue
            score_map[ticker] = float(getattr(row, "score", 0.0))
...
def _render_module(payload: Dict[str, object]) -> str:
    ...
    return (
        "# Auto-generated by two_session_shortlist_history_builder.py\n"
        "from __future__ import annotations\n\n"
