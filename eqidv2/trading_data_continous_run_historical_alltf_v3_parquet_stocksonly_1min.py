...
**ONLY for intraday 1-minute timeframe** (Parquet storage).
...
Usage examples:
- Fetch 1-min:
    python trading_data_continous_run_historical_alltf_v3_parquet_stocksonly_1min.py 1min
- Same (all aliases to 1min):
    python trading_data_continous_run_historical_alltf_v3_parquet_stocksonly_1min.py all
...
def setup_logger() -> logging.Logger:
    ...
    fh = logging.FileHandler("stocks_fetcher_1min_run.log", mode="w", encoding="utf-8")
    fh.setFormatter(fmt)
...
def fetch_historical_1min_df(kite, token, start_dt_ist, end_dt_ist, logger, intraday_ts):
    # Kite historical minute endpoint works reliably in <=60 day windows.
...
def process_ticker(
    ...
    try:
        ...
        if mode == "1min":
            fetched = fetch_historical_1min_df(kite_client, token, inc_start, end_dt_ist, logger, intraday_ts)
        else:
...
def parse_args():
    ...
    p.add_argument("mode", nargs="?", default="1min", help="1min|all")
    ...
    p.add_argument("--report-dir", default="reports/stocks_missing_reports_1min",
