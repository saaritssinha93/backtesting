...
**ONLY for DAILY timeframe** (Parquet storage).
...
Outputs (Parquet):
- stocks_indicators_daily_eq / <TICKER>_stocks_indicators_daily.parquet

Usage examples:
- Fetch daily:
    python trading_data_continous_run_historical_alltf_v3_parquet_stocksonly_daily.py daily
- Same (all aliases to daily):
...
def setup_logger() -> logging.Logger:
    ...
    fh = logging.FileHandler("stocks_fetcher_daily_run.log", mode="w", encoding="utf-8")
    fh.setFormatter(fmt)
...
def expected_last_stamp(mode: str, now_ist: datetime, holidays: set[date], intraday_ts: str) -> dict:
    ...
    if mode != "daily":
        raise ValueError(f"Unsupported mode for daily script: {mode}")

    exp_ts = last_completed_daily_stamp(now_ist, holidays)
...
# ========= CHANGE FEATURES (daily) =========

def add_change_features_daily(df: pd.DataFrame) -> pd.DataFrame:
...
def _compute_common_features(df: pd.DataFrame, mode: str) -> pd.DataFrame:
    ...
    df = add_change_features_daily(df)
    return df
...
def process_ticker(
    ...
    try:
        ...
        if mode == "daily":
            fetched = fetch_historical_daily_df(kite_client, token, inc_start, end_dt_ist, logger, intraday_ts)
        else:
...
def parse_args():
    ...
    p.add_argument("mode", nargs="?", default="daily", help="daily|all")
    ...
    p.add_argument("--intraday-ts", default=DEFAULT_TIMESTAMP_MODE, choices=["end"],
                   help="Timestamp mode (daily script uses candle end semantics only)")
    ...
    p.add_argument("--report-dir", default="reports/stocks_missing_reports_daily",
