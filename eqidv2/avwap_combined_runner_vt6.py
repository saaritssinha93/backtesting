# -*- coding: utf-8 -*-
"""
avwap_combined_runner_vt6.py
============================

Scenario VT6:
- Derive timer from baseline backtest outcomes.
- Exit only when stalled by timer AND adverse movement confirms against direction.
"""
...
def main() -> None:
    _outputs_dir = _THIS_DIR / "outputs_vt6"
    _outputs_dir.mkdir(parents=True, exist_ok=True)
    _logs_dir = _THIS_DIR / "logs"
    _logs_dir.mkdir(parents=True, exist_ok=True)

    ts = now_ist().strftime("%Y%m%d_%H%M%S")
    log_path = _outputs_dir / f"avwap_combined_runner_vt6_{ts}.txt"
    ...
    with open(log_path, "w", encoding="utf-8") as _log_fh:
        ...
        try:
            print("=" * 72)
            print("AVWAP COMBINED RUNNER VT6")
            ...
            print(
                "[INFO] VT6 exits: timer+stall+adverse model "
                "(timer calibrated from baseline outcomes)."
            )
            ...
            print(
                ...
                f"adverse_ratio_min={adverse_ratio_min_by_side}"
            )

            print("\n[PHASE 3] Re-resolving exits on 1-min bars (VT6 timer+stall+adverse)...")
            ...
            if combined.empty:
                ...
                if not skip_df.empty:
                    skip_csv = _outputs_dir / f"avwap_long_entry_skips_vt6_{ts}.csv"
            ...
            if not short_df.empty:
                print_metrics("SHORT (VT6 timer+stall+adverse)", compute_backtest_metrics(short_df))
            else:
                print("[INFO] SHORT metrics skipped (no short trades).")

            if not long_df.empty:
                print_metrics("LONG (VT6 timer+stall+adverse, anti-chase v3)", compute_backtest_metrics(long_df))
            else:
                print("[INFO] LONG metrics skipped (no long trades).")

            print_metrics("COMBINED (VT6 timer+stall+adverse)", compute_backtest_metrics(combined))
            _print_notional_pnl(combined)

            out_csv = _outputs_dir / f"avwap_longshort_trades_ALL_DAYS_vt6_{ts}.csv"
            combined.to_csv(out_csv, index=False)
            print(f"[FILE SAVED] {out_csv}")

            if not long_df.empty:
                out_long = _outputs_dir / f"avwap_long_trades_only_vt6_{ts}.csv"
            ...
            chart_dir_legacy = _outputs_dir / "charts_vt6" / "legacy"
            chart_dir_enhanced = _outputs_dir / "charts_vt6" / "enhanced"
