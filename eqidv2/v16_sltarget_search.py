...
import avwap_combined_runner_v16 as base


ROOT = Path(__file__).resolve().parent
OUT_DIR = ROOT / "outputs_v16_sltarget_search"
ROUNDTRIP_COST_PCT_FALLBACK = 0.16


def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Search v16 short/long SL+target settings")
...
def _latest_v16_csv() -> Path:
    files = sorted(
        (ROOT / "outputs_v16").glob("avwap_longshort_trades*_ALL_DAYS_*.csv"),
        key=lambda p: p.stat().st_mtime,
    )
    if not files:
        raise FileNotFoundError("No v16 trade CSV found in outputs_v16")
    return files[-1]
    ...
    with redirect_stdout(buffer), redirect_stderr(buffer):
        ...
        if not short_df.empty:
            short_df = base._resolve_exits_5min(
                ...
                eod_exit_time=base.V16_EOD_EXIT_TIME,
            )
