# -*- coding: utf-8 -*-
"""
two_session_shortlist_test.py
Two-Session Intraday Stock Shortlisting Framework -- Manual Test Runner

Implements: two_session_shortlisting_framework_nse.txt (v1.1)
...
# ─────────────────────────────────────────────────────────────────────────────
# CONFIG -- all thresholds from framework v1.1
# ─────────────────────────────────────────────────────────────────────────────
...
MORNING_RVOL_MIN         = 1.0
MORNING_ORP_ATR_MIN      = 0.10
MORNING_CLV_MIN          = 0.62
MORNING_TV_MIN           = 3_000_000    # Rs.30 lakhs (30x Rs.1L position)

AFTERNOON_RVOL_MIN       = 0.8
AFTERNOON_ORP_ATR_MIN    = 0.06
AFTERNOON_CLV_MIN        = 0.52
AFTERNOON_TV_MIN         = 2_000_000    # Rs.20 lakhs (20x Rs.1L position)
...
# Scoring weights
MORNING_WEIGHTS  = dict(rvol=0.35, orp_atr=0.25, clv=0.20, gap_hold=0.10)
AFTERNOON_WEIGHTS = dict(rvol=0.45, orp_atr=0.25, clv=0.30)
...
def morning_factors(ticker: str, df5: pd.DataFrame, today, atr14: float) -> Optional[dict]:
    ...
    close_end  = float(w.sort_values("_time").iloc[-1]["close"])
    ...
    first_open = float(w.sort_values("_time").iloc[0]["open"])
...
def export_shortlist_module(session: str, scan_date, long_list: List[dict], short_list: List[dict]) -> List[Path]:
    ...
    body = (
        "# Auto-generated by two_session_shortlist_test.py\n"
    ...
    paths = [
        SHORTLIST_EXPORT_DIR / f"filtered_stocks_two_session_{session.lower()}.py",
        SHORTLIST_EXPORT_DIR / "filtered_stocks_two_session_latest.py",
    ]
    for path in paths:
...
def main():
    parser = argparse.ArgumentParser(
        description="Two-Session Stock Shortlister -- Manual Test Runner"
    )
    parser.add_argument(
    ...
    print(f"\n{'='*72}")
    print(f"  TWO-SESSION SHORTLISTING FRAMEWORK  --  v1.1 TEST RUNNER")
    ...
    print(f"  TV filter : morning >= Rs.30L  |  afternoon >= Rs.20L")
    print(f"{'='*72}")
    ...
    print(f"    orp_atr  = (H-L of window) / ATR_14d  [>= 0.10 morning, >= 0.06 afternoon]")
    print(f"    clv_l/s  = Close location value for long/short  [>= 0.62 morning, >= 0.52 afternoon]")
    ...
    print(f"    score    = Weighted percentile-rank score (0-1, higher = better)")
    print()
