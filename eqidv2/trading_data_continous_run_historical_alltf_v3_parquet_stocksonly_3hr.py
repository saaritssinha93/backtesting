...
**ONLY for intraday 3-hour timeframe** (Parquet storage).
...
Outputs (Parquet):
- stocks_indicators_3hr_eq / <TICKER>_stocks_indicators_3hr.parquet

Usage examples:
- Fetch 3-hour:
    python trading_data_continous_run_historical_alltf_v3_parquet_stocksonly_3hr.py 3hr
- Same (all aliases to 3hr):
    python trading_data_continous_run_historical_alltf_v3_parquet_stocksonly_3hr.py all

Notes:
- Intraday timestamps can be stored as candle "end" (recommended) or "start".
- Fixed historical window start: 2025-08-25 (IST), until latest completed 3-hour candle.
...
# Directories (only intraday 3h)
DIRS = {
    "3hr": {"cache": "stocks_cache_3hr_eq", "out": "stocks_indicators_3hr_eq"},
}
...
MARKET_CLOSE_TIME_INTRADAY = time(15, 30)      # last intraday candle end
...
def setup_logger() -> logging.Logger:
    ...
    fh = logging.FileHandler("stocks_fetcher_3hr_run.log", mode="w", encoding="utf-8")
    fh.setFormatter(fmt)
...
def _intraday_end_shift_minutes(interval: str) -> int:
    return {"60minute": 60}.get(interval, 0)
...
def _fetch_historical_1h_base_df(kite, token, start_dt_ist, end_dt_ist, logger, intraday_ts):
...
def _resample_3hr_from_1h(df: pd.DataFrame) -> pd.DataFrame:
    """
    Build 3-hour bars from 1-hour bars using per-session fixed chunks of 3 candles.
    ...
    w["session_day"] = w["date"].dt.tz_convert(IST_TZ).dt.date

    parts: list[pd.DataFrame] = []
    for _, g in w.groupby("session_day", sort=True):
        g = g.sort_values("date").reset_index(drop=True)
        full_groups = len(g) // 3
        if full_groups <= 0:
            continue
        g = g.iloc[: full_groups * 3].copy()
        g["grp"] = np.arange(len(g)) // 3
        agg = (
            g.groupby("grp", sort=True)
             .agg(
                 date=("date", "last"),
                 open=("open", "first"),
                 high=("high", "max"),
...
def fetch_historical_3hr_df(kite, token, start_dt_ist, end_dt_ist, logger, intraday_ts):
    base_1h = _fetch_historical_1h_base_df(kite, token, start_dt_ist, end_dt_ist, logger, intraday_ts)
    if base_1h.empty:
        return pd.DataFrame()
    out = _resample_3hr_from_1h(base_1h)
    out = out[out["date"] <= end_dt_ist].reset_index(drop=True)
    return out
...
def _incremental_start_from_existing(mode: str, out_path: str, default_start: datetime) -> datetime:
    ...
    if mode == "3hr":
        back = timedelta(minutes=180 * warm)
    else:
...
def process_ticker(
    ...
    try:
        ...
        if mode == "3hr":
            fetched = fetch_historical_3hr_df(kite_client, token, inc_start, end_dt_ist, logger, intraday_ts)
        else:
...
def parse_args():
    ...
    p.add_argument("mode", nargs="?", default="3hr", help="3hr|all")
    p.add_argument("--max-workers", type=int, default=DEFAULT_MAX_WORKERS)
    p.add_argument("--no-skip", action="store_true",
                   help="Disable freshness skip (will refetch/recompute even if fresh)")
    p.add_argument("--intraday-ts", default=DEFAULT_INTRADAY_TIMESTAMP, choices=["end"],
                   help="Store timestamps as candle end (required for 3-hour chunking)")
    ...
    p.add_argument("--report-dir", default="reports/stocks_missing_reports_3hr",
