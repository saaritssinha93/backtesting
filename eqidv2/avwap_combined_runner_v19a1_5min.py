# -*- coding: utf-8 -*-
"""
avwap_combined_runner_v19a1_5min.py - AVWAP v19a1 standalone 5-minute runner
=============================================================================

Standalone v19a1 runner — fully self-contained, no dependency on v18c6 code.
Based on v19a with setup-specific exit profiles retuned from baseline analysis.
...
- v19a1 setup-specific exit levels (Area 1 retuning, 2026-04-15):
    SHORT CONTINUATION:  SL=0.55%, TGT=1.20%  (TGT lowered from 1.80% — hit-rate fix)
    SHORT A_MOD:         SL=0.65%, TGT=1.50%  (TGT lowered from 1.90%)
    SHORT B_HUGE:        SL=0.85%, TGT=1.40%  (unchanged, sweep-optimised)
    LONG standard:       SL=0.65%, TGT=1.30%  (TGT lowered from 1.50%)
    LONG RECLAIM:        SL=0.85%, TGT=1.20%  (SL widened + TGT lowered from 0.70%/1.85%)
"""
...
try:
    ...
    print("[V19A1] Using shortlist v3_3times (filtered_stocks_two_session_history_v3_3times)")
except ImportError:
    ...
    print(
        "[V19A1] WARNING: filtered_stocks_two_session_history_v3_3times not found. "
...
FINAL_LONG_USE_TIME_WINDOWS = True
FINAL_SHORT_SIGNAL_WINDOWS = [
    (V19A_MORNING_WINDOW_START, V19A_MORNING_WINDOW_END),
    (V19A_MID_WINDOW_START, V19A_MID_WINDOW_END),
    (V19A_AFTERNOON_WINDOW_START, V19A_AFTERNOON_WINDOW_END),
]
FINAL_LONG_SIGNAL_WINDOWS = list(FINAL_SHORT_SIGNAL_WINDOWS)
...
V19A_LONG_RECLAIM_STOP_PCT = 0.0085 # RECLAIM SL: 0.70% → 0.85%
V19A_LONG_RECLAIM_TARGET_PCT = 0.0120 # RECLAIM TGT: 1.85% → 1.20%
...
def runtime_dir(*parts: str) -> Path:
    return _ORIG_V19A_RUNTIME_DIR("outputs_v19a1_5min")
...
def _rename_new_outputs(output_dir: Path, before: set[str]) -> List[Path]:
    ...
    for path in output_dir.iterdir():
        ...
        new_name = (
            path.name
            .replace("v18c5_5min", "v19a1_5min")
            .replace("v19a_5min", "v19a1_5min")
            .replace("avwap_combined_runner_v18c3_", "avwap_combined_runner_v19a1_")
            .replace("avwap_combined_runner_v19a_", "avwap_combined_runner_v19a1_")
        )
...
def main() -> None:
    output_dir = runtime_dir("outputs_v19a1_5min")
    output_dir.mkdir(parents=True, exist_ok=True)
    before = {p.name for p in output_dir.iterdir() if p.is_file()}
    print("[V19A1] Starting standalone backtest with three-window shortlist (v3_3times)...")
    print("[V19A1] Area-1 retuned exit profiles active:")
    print("[V19A1]   SHORT CONTINUATION  SL=0.55%  TGT=1.20%")
    print("[V19A1]   SHORT A_MOD         SL=0.65%  TGT=1.50%")
    print("[V19A1]   SHORT B_HUGE        SL=0.85%  TGT=1.40%")
    print("[V19A1]   LONG standard       SL=0.65%  TGT=1.30%")
    print("[V19A1]   LONG RECLAIM        SL=0.85%  TGT=1.20%")
    _ORIG_V19A_MAIN()
    renamed = _rename_new_outputs(output_dir, before)
    if renamed:
        print("[V19A1] Output files:")
        for path in renamed:
            print(f"[V19A1]   {path}")
