# -*- coding: utf-8 -*-
"""
avwap_combined_runner_v19a2_5min.py - AVWAP v19a2 standalone 5-minute runner
=============================================================================

Standalone v19a2 runner — fully self-contained, no dependency on v18c6 code.
Based on v19a1 with RECLAIM exit fix + Area 2 entry cutoffs.
...
- v19a2 changes vs v19a1 (2026-04-15):
    LONG RECLAIM:  SL=0.70% (restored), TGT=1.40% (tighter than old 1.85%)
    SHORT signals: blocked 10:35-11:05 (removes -44% dead zone entries 10:40-11:05)
    LONG signals:  09:45 signal blocked (removes losing 09:50 entries, PF<1)
- Inherited from v19a1:
...
try:
    ...
    print("[V19A2] Using shortlist v3_3times (filtered_stocks_two_session_history_v3_3times)")
except ImportError:
    ...
    print(
        "[V19A2] WARNING: filtered_stocks_two_session_history_v3_3times not found. "
...
def runtime_dir(*parts: str) -> Path:
    return _ORIG_V19A_RUNTIME_DIR("outputs_v19a2_5min")
...
def _rename_new_outputs(output_dir: Path, before: set[str]) -> List[Path]:
    ...
    for path in output_dir.iterdir():
        ...
        new_name = (
            path.name
            .replace("v18c5_5min", "v19a2_5min")
            .replace("v19a1_5min", "v19a2_5min")
            .replace("v19a_5min", "v19a2_5min")
            .replace("avwap_combined_runner_v18c3_", "avwap_combined_runner_v19a2_")
            .replace("avwap_combined_runner_v19a1_", "avwap_combined_runner_v19a2_")
            .replace("avwap_combined_runner_v19a_", "avwap_combined_runner_v19a2_")
        )
...
def main() -> None:
    output_dir = runtime_dir("outputs_v19a2_5min")
    output_dir.mkdir(parents=True, exist_ok=True)
    before = {p.name for p in output_dir.iterdir() if p.is_file()}
    print("[V19A2] Starting standalone backtest with three-window shortlist (v3_3times)...")
    print("[V19A2] Changes vs v19a1:")
    print("[V19A2]   LONG RECLAIM        SL=0.70%  TGT=1.40%  (RECLAIM fix)")
    print("[V19A2]   SHORT signals blocked 10:35-11:05 (dead zone)")
    print("[V19A2]   LONG  signal  blocked 09:45       (09:50 losers)")
    print("[V19A2] Inherited exit profiles:")
    print("[V19A2]   SHORT CONTINUATION  SL=0.55%  TGT=1.20%")
    print("[V19A2]   SHORT A_MOD         SL=0.65%  TGT=1.50%")
    print("[V19A2]   SHORT B_HUGE        SL=0.85%  TGT=1.40%")
    print("[V19A2]   LONG standard       SL=0.65%  TGT=1.30%")
    _ORIG_V19A_MAIN()
    renamed = _rename_new_outputs(output_dir, before)
    if renamed:
        print("[V19A2] Output files:")
        for path in renamed:
            print(f"[V19A2]   {path}")
