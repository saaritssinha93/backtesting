...
def autologin() -> None:
    ...
    try:
        ...
        # Exchange request_token for access_token so downstream jobs can run.
        sess = kite.generate_session(request_token, api_secret=key_secret[1])
        access_token = str(sess.get("access_token", "")).strip()
        if not access_token:
            raise RuntimeError("generate_session() did not return access_token")
        kite.set_access_token(access_token)
        (ROOT / "access_token.txt").write_text(access_token + "\n", encoding="utf-8")
        print(f"[OK] access_token saved -> {ROOT / 'access_token.txt'}", flush=True)
    except TimeoutException as exc:
        raise RuntimeError(
