...
"""
EQIDV2 LIVE Scanner V7 SWEEP LONG (long-only anti-chase split pipeline)
==================================================================
...
4. Isolate all outputs/state with `v7_sweep_long` suffix.
"""
...
# -----------------------------------------------------------------------------
# V7_SWEEP_LONG ANTI-CHASE CONFIG
# -----------------------------------------------------------------------------
LONG_LIMIT_WAIT_MIN = int(os.getenv("EQIDV7_SWEEP_LONG_LIMIT_WAIT_MIN", "60"))
LONG_LIMIT_OFFSET_PCT = float(os.getenv("EQIDV7_SWEEP_LONG_LIMIT_OFFSET_PCT", "-0.005"))  # -0.5%
LONG_STOP_PCT = float(os.getenv("EQIDV7_SWEEP_LONG_STOP_PCT", "0.0058"))                   # 0.58%
LONG_TARGET_PCT = float(os.getenv("EQIDV7_SWEEP_LONG_TARGET_PCT", "0.011"))                # 1.1%

# Optional signal-quality filters before putting LONG into pending queue.
LONG_RSI_CAP_RAW = os.getenv("EQIDV7_SWEEP_LONG_RSI_CAP", "").strip()
LONG_ADX_MIN_RAW = os.getenv("EQIDV7_SWEEP_LONG_ADX_MIN", "").strip()
LONG_QUALITY_MIN_RAW = os.getenv("EQIDV7_SWEEP_LONG_QUALITY_MIN", "").strip()
...
PENDING_STATE_FILE = ROOT / "logs" / "eqidv2_long_pending_state_v7_sweep_long.json"
...
LONG_PENDING_POLL_ENABLED = _env_bool("EQIDV7_SWEEP_LONG_PENDING_POLL_ENABLED", True)
LONG_PENDING_POLL_INTERVAL_SEC = max(
    1.0,
    float(os.getenv("EQIDV7_SWEEP_LONG_PENDING_POLL_INTERVAL_SEC", "5")),
)
...
def _disable_kite_rebase_write(signals_df: pd.DataFrame) -> int:
    ...
    We already finalize entry/SL/TGT in v7_sweep_long pending-fill logic.
    """
...
def _latest_entry_signals_for_ticker_v7_sweep_long(
    ticker: str,
    df_raw: pd.DataFrame,
    state: Dict[str, Any],
...
def _scan_short_disabled(*_args, **_kwargs):
    """v7_sweep_long must not compute SHORT side internals."""
    return []
...
def _process_signals_and_pending_v7_sweep_long(
...
def _write_signals_csv_v7_sweep_long(signals_df: pd.DataFrame) -> int:
    return _process_signals_and_pending_v7_sweep_long(
        signals_df,
        log_label="[V7_SWEEP_LONG CSV]",
        log_noop=True,
    )


def _pending_poll_once_v7_sweep_long() -> int:
    return _process_signals_and_pending_v7_sweep_long(
        pd.DataFrame(),
        log_label="[V7_SWEEP_LONG PENDING]",
        log_noop=False,
    )


def _start_pending_poll_worker_v7_sweep_long() -> Optional[threading.Event]:
    ...
    def _worker() -> None:
        print(
            f"[V7_SWEEP_LONG PENDING] worker_started | interval={LONG_PENDING_POLL_INTERVAL_SEC:.1f}s",
            flush=True,
        )
        while not stop_event.is_set():
            ...
            if in_poll_window:
                try:
                    _pending_poll_once_v7_sweep_long()
                except Exception as exc:
                    print(f"[V7_SWEEP_LONG PENDING] poll_error: {exc}", flush=True)
    ...
    t = threading.Thread(
        target=_worker,
        name="eqidv2-v7-sweep-long-pending-poller",
        daemon=True,
    )
    t.start()
    return stop_event
...
def _apply_v7_sweep_long_overrides() -> None:
    ...
    v2.STATE_FILE = ROOT / "logs" / "eqidv2_avwap_live_state_v11_v7_sweep_long.json"
    v2.SIGNAL_CSV_PATTERN = "signals_{}_v7_sweep_long.csv"
    ...
    # Keep Kite enabled because v7_sweep_long pending queue relies on LTP.
    ...
    v2._latest_entry_signals_for_ticker = _latest_entry_signals_for_ticker_v7_sweep_long
    v2._write_signals_csv = _write_signals_csv_v7_sweep_long

    # Ensure per-scan parquet outputs get explicit `_v7_sweep_long` filename suffix.
    def _run_one_scan_v7_sweep_long(run_tag: str = "A"):
        ...
        def _rename_latest(folder: Path, prefix: str, tag: str) -> None:
            ...
            if not candidates:
                return
            src = candidates[-1]
            if src.stem.endswith("_v7_sweep_long"):
                return
            dst = src.with_name(src.stem + "_v7_sweep_long" + src.suffix)
            try:
                if dst.exists():
                    dst.unlink()
                src.rename(dst)
        ...
        if STALE_ONLY_RETRY_ENABLED:
            ...
            if stale_tickers:
                ...
                print(
                    f"[V7_SWEEP_LONG RETRY] stale_tickers={len(stale_tickers)} | "
                    f"rerun_subset_tag={retry_tag}",
                    flush=True,
                )
                print(
                    f"[V7_SWEEP_LONG RETRY] stale_ticker_names={','.join(stale_tickers)}",
                    flush=True,
                )
                ...
                print(
                    f"[V7_SWEEP_LONG RETRY] done | extra_checks={0 if checks_retry is None else len(checks_retry)} "
    ...
    v2.run_one_scan = _run_one_scan_v7_sweep_long

    # Ensure replay default output filename carries `_v7_sweep_long`.
    def _run_replay_for_date_v7_sweep_long(date_str: str, out_csv: Optional[str] = None) -> pd.DataFrame:
        if out_csv is None:
            out_csv = str(v2.OUT_SIGNALS_DIR / f"replay_signals_{date_str}_v7_sweep_long.csv")
        return _ORIG_RUN_REPLAY_FOR_DATE(date_str, out_csv=out_csv)

    v2.run_replay_for_date = _run_replay_for_date_v7_sweep_long


def main() -> None:
    _apply_v7_sweep_long_overrides()
    ...
    if not is_help_mode:
        print(
            "[V7_SWEEP_LONG] LONG-only anti-chase enabled | "
        ...
        if LONG_RSI_CAP is not None:
            print(f"[V7_SWEEP_LONG] LONG_RSI_CAP={LONG_RSI_CAP}", flush=True)
        if LONG_ADX_MIN is not None:
            print(f"[V7_SWEEP_LONG] LONG_ADX_MIN={LONG_ADX_MIN}", flush=True)
        if LONG_QUALITY_MIN is not None:
            print(f"[V7_SWEEP_LONG] LONG_QUALITY_MIN={LONG_QUALITY_MIN}", flush=True)

    poll_stop: Optional[threading.Event] = None
    if should_start_pending_worker:
        poll_stop = _start_pending_poll_worker_v7_sweep_long()
    elif (not is_help_mode) and LONG_PENDING_POLL_ENABLED and is_replay_mode:
        print("[V7_SWEEP_LONG] pending worker skipped in replay mode.", flush=True)
