# -*- coding: utf-8 -*-
"""
avwap_combined_runner_v2.py — AVWAP v11 COMBINED LONG + SHORT runner (optimised v2)
====================================================================================

Upgraded from avwap_combined_runner.py with the following parameter improvements:

RISK / R:R:
  - SHORT SL  0.90% → 0.75%  |  SHORT TGT  1.20% → 1.50%  →  R:R 1.33:1 → 2.0:1
  - LONG  SL  0.60% (keep)   |  LONG  TGT  1.20% → 1.80%  →  R:R 2.0:1  → 3.0:1

BREAKEVEN / TRAIL (tighter protection):
  - SHORT BE trigger 0.50% → 0.40%  |  SHORT trail 0.30% → 0.22%
  - LONG  BE trigger 0.60% → 0.45%  |  LONG  trail 0.30% → 0.22%

TREND FILTERS (relaxed for more entries, ~4-8/day target):
  - SHORT ADX min 22 → 20  |  SHORT ADX slope 1.0 → 0.75
  - SHORT RSI max 58 → 62  |  SHORT Stoch max  75 → 80
  - LONG  ADX min 22 → 18  |  LONG  ADX slope 1.25 → 0.60  ← biggest lever
  - LONG  RSI min 38 → 35  |  LONG  Stoch min  30 → 22

SIGNAL WINDOWS (extended by 30 min):
  - Both sides:  09:15 – 15:00  (was 09:15 – 14:30)

PULLBACK LAG (faster entry):
  - pullback setup lag 3 bars → 2 bars  (both sides)

AVWAP DISTANCE (more permissive):
  - avwap_dist_atr_mult  0.10 → 0.05

OTHER:
  - DISALLOW_BOTH_SIDES_SAME_TICKER_DAY = True  (was False; avoids conflicting signals)
  - ATR_PCT_MIN 0.0015 → 0.0018  (slightly tighter volatility gate)

Outputs saved to  outputs_v2/  (separate from v1 outputs/).
"""
...
# ===========================================================================
# RUNNER CONFIG v2 — all optimised constants live here
# ===========================================================================
...
FINAL_SHORT_SIGNAL_WINDOWS = [
    (dtime(9, 15, 0), dtime(15, 0, 0))   # extended from 14:30
]
...
# ---- Risk (improved R:R) ----
SHORT_STOP_PCT   = 0.0075   # was 0.0090  →  R:R now 2.0:1
SHORT_TARGET_PCT = 0.0150   # was 0.0120
LONG_STOP_PCT    = 0.0060   # unchanged
LONG_TARGET_PCT  = 0.0180   # was 0.0120  →  R:R now 3.0:1

# ---- Breakeven / trailing stop (tighter) ----
SHORT_BE_TRIGGER_PCT = 0.0040   # was 0.0050 — arm BE sooner
SHORT_TRAIL_PCT      = 0.0022   # was 0.0030 — lock-in more
LONG_BE_TRIGGER_PCT  = 0.0045   # was 0.0060
LONG_TRAIL_PCT       = 0.0022   # was 0.0030

# ---- Trend filters (relaxed for ~4-8 entries/day) ----
SHORT_ADX_MIN        = 20.0    # was 22.0
SHORT_ADX_SLOPE_MIN  = 0.75    # was 1.0
SHORT_RSI_MAX        = 62.0    # was 58.0
SHORT_STOCHK_MAX     = 80.0    # was 75.0

LONG_ADX_MIN         = 18.0    # was 22.0
LONG_ADX_SLOPE_MIN   = 0.60    # was 1.25  ← biggest entry-count lever
LONG_RSI_MIN         = 35.0    # was 38.0
LONG_STOCHK_MIN      = 22.0    # was 30.0
LONG_STOCHK_MAX      = 98.0    # unchanged

# ---- Volume / ATR ----
USE_VOLUME_FILTER  = True
VOLUME_MIN_RATIO   = 1.0
USE_ATR_PCT_FILTER = True
ATR_PCT_MIN        = 0.0018    # was 0.0015 — tighter volatility gate

# ---- AVWAP ----
AVWAP_DIST_ATR_MULT = 0.05    # was 0.10 — more permissive distance
    ...
    if max_workers <= 1:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                if done_count % 100 == 0:
                    print(f"  [{side}] scanned {done_count}/{len(tickers)} | trades={len(all_dicts)}")
...
# ===========================================================================
# ENHANCED CHARTING SUITE (20 charts — identical to v1 runner)
# ===========================================================================
def generate_enhanced_charts(
    ...
    ax.set_title("Cumulative P&L (Rs.) — v2 Optimised", fontsize=14, fontweight="bold")
    ax.set_xlabel("Trade #")
    ...
    if "trade_date" in combined_sorted.columns:
        ...
        ax.axhline(0, color="grey", linewidth=0.8, linestyle="--")
        ax.set_title("Daily Net P&L (Rs.) — v2", fontsize=14, fontweight="bold")
    ...
    if len(cum_pnl) > 0:
        ...
        ax1.set_title("Equity Curve & Drawdown — v2", fontsize=14, fontweight="bold")
    ...
    if "side" in combined_sorted.columns:
        ...
        if sides_data:
            ...
            ax.set_title("Win Rate by Side — v2", fontsize=14, fontweight="bold")
    ...
    if len(pnl_vals) > 0:
        ...
        ax.set_title("P&L Distribution (%) — All Trades v2", fontsize=14, fontweight="bold")
        ax.set_xlabel("P&L (%)")
    ...
    ax.axvline(0, color="grey", linewidth=1, linestyle="-")
    ax.set_title("P&L Distribution by Side (Overlay) — v2", fontsize=14, fontweight="bold")
    ax.set_xlabel("P&L (%)")
    ax.set_ylabel("Frequency")
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3, axis="y")
    ...
    if "outcome" in combined_sorted.columns:
        ...
        ax.set_title("Trade Outcome Breakdown — v2", fontsize=14, fontweight="bold")
    ...
    if "outcome" in combined_sorted.columns and "side" in combined_sorted.columns:
        ...
        ax.set_title("Outcome Breakdown by Side — v2", fontsize=14, fontweight="bold")
    ...
    if "trade_date" in combined_sorted.columns:
        ...
        if len(monthly) > 1:
            ...
            ax.axhline(0, color="grey", linewidth=0.8, linestyle="--")
            ax.set_title("Monthly P&L (Rs.) — v2", fontsize=14, fontweight="bold")
            ax.set_ylabel("P&L (Rs.)")
            ax.grid(True, alpha=0.3, axis="y")
            ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"₹{x:,.0f}"))
    ...
    if len(pnl_pct) >= 20:
        ...
        ax.set_title(f"Rolling Sharpe ({rolling_window}-trade window) — v2", fontsize=14, fontweight="bold")
        ax.set_xlabel("Trade #")
    ...
    if {"entry_time_ist", "exit_time_ist"}.issubset(combined_sorted.columns):
        ...
        if len(durations_min) > 0:
            ...
            ax.set_title("Trade Duration Distribution (minutes) — v2", fontsize=14, fontweight="bold")
    ...
    if "pnl_rs" in combined_sorted.columns and "ticker" in combined_sorted.columns:
        ...
        if len(top_win) > 0:
            ...
            ax1.set_title("Top 10 Winners (Rs.) — v2", fontsize=12, fontweight="bold")
        ...
        if len(top_loss) > 0:
            ...
            ax2.set_title("Top 10 Losers (Rs.) — v2", fontsize=12, fontweight="bold")
    ...
    for col_name, chart_num, title in [
        ...
        if col_name in combined_sorted.columns:
            ...
            if len(grp) > 0 and len(grp) <= 20:
                ...
                ax1.set_title(f"Total {title} (Rs.) — v2", fontsize=12, fontweight="bold")
                ...
                ax2.set_title(f"Avg {title} per Trade (Rs.) — v2", fontsize=12, fontweight="bold")
    ...
    if "quality_score" in combined_sorted.columns:
        ...
        if mask.sum() > 5:
            ...
            ax.axhline(0, color="grey", linewidth=0.8, linestyle="--")
            ax.set_title("Quality Score vs P&L (%) — v2", fontsize=14, fontweight="bold")
    ...
    if "entry_time_ist" in combined_sorted.columns:
        ...
        if len(hourly_pnl) > 0:
            ...
            ax1.set_title("Avg P&L per Trade by Entry Hour — v2", fontsize=12, fontweight="bold")
            ...
            ax2.set_title("Total P&L by Entry Hour — v2", fontsize=12, fontweight="bold")
...
def main() -> None:
    ...
    _outputs_dir = _project_root / "outputs_v2"
    _outputs_dir.mkdir(parents=True, exist_ok=True)

    ts = now_ist().strftime("%Y%m%d_%H%M%S")
    log_path = _outputs_dir / f"avwap_combined_runner_v2_{ts}.txt"
    ...
    with open(log_path, "w", encoding="utf-8") as _log_fh:
        ...
        try:
            print("=" * 72)
            print("AVWAP v11 COMBINED runner v2 — LONG + SHORT (optimised)")
            ...
            # Build configs with v2 overrides
            ...
            # PHASE 1: Entry signals (15-min)
            print("\n[PHASE 1] Scanning entry signals using 15-min data...")
            ...
            # Metrics
            if not short_df.empty:
                print_metrics("SHORT v2 (5-min exits)", compute_backtest_metrics(short_df))
            if not long_df.empty:
                print_metrics("LONG v2 (5-min exits)", compute_backtest_metrics(long_df))
            print_metrics("COMBINED v2 (5-min exits)", compute_backtest_metrics(combined))
            ...
            # Save CSV
            out_csv = _outputs_dir / f"avwap_longshort_trades_ALL_DAYS_v2_{ts}.csv"
            combined.to_csv(out_csv, index=False)

            # Charts
            print("\n[INFO] Generating charts...")
            chart_dir_legacy   = _outputs_dir / "charts_v2" / "legacy"
            chart_dir_enhanced = _outputs_dir / "charts_v2" / "enhanced"
            legacy_files = generate_backtest_charts(combined, short_df, long_df, save_dir=chart_dir_legacy, ts_label=f"{ts}_v2")
            enhanced_files = generate_enhanced_charts(combined, short_df, long_df, save_dir=chart_dir_enhanced, ts_label=f"{ts}_v2")
